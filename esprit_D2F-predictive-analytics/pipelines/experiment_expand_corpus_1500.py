"""Experience : expansion du corpus auteur (217 lignes) vers 1 500 lignes.

Variante de experiment_expand_corpus_1000.py avec TARGET_ROWS = 1 500 et
stockage effectif des IC95 (correction : dans la version 1000 le delta RMSE
vs MLP etait calcule mais non enregistre dans le rapport).

Meme protocole honnete :
- re-echantillonnage des 217 lignes reelles avec bruit calibre (re-mesures) ;
- anti-fuite par paire source (train/test disjoint en paires) ;
- baseline 217 et expansion 1 500 sous le MEME protocole (seed 42).
"""
...
REPORT = BASE / "reports" / "expand_corpus_1500_results.json"
...
def run_protocol(X_train, y_train, X_test, y_test, label: str) -> dict:
    ...
    for name, p in preds.items():  # IC95 des deltas vs MLP (stocke cette fois)
        if name == "MLP_32_16":
            continue
        lo, hi, mean_d = bootstrap_ci(y_arr, preds["MLP_32_16"], p)
        out[name]["delta_rmse_vs_mlp"] = {
            "mean": mean_d,
            "ic95": [lo, hi],
...
def main() -> dict:
    ...
    results["expanded_1500"] = run_protocol(
        exp_train[FEATURES], exp_train[TARGET],
        exp_test[FEATURES], exp_test[TARGET], "expanded_1500")
    results["expanded_1500"]["n_source_pairs_test"] = int(len(test_src))
    results["honest_note"] = (
        ...
        "auteur. Valeur : effet du volume, IC95 plus serre (test 300 lignes), "
    ...
    for label in ("baseline_217", "expanded_1500"):
        r = results[label]
