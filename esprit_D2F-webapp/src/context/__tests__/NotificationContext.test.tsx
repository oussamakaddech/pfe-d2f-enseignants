...
describe('NotificationContext', () => {
  ...
  it('reflète le statut du transport via onStatus', async () => {
    ...
    act(() => {
      transportHandlers.onStatus('open');
    });
    expect((ctx as { status: string }).status).toBe('open');
    act(() => {
      transportHandlers.onError();
    });
  ...
  it('markAsRead updates local state and calls service', async () => {
    ...
    await act(async () => {
      (ctx as { markAsRead: (id: string) => void }).markAsRead('n1');
    });
  ...
  it('markAsRead ignore un id inexistant', async () => {
    ...
    await act(async () => {
      (ctx as { markAsRead: (id: string) => void }).markAsRead('nope');
    });
  ...
  it('markAllAsRead marks every notification read', async () => {
    ...
    await act(async () => {
      (ctx as { markAllAsRead: () => void }).markAllAsRead();
    });
  ...
  it('remove deletes a notification', async () => {
    ...
    await act(async () => {
      (ctx as { remove: (id: string) => void }).remove('n1');
    });
    const c = ctx as { notifications: { id: string }[] };
  ...
  it('remove ignore un id inexistant', async () => {
    ...
    await act(async () => {
      (ctx as { remove: (id: string) => void }).remove('nope');
    });
  ...
  it('addNotification avec recipient fait un fallback local si create échoue', async () => {
    (notificationService.create as ReturnType<typeof vi.fn>).mockRejectedValueOnce(
      new Error('no rights'),
    );
    let ctx: unknown;
    render(
