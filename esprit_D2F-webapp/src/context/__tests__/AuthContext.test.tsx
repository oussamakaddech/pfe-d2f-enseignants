...
const renderWithCapture = () => {
  ...
  const utils = render(
    <AuthProvider>
      <Capture onCtx={(c) => { ref.current = c; }} />
    </AuthProvider>,
  );
  return { utils, getCtx: () => ref.current as { user: unknown; login: (u: unknown) => void; logout: () => void } };
};
...
describe("AuthContext", () => {
  beforeEach(() => {
    ...
    (refreshTokenApi as ReturnType<typeof vi.fn>).mockResolvedValue({
      userId: 1, username: "u", role: "ENSEIGNANT", email: "e", expiresIn: 100,
    });
  });
  ...
  it("login sets the user and persists to sessionStorage", async () => {
    (refreshTokenApi as ReturnType<typeof vi.fn>).mockResolvedValue({
      userId: 9, username: "x", role: "ADMIN", email: "e", expiresIn: 100,
    });
    const { getCtx } = renderWithCapture();
    await act(async () => {
  ...
  it("doRefresh updates the user on success", async () => {
    (refreshTokenApi as ReturnType<typeof vi.fn>).mockResolvedValue({
      userId: 1, username: "refreshed", role: "ENSEIGNANT", email: "e", expiresIn: 100,
    });
    const { getCtx } = renderWithCapture();
    await act(async () => {
      getCtx().login({ userId: 1, username: "a", role: "ENSEIGNANT" } as never);
