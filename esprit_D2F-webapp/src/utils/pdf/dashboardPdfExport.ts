...
import type { CompetenceDemandee, BesoinParDept, BesoinPriorise, TauxReussiteDomaine } from "@/hooks/dashboard/useCupDashboard";
...
export function generateDashboardPdf(data: DashboardPdfData): void {
  ...
  autoTable(doc, {
    ...
    columnStyles: { 0: { cellWidth: contentW * 0.6 }, 1: { cellWidth: contentW * 0.4, halign: "right", fontStyle: "bold" } },
  });
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `D2F — Plateforme ESPRIT · Page ${i}/${totalPages}`,
      pageW / 2,
      290,
      { align: "center" }
    );
