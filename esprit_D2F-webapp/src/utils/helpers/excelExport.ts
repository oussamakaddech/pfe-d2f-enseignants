...
export function styledSheet(
  ...
  let metaRows = 0;
  if (hasTitle) {
    ws['!merges'].push({ s: { r: metaRows, c: 0 }, e: { r: metaRows, c: ncols - 1 } });
    metaRows++;
  }
  if (hasSub) {
    ws['!merges'].push({ s: { r: metaRows, c: 0 }, e: { r: metaRows, c: ncols - 1 } });
    ...
    rowArr.forEach((_, col) => {
      ...
      if (hasTitle && R === 0) {
        ...
        cell.s = {
          ...
          border: {
            top: border(C.headerBorder),
            bottom: border(C.headerBorder),
            left: border(C.headerBorder),
            right: border(C.headerBorder),
          },
