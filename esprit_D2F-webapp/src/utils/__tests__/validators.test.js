...
describe('validators — Ant Design rules', () => {
  ...
  describe('positiveNumberRule', () => {
    it('resolves for valid positive number', async () => {
      ...
      await expect(rule.validator(null, 5)).resolves.toBeUndefined();
    });
    it('resolves for empty value', async () => {
      const rule = positiveNumberRule();
      await expect(rule.validator(null, undefined)).resolves.toBeUndefined();
    });
    it('treats zero as empty (falsy) and resolves', async () => {
      const rule = positiveNumberRule();
      await expect(rule.validator(null, 0)).resolves.toBeUndefined();
    });
    it('rejects for negative number', async () => {
      const rule = positiveNumberRule();
      await expect(rule.validator(null, -5)).rejects.toThrow('La valeur doit être un nombre positif');
    });
  });

  describe('codeFormatRule', () => {
    it('resolves for empty value', async () => {
      await expect(codeFormatRule().validator(null, '')).resolves.toBeUndefined();
    });
    it('resolves for valid code', async () => {
      await expect(codeFormatRule().validator(null, 'INF-01')).resolves.toBeUndefined();
    });
    it('rejects for invalid code', async () => {
      await expect(codeFormatRule('msg').validator(null, 'bad code!')).rejects.toThrow('msg');
    });
  });

  describe('rangeRule', () => {
    it('resolves for value in range', async () => {
      await expect(rangeRule(1, 10).validator(null, 5)).resolves.toBeUndefined();
    });
    it('resolves for empty value', async () => {
      await expect(rangeRule(1, 10).validator(null, '')).resolves.toBeUndefined();
    });
    it('rejects for out of range', async () => {
      await expect(rangeRule(1, 10, 'msg').validator(null, 20)).rejects.toThrow('msg');
    });
  });

  describe('noLeadingTrailingSpacesRule', () => {
    it('resolves for trimmed string', async () => {
      await expect(noLeadingTrailingSpacesRule().validator(null, 'hello')).resolves.toBeUndefined();
    });
    it('rejects for leading space', async () => {
      await expect(noLeadingTrailingSpacesRule('msg').validator(null, ' hello')).rejects.toThrow('msg');
    });
  });
  ...
  describe('confirmPasswordRule', () => {
    it('resolves when passwords match', async () => {
      ...
      await expect(confirmPasswordRule(getFieldValue).validator(null, 'pass')).resolves.toBeUndefined();
    });
    it('resolves for empty value', async () => {
      const getFieldValue = vi.fn(() => 'pass');
      await expect(confirmPasswordRule(getFieldValue).validator(null, '')).resolves.toBeUndefined();
    });
    it('rejects when passwords differ', async () => {
      const getFieldValue = vi.fn(() => 'pass');
      await expect(confirmPasswordRule(getFieldValue, 'password', 'msg').validator(null, 'diff')).rejects.toThrow('msg');
