...
function InsightRow({ icon, tone, title, value, onClick }: InsightRowProps) {
  return (
    <button
      type="button"
      className={`dash-insight dash-insight-${tone}`}
      onClick={onClick}
      disabled={!onClick}
    >
      <span className="dash-insight-icon">{icon}</span>
      <span className="dash-insight-title">{title}</span>
      <span className="dash-insight-value">
        {value}
        {onClick && <RightOutlined />}
      </span>
    </button>
  );
}

const DashboardPredictiveInsights = memo(function DashboardPredictiveInsights({
  scope,
}: {
  ...
  const lowCoverageDepts = (global.data?.taux_couverture_departements ?? []).filter(
    (d) => d.taux_couverture < 50,
  );

  return (
    <section className="dash-predictive">
      <div className="dash-predictive-head">
        <span className="dash-predictive-icon">
          <ThunderboltOutlined />
        </span>
        <div className="dash-predictive-head-text">
          <div className="dash-predictive-title-row">
            ...
            <Tag className="dash-preview-tag" color="gold">
              Aperçu
            </Tag>
          </div>
        ...
        <Tooltip title="Ouvrir l'analyse prédictive détaillée">
          <Button
            ...
            onClick={() => {
              ...
              navigate(
                firstTeacher ? `/home/analytics/teacher/${firstTeacher}` : '/home/analytics',
              );
            }}
          >
        ...
        <Col xs={24} lg={14}>
          <div className="dash-predictive-chart">
            ...
            {forecast.isLoading ? (
              ...
              <DemandForecastChart data={forecast.data} height={240} />
            )}
          </div>
        </Col>
        <Col xs={24} lg={10}>
          <div className="dash-insight-list">
            ...
            {topDeclining.map((c) => (
              <InsightRow
                key={`d-${c.competency_id}`}
                icon={<FallOutlined />}
                tone="warning"
                title={`Déclin — ${c.competency_name}`}
                value={`${c.demand_3m ?? 0} / 3m`}
              />
            ))}
            {reco.map((f) => (
              <InsightRow
                key={`r-${f.formation_id}`}
                icon={<BulbOutlined />}
                tone="success"
                title={`À lancer — ${f.formation_titre}`}
                value={`${f.nb_recommandations}×`}
            ...
            {scope.isAdmin &&
              lowCoverageDepts.map((d) => (
                <InsightRow
                  ...
                  tone="info"
                  title={`Intervenir — ${d.departement}`}
                  value={`${d.taux_couverture}%`}
                  onClick={() => {
                    const deptTeacher =
                      riskList.find((r) => r.departement === d.departement)?.teacher_id ||
                      topAtRisk[0]?.teacher_id;
                    navigate(
                      deptTeacher ? `/home/analytics/teacher/${deptTeacher}` : '/home/analytics',
                    );
                  }}
                />
              ))}
