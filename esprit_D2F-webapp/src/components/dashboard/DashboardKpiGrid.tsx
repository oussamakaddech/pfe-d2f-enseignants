...
import {
  BookOutlined, TeamOutlined, RiseOutlined, WarningOutlined,
  BellOutlined, SafetyCertificateOutlined, AimOutlined, HeartOutlined,
...
const DashboardKpiGrid = memo(function DashboardKpiGrid({ scope }: { readonly scope: DashboardScope }) {
  ...
  const specs: KpiSpec[] = isAdmin
    ? [
        ...
        { title: "Enseignants à risque", value: global.data?.enseignants_a_risque?.length ?? 0, icon: <WarningOutlined />, color: "danger", loading: global.isLoading, to: "/home/AnalysePredictive" },
    ...
    : [
        ...
        { title: "Participations", value: sum((u) => u.nombreParticipations), icon: <TeamOutlined />, color: "info", loading: upQ.isLoading },
        { title: "Taux participation", value: pct(avg((u) => u.tauxParticipation)), unit: "%", icon: <RiseOutlined />, color: presenceColor(avg((u) => u.tauxParticipation)), loading: upQ.isLoading },
        { title: "Enseignants suivis", value: sum((u) => u.nombreEnseignants), icon: <TeamOutlined />, color: "info", loading: upQ.isLoading },
        { title: "Sans formation", value: inactifs.data?.total ?? 0, icon: <WarningOutlined />, color: "warning", loading: inactifs.isLoading, to: "/home/analytics/enseignants-inactifs" },
        { title: "Engagement moyen", value: pct(avg((u) => u.scoreEngagement)), unit: "/100", icon: <HeartOutlined />, color: "success", loading: upQ.isLoading },
      ];
