...
const TRAINER_CONFIG = [
  { key: "externeOnlyCount", label: "Formateur externe", sub: "formations avec formateur externe", icon: <GlobalOutlined />, color: "#3b82f6", bg: "#eff6ff", border: "#bfdbfe" },
  { key: "interneOnlyCount", label: "Formateur interne", sub: "formations avec animateur interne", icon: <UserOutlined />,   color: "#059669", bg: "#ecfdf5", border: "#a7f3d0" },
  { key: "mixteCount",       label: "Mixte",             sub: "formations avec les deux types",   icon: <TeamOutlined />,   color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe" },
] as const;
...
const DashboardTrainerTypes = memo(function DashboardTrainerTypes({ scope }: Props) {
  ...
  const { data: raw, isLoading, isError } = useKpiCountByTrainerType(filters as Record<string, unknown>);
          ...
          {TRAINER_CONFIG.map(({ key, label, sub, icon, color, bg, border }) => (
            <div key={key} className="dash-list-row dash-list-row-static">
              <div style={{
                ...
                background: bg, border: `1px solid ${border}`, fontSize: 18, color,
              }}>
                {icon}
              </div>
              ...
              <span style={{
                fontWeight: 800, fontSize: 22, color: "var(--neutral-900)",
