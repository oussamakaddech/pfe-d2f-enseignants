...
const DashboardPendingNeeds = memo(function DashboardPendingNeeds({
  scope,
}: {
  ...
  return (
    <InfoCard
      ...
      icon={<BellOutlined />}
      footer={
        <Button type="link" style={{ paddingInline: 0 }} onClick={() => navigate('/home/besoins')}>
          Tous les besoins <RightOutlined />
        </Button>
      }
    >
      {(() => {
        if (src.isLoading) return <Skeleton active paragraph={{ rows: 4 }} />;
        if (pending.length === 0)
          return (
            <EmptyState
              icon={<CheckCircleOutlined style={{ color: 'var(--color-success)', fontSize: 32 }} />}
        ...
        return (
          <div className="dash-list">
            {pending.map((b) => {
              ...
              return (
                <button
                  key={String(b.idBesoinFormation ?? b.titre)}
                  type="button"
                ...
                >
                  <div className="dash-list-main">
                    <div className="dash-list-title">
                      {b.titre ?? b.theme ?? 'Besoin de formation'}
                    </div>
                    <div className="dash-list-meta">
                      {b.username ?? '—'}
                      {prio && (
                        <Tag
                          color={PRIORITE_COLOR[prio] ?? 'default'}
