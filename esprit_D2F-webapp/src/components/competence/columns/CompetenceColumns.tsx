...
function countSousComp(nodes: { enfants?: unknown[] }[]): number {
  return (nodes ?? []).reduce(
    (sum, node) => sum + 1 + countSousComp((node.enfants ?? []) as { enfants?: unknown[] }[]),
    0,
  );
}

export function buildDomaineColumns(onToggleActif: (id: Id) => void): TableColumnsType<Domaine> {
  return [
