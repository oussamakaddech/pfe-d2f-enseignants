...
import {
  Card, Table, Tag, Space, Button, Empty, Spin, Typography,
  Select, Statistic, Row, Col, Tooltip, InputNumber,
} from "antd";
import {
  TeamOutlined, BulbOutlined, DownloadOutlined,
  CheckCircleOutlined, UserAddOutlined, BarChartOutlined,
...
interface CohortRecommendationPanelProps {
  ...
  readonly onGenerate: (teacherIds: string[], topN: number) => Promise<BatchRecommendationResponse | undefined>;
...
export default function CohortRecommendationPanel({
  teachers, teachersLoading, onGenerate, onExport,
  ...
  const columns: ColumnsType<NonNullable<BatchRecommendationResponse>["recommendations"][number]> = [
    ...
    },
    {
      ...
      render: (v: number[]) => (
        <Space size={2} wrap>
          {v.slice(0, 3).map((c) => (
            <Tag key={c} style={{ fontSize: 11 }}>C{c}</Tag>
          ))}
          {v.length > 3 && (
            <Tooltip title={v.slice(3).map((c) => `C${c}`).join(", ")}>
              <Tag>+{v.length - 3}</Tag>
            </Tooltip>
          )}
        </Space>
                ...
                <Statistic
                  ...
                  value={
                    result.nb_enseignants > 0
                      ? Math.round(
                          (result.recommendations.filter((r) => r.nb_enseignants_concernes > 0).length /
