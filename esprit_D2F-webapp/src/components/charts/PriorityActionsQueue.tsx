...
import { ThunderboltOutlined, UserOutlined, FireOutlined } from '@ant-design/icons';
    ...
    () => [
      ...
      },
      {
        ...
        render: (v: string | null) => {
          ...
          return (
            <Tag color={t.color} icon={v === 'en_hausse' ? <FireOutlined /> : undefined}>
              {t.label}
            </Tag>
          );
        },
    ...
    <Card
      ...
      title={
        <Space>
          ...
          {(data || []).length > 0 && <Tag color="red">{data!.length} action(s)</Tag>}
        </Space>
      }
      extra={
        <Select
