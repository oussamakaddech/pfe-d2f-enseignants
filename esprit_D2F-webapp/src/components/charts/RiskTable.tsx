...
const RiskTable = memo(function RiskTable({
  ...
  const columns = [
      ...
      render: (_: unknown, r: TeacherRiskIndicator) => (
        <Space>
          <Avatar
            size="small"
            icon={<UserOutlined />}
            style={{ backgroundColor: riskColor(r.attrition_risk_score) }}
          />
          <div>
    ...
    },
    {
      ...
      width: 160,
      sorter: (a: TeacherRiskIndicator, b: TeacherRiskIndicator) =>
        a.attrition_risk_score - b.attrition_risk_score,
    ...
    },
    {
      ...
      align: 'center' as const,
      render: (v: number) => (
        <Badge count={v || 0} showZero style={{ backgroundColor: badgeColor(v) }} />
      ),
    },
    {
      title: 'Action',
      ...
      width: 160,
      render: (v: string) => <Tag color={tagColor(v)}>{v}</Tag>,
    },
    ...(onAnalyze
      ? [
          {
