...
export default function GroupedRecommendations({
  enseignantId,
}: {
    ...
    <Card
      ...
      extra={
        <Segmented
          ...
          value={groupBy}
          onChange={(v) => setGroupBy(v as RecoGroupBy)}
        />
      }
    >
      <Spin spinning={isLoading}>
        {(() => {
          if (isError)
            return <Alert type="error" showIcon message="Impossible de charger le regroupement." />;
          if (!data || data.total === 0)
            return (
              <Empty
                description={
                  enseignantId
                    ? 'Aucune recommandation à regrouper'
