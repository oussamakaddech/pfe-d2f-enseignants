...
const TILES: readonly TileDef[] = [
  {
    key: 'nb_enseignants_suivis',
    label: 'Enseignants suivis',
    icon: <TeamOutlined />,
    higherBetter: true,
    fmt: (v) => String(Math.round(v)),
    hint: "Nombre d'enseignants actuellement monitorés par le modèle prédictif.",
  },
  {
  ...
  },
  {
    ...
    higherBetter: false,
    fmt: (v) => String(Math.round(v)),
    hint: 'Écarts de compétence les plus graves nécessitant une action prioritaire.',
  },
  {
    key: 'nb_alertes_nouvelles',
    label: 'Alertes nouvelles',
    icon: <BellOutlined />,
    higherBetter: false,
    fmt: (v) => String(Math.round(v)),
    hint: 'Alertes générées par le modèle et non encore traitées.',
  },
  {
    key: 'taux_couverture_global',
    label: 'Couverture',
...
function DeltaChip({
  ...
  if (delta == null) {
    return (
      <span className="analyse-kpi-delta flat">
            ...
            <div key={t.key} className="analyse-kpi-tile">
              <div className="analyse-kpi-head">
                <Tooltip title={t.hint} placement="top">
                  <span className="analyse-kpi-info">
                    <InfoCircleOutlined /> {t.label}
                  </span>
                </Tooltip>
              </div>
