...
export default function GlassTrend({ data, height = 220 }: GlassTrendProps) {
  const view = useMemo(() => {
    ...
    const pts = (key: 'critical' | 'high') => data.map((d, i) => ({ x: X(i), y: Y(d[key]) }));
      ...
      <svg width={view.width} height={height} role="img" aria-label="Évolution du risque">
        {view.yTicks.map((t, i) => (
          <g key={t.v}>
            ...
            />
            <text x={view.padX - 8} y={t.y + 3} fontSize={10} fill={neutral[400]} textAnchor="end">
              {Math.round(t.v)}
            </text>
          </g>
        ))}
        <path
          d={view.path('critical')}
          fill="none"
        ...
        <path
          ...
          strokeLinecap="round"
        />
        {view.ptsCritical.map((p) => (
          <circle key={`c-${p.x}-${p.y}`} cx={p.x} cy={p.y} r={3} fill={COLORS.critical} />
        ))}
        {view.ptsHigh.map((p) => (
          <circle key={`h-${p.x}-${p.y}`} cx={p.x} cy={p.y} r={3} fill={COLORS.high} />
        ))}
        {view.labels.map(
          (l, i) =>
            l.show && (
