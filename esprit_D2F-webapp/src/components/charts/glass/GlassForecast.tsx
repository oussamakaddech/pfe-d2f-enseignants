...
export default function GlassForecast({ data, height = 240 }: GlassForecastProps) {
  const view = useMemo(() => {
    ...
    const padX = 38, padTop = 16, padBottom = 28;
  ...
  if (!view) return <Empty description={data?.note ?? "Historique insuffisant pour une prévision"} />;
  ...
  return (
    <div style={{ overflowX: "auto" }}>
      <svg width={view.width} height={height} role="img" aria-label="Prévision de la demande">
        ...
        <line x1={view.boundaryX} y1={view.padTop} x2={view.boundaryX} y2={view.padTop + view.innerH} stroke={neutral[300]} strokeDasharray="3 3" />
        <polyline points={view.histPts} fill="none" stroke={brand[500]} strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />
        <polyline points={view.fcPts} fill="none" stroke={accent[500]} strokeWidth={2.5} strokeDasharray="6 5" strokeLinejoin="round" strokeLinecap="round" />
        {view.labels.map((l, i) => l.show && (
          <text key={`${l.label}-${i}`} x={l.x} y={height - 8} fontSize={10} fill={neutral[500]} textAnchor="middle">{l.label}</text>
        ))}
      </svg>
      <div style={{ display: "flex", gap: 16, fontSize: 12, color: neutral[600], marginTop: 4 }}>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><i style={{ width: 14, height: 3, background: brand[500], display: "inline-block", borderRadius: 2 }} />Historique</span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><i style={{ width: 14, height: 3, background: accent[500], display: "inline-block", borderRadius: 2 }} />Projection</span>
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><i style={{ width: 14, height: 10, background: "rgba(0,180,216,0.3)", display: "inline-block", borderRadius: 2 }} />Intervalle de confiance</span>
      </div>
    </div>
  );
}
