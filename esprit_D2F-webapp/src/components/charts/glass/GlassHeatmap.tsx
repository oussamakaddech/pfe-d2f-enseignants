  ...
  return (
    <div style={{ overflowX: "auto" }}>
      ...
      >
        <div />
        {competences.map((c) => (
          <div key={c.id} style={{ fontSize: 11, fontWeight: 700, color: neutral[600], textAlign: "center", padding: "4px 2px" }}>
...
function Row({
  ...
  return (
    <>
      <div style={{ fontSize: 12.5, fontWeight: 600, color: neutral[700], display: "flex", alignItems: "center" }}>
        {r.departement}
      </div>
      {competences.map((c) => {
        const cell = r.map.get(c.id);
        if (!cell) {
          return <div key={c.id} style={{ height: 38, borderRadius: 10, background: "rgba(15,23,42,0.04)" }} />;
        }
        return (
          <div
            key={c.id}
