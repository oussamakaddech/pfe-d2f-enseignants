            ...
            <div key={d.competence_id}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 4 }}>
                <span style={{ fontSize: 12.5, fontWeight: 600, color: neutral[800] }}>
                  {d.competence_nom}
                  <span style={{ color: neutral[400], fontWeight: 400, marginLeft: 6, fontSize: 11 }}>{d.domaine_nom}</span>
                </span>
                <Tag style={{ color: q.color, background: `${q.color}1a`, borderColor: "transparent", fontWeight: 700, fontSize: 11 }}>
                  {q.label}
                </Tag>
              </div>
              <div style={{ position: "relative", height: 10, borderRadius: 999, background: "rgba(15,23,42,0.07)", overflow: "hidden" }}>
                <div style={{ width: `${Math.max(4, d.demand_score * 100)}%`, height: "100%", borderRadius: 999, background: `linear-gradient(90deg, ${q.color}cc, ${q.color})` }} />
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: neutral[500], marginTop: 3 }}>
                <span>Demande {Math.round(d.demand_score * 100)}%</span>
                <span>{d.nb_enseignants} ens. · {d.nb_critiques} critiques</span>
              </div>
            </div>
          );
        })}
