...
export default function GlassTrainingNeedForecast({ data, height = 300 }: GlassTrainingNeedForecastProps) {
  const view = useMemo(() => {
    ...
    const padX = 38, padTop = 16, padBottom = 30;
    const innerW = width - padX * 2;
    const innerH = height - padTop - padBottom;
    const maxTotal = Math.max(
      1,
      ...data.total_forecast.map((p) => p.upper ?? p.value)
    );
    ...
    return {
      width, months, series, valueByDept, n, padX, padTop, innerH,
      maxTotal, stepX, barW, Y, CX,
    };
  }, [data, height]);
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((f) => {
          ...
          return (
            <g key={f}>
              ...
              <text x={6} y={y + 3} fontSize={9} fill={neutral[400]}>{Math.round(maxTotal * f)}</text>
            </g>
          );
        })}
        {months.map((m, i) => {
          ...
          return (
            <g key={m}>
              {series.map((dept, di) => {
                ...
                return (
                  <rect key={dept} x={x} y={yTop} width={barW} height={Math.max(0, h)}
                    fill={color} rx={2} opacity={0.9} />
                );
              })}
              <text x={CX(i)} y={height - 10} fontSize={10} fill={neutral[500]} textAnchor="middle">{m}</text>
            </g>
          );
        })}
      </svg>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 12, fontSize: 12, color: neutral[600], marginTop: 4 }}>
        {view.series.map((dept, di) => (
          <span key={dept} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <i style={{ width: 12, height: 10, background: dept === "__autres__" ? neutral[400] : PALETTE[di % PALETTE.length], display: "inline-block", borderRadius: 2 }} />
