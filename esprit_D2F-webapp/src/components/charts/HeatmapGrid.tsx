  ...
  return (
    <div className={styles.scroll}>
      <div
        ...
        style={{ gridTemplateColumns: `minmax(120px, auto) repeat(${xLabels.length}, minmax(36px, 1fr))` }}
      >
        {/* coin vide + en-têtes de colonnes */}
        <div className={styles.cornerCell} />
        {xLabels.map((x) => (
          <div key={x} className={styles.colHeader}>{x}</div>
        ))}
      ...
      <div className={styles.scale} aria-hidden="true">
        ...
        <span className={styles.scaleLabel}>{max.toLocaleString("fr-FR")}{valueSuffix}</span>
      </div>
    </div>
  );
});
  ...
  return (
    <>
      ...
      {xLabels.map((x, colIdx) => {
        ...
        return (
          <Tooltip key={x} title={`${label} — ${x} : ${value.toLocaleString("fr-FR")}${valueSuffix}`}>
            <button
              type="button"
