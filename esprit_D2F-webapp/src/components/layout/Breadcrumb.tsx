...
const Breadcrumb = memo(function Breadcrumb() {
  ...
  const items = useMemo(() => {
    ...
    return segments.map((seg, i) => {
      ...
      return {
        title: isLast ? (
          <span style={{ color: 'var(--breadcrumb-active)', fontWeight: 600, fontSize: 13 }}>
            {label}
          </span>
        ) : (
          <Link to={path} style={{ color: 'var(--breadcrumb-link)', fontSize: 13 }}>
            {label}
          </Link>
        ),
      };
  ...
  return (
    <nav aria-label="Fil d'Ariane" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <AntBreadcrumb
        items={items}
        separator={<span style={{ color: 'var(--neutral-300)' }}>/</span>}
      />
    </nav>
  );
});
