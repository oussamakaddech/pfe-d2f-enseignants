...
import { useD2FTeacherProfile, useD2FTeacherMLSignal } from '@/hooks/analyse/useD2FData';
...
function ScoreCell(props: Readonly<ScoreCellProps>) {
  ...
  return h(
    ...
    h(
      Card,
      { size: 'small', style: { borderTop: '3px solid ' + color } },
      h(
        Space,
        { direction: 'vertical', style: { width: '100%' }, size: 6 },
        h(
          'div',
...
function TopFactorsCard(props: Readonly<TopFactorsCardProps>) {
  return h(
    Card,
    { size: 'small', title: 'Top facteurs ML (' + props.modelName + ')' },
    h(
      Space,
      { direction: 'vertical', style: { width: '100%' }, size: 4 },
