  ...
  return (
    <div>
      ...
      <Card
        ...
        title="Plan de formation projeté"
      >
        {rows.length === 0 ? (
              ...
              <Space>
                <Checkbox
                  ...
                  onChange={(e) => {
                    ...
                    setSelected(
                      Object.fromEntries(rows.map((r) => [r.gap.competence_id, checked])),
                    );
                  }}
                >
                  Tout sélectionner
      ...
      {res && (
        <Card size="small" style={{ borderRadius: 12 }} title="Résultat de la simulation">
          <Space size="large" wrap style={{ marginBottom: 16 }}>
            <Statistic
              ...
              valueStyle={{
                color:
                  RISK_LEVEL_COLORS[
                    (res.risk_before.niveau as keyof typeof RISK_LEVEL_COLORS) ?? 'ELEVE'
                  ],
              }}
            />
            ...
            <Statistic
              ...
              valueStyle={{
                color:
                  RISK_LEVEL_COLORS[
                    (res.risk_after.niveau as keyof typeof RISK_LEVEL_COLORS) ?? 'FAIBLE'
                  ],
              }}
            />
            <Statistic
              title="Baisse de risque"
              value={fmtPct(res.risk_reduction)}
              valueStyle={{ color: '#52c41a' }}
            />
            <Statistic
              title="Écarts résolus"
              value={`${res.nb_gaps_resolus} / ${res.nb_gaps_before}`}
            />
          </Space>
            ...
            columns={[
              {
                ...
                render: (id: number) =>
                  gaps.find((g) => g.competence_id === id)?.competence_nom ?? `#${id}`,
              },
              {
                title: 'Niveau',
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: '#94a3b8' }}>
            ...
            {
              RISK_LEVEL_LABELS[
                (res.risk_after.niveau as keyof typeof RISK_LEVEL_LABELS) ?? 'FAIBLE'
              ]
            }
          </div>
        </Card>
