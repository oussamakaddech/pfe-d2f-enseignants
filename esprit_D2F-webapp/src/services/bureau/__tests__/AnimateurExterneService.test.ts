...
describe('AnimateurExterneService', () => {
  ...
  it('lists animateurs of a bureau and tolerates a non-array payload', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(expect.stringContaining('/bureaux/5/animateurs'));
  ...
  it('creates, updates and deletes an animateur', async () => {
    ...
    await expect(AnimateurExterneService.create(5, { nom: 'X' } as never)).resolves.toEqual({ id: 2, nom: 'X' });

    httpMocks.mockPut.mockResolvedValueOnce({ data: { id: 2, nom: 'Y' } });
    await expect(AnimateurExterneService.update(5, 2, { nom: 'Y' } as never)).resolves.toEqual({ id: 2, nom: 'Y' });
    expect(httpMocks.mockPut).toHaveBeenCalledWith(expect.stringContaining('/bureaux/5/animateurs/2'), { nom: 'Y' });

    httpMocks.mockDelete.mockResolvedValueOnce({});
    await AnimateurExterneService.delete(5, 2);
    expect(httpMocks.mockDelete).toHaveBeenCalledWith(expect.stringContaining('/bureaux/5/animateurs/2'));
  });
});
