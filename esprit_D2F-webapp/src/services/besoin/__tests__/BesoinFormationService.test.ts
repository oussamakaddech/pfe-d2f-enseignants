...
describe('BesoinFormationService', () => {
  ...
  it('adds, modifies and removes besoin formation', async () => {
    ...
    await expect(
      BesoinFormationService.addBesoinFormation({ intitule: 'IA' } as never),
    ).resolves.toEqual({ id: 3 });

    httpMocks.mockPut.mockResolvedValueOnce({ data: { id: 3, commentaire: 'ok' } });
    await expect(
      BesoinFormationService.modifyBesoinFormation({ id: 3 } as never, 'ok'),
    ).resolves.toEqual({ id: 3, commentaire: 'ok' });

    httpMocks.mockDelete.mockResolvedValueOnce({ data: { deleted: true } });
    await expect(BesoinFormationService.removeBesoinFormation(3)).resolves.toEqual({
      deleted: true,
    });
  });
  ...
  it('approves and retrieves notifications', async () => {
    ...
    await expect(BesoinFormationService.getApprovedBesoinFormations()).resolves.toEqual([
      { id: 4 },
    ]);

    httpMocks.mockPut.mockResolvedValueOnce({ data: { id: 4, statut: 'APPROUVE' } });
    await expect(BesoinFormationService.approveBesoin(4)).resolves.toEqual({
      id: 4,
      statut: 'APPROUVE',
    });
  ...
  it('supports consultation and prioritization endpoints', async () => {
    ...
    await expect(BesoinFormationService.getBesoinsByDepartement('DEP-1')).resolves.toEqual([
      { id: 6 },
    ]);
    ...
    await expect(BesoinFormationService.getBesoinsByPrioriteLevel('HAUTE')).resolves.toEqual([
      { id: 8 },
    ]);
  });
});
