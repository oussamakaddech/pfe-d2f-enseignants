...
export async function login({ username, password }: LoginRequest): Promise<LoginResponse> {
  ...
  const response: AxiosResponse<LoginResponse> = await api.post(`${AUTH_BASE}/login`, params);
...
export async function forgotPassword(emailAddress: string): Promise<AccountActionResponse> {
  const url = `${AUTH_BASE}/forgot-password?emailAddress=${encodeURIComponent(emailAddress)}`;
...
export async function refreshToken(): Promise<LoginResponse> {
  const response: AxiosResponse<LoginResponse> = await api.get(`${AUTH_BASE}/refresh`, {
    meta: { silent: true },
  } as never);
  return response.data;
}
