...
describe('AnalyticsService', () => {
  ...
  it('triggerBatchAnalysis posts to trigger endpoint', async () => {
    ...
    expect(httpMocks.mockPost).toHaveBeenCalledWith(
      expect.stringContaining('/trigger-batch-analysis'),
    );
  });
  ...
  it('getGaps with default params', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(expect.stringContaining('/gaps/E01'), {
  ...
  it('getGaps with custom params', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(expect.stringContaining('/gaps/E02'), {
  ...
  it('getCompetencesDeclining', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(
      expect.stringContaining('/competences-declining'),
    );
  });
  ...
  it('getTeachersAtRisk with default seuil', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(expect.stringContaining('/teachers-at-risk'), {
      params: { seuil: 0.5 },
    });
  });
  ...
  it('getEnseignantsSansFormation with default and custom params', async () => {
    ...
    const r2 = await AnalyticsService.getEnseignantsSansFormation({
      mois: 6,
      departement: 'GC',
      up: 'UP1',
      page: 1,
  ...
  it('getFormationsParPeriode with default and custom params', async () => {
    ...
    const r2 = await AnalyticsService.getFormationsParPeriode({
  ...
  it('getFormationsParUp with default and custom opts', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(expect.stringContaining('/formations-par-up'), {
  ...
  it('exportExcel and exportPdf return blob data', async () => {
    ...
    expect(httpMocks.mockGet).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        responseType: 'blob',
        params: { type: 'rapport-annuel', annee: 2026 },
      }),
    );
  });
});
