...
describe('notificationService', () => {
  ...
  const raw = (over: Record<string, unknown> = {}) => ({
    id: 1, type: 'FORMATION', severity: 'INFO', title: 'T', message: 'M', read: false, createdAt: '2026-01-01T00:00:00Z', link: '/x', actor: 'A', meta: { k: 1 }, ...over,
  });

  it('lists and maps notifications', async () => {
    httpMocks.mockGet.mockResolvedValueOnce({ data: { content: [raw()], totalElements: 1, totalPages: 1, number: 0 } });
    const out = await notificationService.list({ unreadOnly: true, page: 2, size: 5 });
    expect(out).toEqual([{ id: '1', type: 'FORMATION', severity: 'info', title: 'T', message: 'M', read: false, createdAt: '2026-01-01T00:00:00Z', link: '/x', actor: 'A', meta: { k: 1 } }]);
    expect(httpMocks.mockGet).toHaveBeenCalledWith(base, { params: { unreadOnly: true, page: 2, size: 5 } });
  });
  ...
  it('list returns empty when content missing and applies defaults', async () => {
    ...
    expect(out).toEqual([]);
    expect(httpMocks.mockGet).toHaveBeenCalledWith(base, { params: { unreadOnly: false, page: 0, size: 100 } });
  });

  it('normalizes missing fields in toApp', async () => {
    httpMocks.mockGet.mockResolvedValueOnce({ data: { content: [{ id: 2, type: 'SYSTEM', title: 'X' }] } });
  ...
  it('creates a notification and uppercases severity', async () => {
    ...
    const out = await notificationService.create({
      recipient: 'u', type: 'FORMATION', severity: 'success', title: 'T', message: 'M', link: '/l', actor: 'A', meta: { a: 1 },
    });
