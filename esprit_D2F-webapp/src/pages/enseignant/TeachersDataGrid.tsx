...
import { Table, Button, Upload, Space, Tooltip, Card, Input, Select } from 'antd';
...
export default function TeachersDataGrid({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const displayedData = useMemo(() => {
    ...
    switch (sortBy) {
      case 'nom_asc':
        sorted.sort((a, b) => cmp(a, b, 'nom'));
        break;
      case 'nom_desc':
        sorted.sort((a, b) => -cmp(a, b, 'nom'));
        break;
      case 'type':
        sorted.sort((a, b) => cmp(a, b, 'type'));
        break;
      case 'up':
        sorted.sort((a, b) => cmp(a, b, 'upLibelle'));
        break;
      case 'dept':
        sorted.sort((a, b) => cmp(a, b, 'deptLibelle'));
        break;
      case 'email':
        sorted.sort((a, b) => cmp(a, b, 'mail'));
        break;
    }
    return sorted;
  ...
  const rowSelection = {
    ...
    selectedRowKeys: selectedTeacher
      ? [(selectedTeacher as Record<string, unknown>).id as string]
      : [],
  ...
  const exportExcel = () => {
    const rows = displayedData.map((e: Record<string, unknown>) => {
      ...
      return {
        ...
        'Chef Dépt':
          e.chefDepartement === 'O' || e.chefDepartement === 'Y' || e.chefDepartement === '1'
            ? 'Oui'
            : 'Non',
      };
    });
      ...
      [
        {
          name: 'Enseignants',
          rows,
          title: 'Liste des Enseignants — Esprit',
      ...
      <div className="teachers-page">
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <span className="teachers-hero-badge">
                    {data.length}
                    <span className="teachers-hero-badge-total">
                      enseignant{data.length === 1 ? '' : 's'}
                    </span>
                  </span>
                </div>
                <div className="teachers-hero-subtitle">
                  Gérer, importer et créer les comptes enseignants
                </div>
              </div>
        ...
        <div className="teachers-stats">
          <div className="teachers-stat-card teachers-stat-card--total">
            ...
            <div className="teachers-stat-value" style={{ color: '#b51200' }}>
              {data.length}
            </div>
          </div>
          <div className="teachers-stat-card teachers-stat-card--perm">
            ...
            <div className="teachers-stat-value" style={{ color: '#2563eb' }}>
              {permCount}
            </div>
          </div>
          <div className="teachers-stat-card teachers-stat-card--vac">
            ...
            <div className="teachers-stat-value" style={{ color: '#d97706' }}>
              {vacCount}
            </div>
          </div>
          <div className="teachers-stat-card teachers-stat-card--cup">
            ...
            <div className="teachers-stat-value" style={{ color: '#7c3aed' }}>
              {cupCount}
            </div>
          </div>
        </div>
            ...
            <div className="teachers-extracted-grid">
              {extracted.map((ex: Record<string, unknown>, idx: number) => (
                ...
                >
                  ...
                  {!!ex?.fichier && (
                    <div className="teachers-extracted-file">{String(ex.fichier)}</div>
                  )}
                  <Space style={{ marginTop: 8 }}>
                    ...
                    <Button size="small" onClick={() => handleIgnoreExtract(idx)}>
                      Ignorer
                    </Button>
                  </Space>
                </div>
        ...
        <div className="teachers-toolbar">
          ...
          <Tooltip
            title={
              file ? 'Importer le fichier sélectionné' : "Sélectionnez d'abord un fichier Excel"
            }
          >
            <Button
              type="primary"
        ...
        <Card className="teachers-table-wrapper">
          <Table
            ...
            pagination={{
              pageSize: 10,
              showTotal: (total) => `${total} enseignant${total === 1 ? '' : 's'}`,
            }}
            onRow={(record: Record<string, unknown>) => ({
              onClick: () => {
                navigate(`/home/calendar/${record.id}`);
              },
            })}
        ...
        <CreateAccountDrawer
          ...
          initialValues={{
            ...
            username:
              String((selectedTeacher as Record<string, unknown>)?.mail ?? '').split('@')[0] || '',
            role: 'ANIMATEUR',
          }}
        />
