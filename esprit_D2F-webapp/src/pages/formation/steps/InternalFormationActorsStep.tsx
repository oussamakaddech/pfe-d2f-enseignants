...
import { Row, Col, Card, Space, Radio, Select, Typography, Alert, Empty, Divider, Tag } from 'antd';
import { TeamOutlined } from '@ant-design/icons';
import type { PersonItem, LookupNode } from '../hooks/useFormationWorkflow';
import {
  AdditionMode,
  type AnimateurAdditionConfig,
  type ParticipantAdditionConfig,
} from '../formationWorkflowTypes';
...
export default function InternalFormationActorsStep(
  ...
  const {
    ...
    handleExcelImportParticipantFile,
            ...
            <Row gutter={[16, 12]} style={{ marginTop: 16 }}>
              <Col span={24}>
                <Select
                  ...
                  onChange={(ids) => {
                    const selected = optionsAnim.filter((o) => ids.includes(String(o.id ?? '')));
                    setAnimSel(selected);
                  }}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {animSel.map((a) => (
                    <Tag
                      key={String(a.id ?? '')}
                      closable
                      onClose={() => setAnimSel(animSel.filter((x) => x.id !== a.id))}
                    >
                      {a.nom} {a.prenom}
                    </Tag>
                  ))}
            ...
            <Row gutter={[16, 12]} style={{ marginTop: 16 }}>
              <Col span={24}>
                <Select
                  ...
                  onChange={(ids) => {
                    const selected = optionsPart.filter((o) => ids.includes(String(o.id ?? '')));
                    setPartSel(selected);
                  }}
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {partSel.map((p) => (
                    <Tag
                      key={String(p.id ?? '')}
                      closable
                      onClose={() => setPartSel(partSel.filter((x) => x.id !== p.id))}
                    >
                      {p.nom} {p.prenom}
                    </Tag>
                  ))}
