  ...
  return (
    <div className="creation-field">
      ...
      <span className="creation-field-help">
        {calc ? (
          <>
            Calculé depuis les séances&nbsp;: <strong>{calc}</strong> — auto-synchronisé à l'étape
            suivante.
          </>
        ) : (
...
export type GeneralStepProps = {
  ...
  setTypeFormation: (v: string) => void;
  etatFormation: string;
  setEtatFormation: (v: string) => void;
  dateDebut: string;
  setDateDebut: (v: string) => void;
  dateFin: string;
  setDateFin: (v: string) => void;
  salle: string;
  setSalle: (v: string) => void;
  periodCode: string;
  setPeriodCode: (v: string) => void;
  customPeriodLabel: string;
  setCustomPeriodLabel: (v: string) => void;
  chargeH: number;
  setChargeH: (v: number) => void;
  seances: { heureDebut?: unknown; heureFin?: unknown }[];
  toMinutes: (t: unknown) => number | null;
  ups: LookupNode[];
  depts: LookupNode[];
  selectedUp: LookupNode | null;
  setSelectedUp: (v: LookupNode | null) => void;
  selectedDept: LookupNode | null;
  setSelectedDept: (v: LookupNode | null) => void;
  ouverte: boolean;
  setOuverte: (v: boolean) => void;
};
        ...
        <AntAlert
          ...
          description={
            <div style={{ marginTop: 4, fontSize: '0.875rem' }}>
              {besoinInfo.propositionAnimateur && (
                <div>
                  • <strong>Animateur proposé :</strong> {besoinInfo.propositionAnimateur}
                </div>
              )}
              {(besoinInfo.dateDebut || besoinInfo.dateFin) && (
                <div>
                  • <strong>Période :</strong> {besoinInfo.dateDebut || '?'} →{' '}
                  {besoinInfo.dateFin || '?'}
                </div>
              )}
              {besoinInfo.priorite && (
                <div>
                  • <strong>Priorité :</strong> {besoinInfo.priorite}
                </div>
              )}
            </div>
          }
      ...
      <div className="creation-section-box">
        ...
        <div className="creation-field" style={{ marginBottom: 20 }}>
          <label className="creation-field-label" htmlFor="wf-titre">
            ...
            <span className="creation-field-required" aria-hidden="true">
              *
            </span>
          </label>
          <Input
          ...
          />
          <span className="creation-field-help">
            Minimum 5 caractères — sera affiché dans le catalogue et le calendrier
          </span>
        </div>
        <div className="creation-field" style={{ marginBottom: 20 }}>
          <label className="creation-field-label">
          ...
          <div className="creation-type-grid" role="radiogroup" aria-label="Type de formation">
            {[
              {
                value: 'INTERNE',
                label: 'Interne (Esprit)',
                desc: 'Animée par un enseignant Esprit',
                icon: <BankOutlined />,
              },
              {
                value: 'EXTERNE',
                label: 'Externe',
                desc: 'Dispensée par un prestataire externe',
                icon: <GlobalOutlined />,
              },
              {
                value: 'EN_LIGNE',
                label: 'En ligne (Teams)',
                desc: 'À distance via Microsoft Teams',
                icon: <LaptopOutlined />,
              },
            ].map((opt) => (
              <label
              ...
              >
                ...
                />
                <div className="creation-type-card-check" aria-hidden="true">
                  <CheckOutlined />
                </div>
                <div className="creation-type-card-icon" aria-hidden="true">
                  {opt.icon}
                </div>
        ...
        <div className="creation-field">
          ...
          <span className="creation-field-help">
            Peut être mis à jour à tout moment depuis la liste des formations
          </span>
        </div>
      </div>
      <div className="creation-section-box">
        ...
        <Row gutter={[20, 16]}>
          ...
          <Col xs={24} sm={12}>
            <div className="creation-field">
              ...
              />
              <span className="creation-field-help">
                Utilisé comme titre de l'événement Outlook : D2f‑{salle || 'Salle'}‑
                {titre || 'Formation'}‑Animateur
              </span>
            </div>
          </Col>
          <Col xs={24} sm={12}>
          ...
          <Col xs={24} sm={12}>
            {periodCode === 'OTHER' ? (
              <div className="creation-field">
                <label className="creation-field-label" htmlFor="custom-period-label">
                  Précisez la période
                </label>
                <Input
          ...
          <Col span={24}>
            <div className="creation-switch-row">
              ...
              <Text type="secondary" style={{ fontSize: 'var(--text-xs)' }}>
                {ouverte
                  ? 'Accessible à toutes les UPs'
                  : "Réservée aux participants de l'UP uniquement"}
              </Text>
            </div>
          </Col>
        </Row>
