...
export function rangeToDates(
  ...
  switch (key) {
    ...
    case "6m":
      return { start: today.subtract(6, "month").startOf("month").format(FMT), end: today.endOf("month").format(FMT) };
    case "12m":
      return { start: today.subtract(12, "month").startOf("month").format(FMT), end: today.endOf("month").format(FMT) };
    case "annee":
