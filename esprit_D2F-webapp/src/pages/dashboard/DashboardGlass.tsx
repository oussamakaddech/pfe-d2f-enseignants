...
function HealthRing({ score, color }: { readonly score: number; readonly color: string }) {
  ...
  return (
    <div className="glass-health-ring">
      ...
      <div className="score">
        <div className="glass-health-score" style={{ color }}>
          {score}
        </div>
...
function DashboardHero({
  ...
  return (
    <section className="glass-hero">
      <div className="glass-hero-avatar">
        <ThunderboltOutlined />
      </div>
      <div className="glass-hero-body">
        ...
        <p className="glass-hero-sub">
          {todayLabel.charAt(0).toUpperCase() + todayLabel.slice(1)} · Vue d'ensemble de la
    ...
    <section className="glass-section" style={{ marginTop: 22 }}>
      <div className="glass-kpi-grid">
        <GlassKpi
          ...
          tint={semantic.errorBg}
          value={d ? (d.enseignants_a_risque?.length ?? 0) : '—'}
          hint="Enseignants détectés en risque (score de risque prédictif élevé ou critique). Source : profils de risque MSAS. Fréquence : quotidienne."
        />
        <GlassKpi
          label="Couverture compétences"
          icon={<SafetyCertificateOutlined />}
          accent={semantic.success}
          tint={semantic.successBg}
        ...
        <GlassKpi
          ...
          tint={semantic.warningBg}
          value={d ? declining.length : '—'}
          hint="Compétences dont le niveau régresse"
        />
        <GlassKpi
          label="Alertes récentes"
          icon={<BellOutlined />}
          accent={semantic.info}
          tint={semantic.infoBg}
          value={d ? alerts.length : '—'}
          hint="Alertes générées récemment"
        />
        <GlassKpi
          label="Formations suggérées"
          icon={<BulbOutlined />}
          accent="#8b5cf6"
          tint="rgba(139,92,246,0.12)"
          value={d ? topFormations.length : '—'}
          hint="Top formations recommandées"
        />
        <GlassKpi
          ...
          hint="Indicateur de pilotage composite ; il ne constitue pas une décision automatique."
        />
      </div>
    </section>
  );
  ...
  return (
    <section className="glass-section">
      <div className="glass-section-head">
        <span className="bar" />
        <span className="txt">Santé & couverture</span>
        <span className="sub">Pulsation de la plateforme</span>
        <span className="line" />
      </div>
      <Row gutter={[18, 18]}>
        <Col xs={24} lg={8}>
          <GlassCard
            ...
            subtitle="Indice composite pondéré"
            icon={<HeartOutlined />}
            iconColor={healthColorFor(health.level)}
            iconBg={`${healthColorFor(health.level)}1f`}
          >
        ...
        <Col xs={24} lg={16}>
          <GlassCard
            ...
            subtitle="Taux de compétences au niveau requis"
            icon={<SafetyCertificateOutlined />}
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          ...
                          <span className="glass-muted" style={{ fontSize: 12.5, fontWeight: 600 }}>
                            {nonCalc ? NA_CALC : `${dep.taux_couverture}%`}
                          </span>
                        </div>
                        <div className="glass-bar">
                          <span style={{ width: `${nonCalc ? 0 : dep.taux_couverture}%` }} />
                        </div>
                      </div>
                      <span
                        className="glass-muted"
                        style={{ fontSize: 12, minWidth: 64, textAlign: 'right' }}
                      >
                        {nonCalc ? 'À évaluer' : `${dep.nb_evalues} évalués`}
                      </span>
                    </div>
                  );
                })}
...
function RiskEfficiencySection({
  ...
  return (
    <section className="glass-section">
      <div className="glass-section-head">
        <span className="bar" />
        <span className="txt">Risque & efficacité</span>
        <span className="sub">Enseignants à risque et impact des formations</span>
        <span className="line" />
      </div>
      <Row gutter={[18, 18]}>
        <Col xs={24} lg={10}>
                ...
                {riskTeachers.map((t) => {
                  ...
                  return (
                    <div key={t.enseignant_id} className="glass-list-item">
                      <div
                        className="glass-avatar-sm"
                        style={{ background: riskStyle(t.score_risque ?? 0).bg }}
                      >
                      ...
                      {t.score_risque != null ? (
                        <RiskBadge score={t.score_risque} size="sm" />
                      ) : (
                        <span className="glass-muted">—</span>
                      )}
                    </div>
                  );
                })}
        ...
        <Col xs={24} lg={14}>
          <GlassCard
            title="Efficacité des formations"
            subtitle="Gain de niveau & complétion"
            icon={<BulbOutlined />}
            iconColor="#8b5cf6"
                ...
                {eff.map((f) => (
                  <div key={f.formation_id} className="glass-list-item">
                    <div className="li-main">
                      ...
                      <div className="li-sub">
                        Complétion {Math.round((f.completion_rate ?? 0) * 100)}%
                      </div>
                    </div>
                    <span className="glass-chip" style={{ color: semantic.success }}>
                      +{Number(f.avg_level_gain ?? 0).toFixed(1)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <div className="glass-list">
                {declining.map((c) => (
                  <div key={c.competence_id} className="glass-list-item">
                    ...
                    <span className="glass-chip" style={{ color: semantic.error }}>
                      <ArrowDownOutlined /> {c.delta?.toFixed(1) ?? '—'}
                    </span>
                  </div>
                ))}
              </div>
                ...
                {inDemand.map((c) => (
                  <div key={c.competence_id} className="glass-list-item">
                    <div className="li-main">
                      <div className="li-title">{c.competence_nom}</div>
                      <div className="li-sub">
                        {c.domaine_nom} · {c.nb_gaps} gaps
                      </div>
                    </div>
                    <span className="glass-chip" style={{ color: semantic.info }}>
                      <ArrowUpOutlined /> {c.score_demande?.toFixed(0) ?? '—'}
                    </span>
                  </div>
                ))}
              </div>
                ...
                {alerts.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="glass-list-item">
                      <span className="glass-avatar-sm" style={{ background: col, fontSize: 14 }}>
                        <WarningOutlined />
                      </span>
                      <div className="li-main">
                ...
                {topFormations.map((f, i) => (
                  <div key={f.formation_id} className="glass-list-item">
                    ...
                    <div className="li-main">
                      <div className="li-title">{f.formation_titre}</div>
                      <div className="li-sub">
                        {f.nb_recommandations} recommandations · score{' '}
                        {f.score_moyen?.toFixed(1) ?? '—'}
                      </div>
                    </div>
                    <span className="glass-chip" style={{ color: semantic.success }}>
                      {Math.round((f.proba_reussite_moy ?? 0) * 100)}%
                    </span>
                  </div>
                ))}
              </div>
...
export default function DashboardGlass() {
  ...
  return (
    <div className="glass-app">
      ...
      <section className="glass-section" style={{ marginTop: 22 }}>
        {error && (
          <Alert message={error} type="error" showIcon style={{ marginBottom: 12 }} banner />
        )}
        <div className="glass-kpi-grid">
          <GlassKpi
            label="Enseignants à risque"
