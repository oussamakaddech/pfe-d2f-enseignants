...
export default function BesoinFiltersPanel({
  ...
  const setFilter = <K extends keyof BesoinFilters>(key: K, value: BesoinFilters[K]) =>
  ...
  const activeChips = useMemo(() => {
    ...
    if (filters.statut) {
      ...
      chips.push({
        key: 'statut',
        label: `Statut : ${lbl}`,
        clear: () => setFilter('statut', null),
      });
    }
    if (filters.priorite)
      chips.push({
        key: 'priorite',
        label: `Priorité : ${filters.priorite}`,
        clear: () => setFilter('priorite', null),
      });
    if (filters.type) {
      chips.push({
        key: 'type',
        label: `Type : ${(TYPE_LABELS as Record<string, string>)[filters.type] || filters.type}`,
        clear: () => setFilter('type', null),
      });
    }
    if (filters.upId) {
      const lbl =
        ups.find((u) => String(u.id) === String(filters.upId))?.name ||
        ups.find((u) => String(u.id) === String(filters.upId))?.libelle;
      chips.push({ key: 'up', label: `UP : ${lbl}`, clear: () => setFilter('upId', null) });
    }
    if (filters.deptId) {
      const lbl =
        departements.find((d) => String(d.id) === String(filters.deptId))?.name ||
        departements.find((d) => String(d.id) === String(filters.deptId))?.libelle;
      chips.push({
        key: 'dept',
        label: `Département : ${lbl}`,
        clear: () => setFilter('deptId', null),
      });
    }
    return chips;
  ...
  const isQuickActive = <K extends keyof BesoinFilters>(key: K, value: BesoinFilters[K]) =>
    filters[key] === value;
  const toggleQuick = <K extends keyof BesoinFilters>(key: K, value: BesoinFilters[K]) =>
    setFilter(key, isQuickActive(key, value) ? null : value);
  const isThisWeekActive = useMemo(() => {
    ...
    return (
      filters.dateRange[0].isSame(weekStart, 'day') && filters.dateRange[1].isSame(weekEnd, 'day')
    );
  }, [filters.dateRange]);
    ...
    <section className="bf-filters" aria-label="Filtres">
      ...
      <div className="bf-filters__primary">
        ...
        <RangePicker
          ...
          onChange={(dates) =>
            setFilter('dateRange', dates as [import('dayjs').Dayjs, import('dayjs').Dayjs] | null)
          }
          size="middle"
        ...
        >
          {types.map((t) => (
            <Option key={t} value={t}>
              {' '}
              {(TYPE_LABELS as Record<string, string>)[t] || t?.replaceAll('_', ' ')}
            </Option>
          ))}
        </Select>
        <Select
      ...
      {activeCount > 0 && (
        <div className="bf-filters__footer">
          <span className="bf-filters__footer-label">
            <FilterOutlined /> {activeCount} filtre{activeCount > 1 ? 's' : ''} actif
            {activeCount > 1 ? 's' : ''}
          </span>
