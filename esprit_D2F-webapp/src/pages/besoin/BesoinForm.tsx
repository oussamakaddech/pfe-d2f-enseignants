...
const STEPS_META = [
  ...
  { key: 'formation', title: 'Formation', subtitle: 'Titre & objectifs', icon: <AimOutlined /> },
  {
    key: 'details',
    title: 'Détails',
    subtitle: 'Formateur, horaire & période',
    icon: <ClockCircleOutlined />,
  },
  {
    key: 'competences',
    title: 'Compétences',
    subtitle: 'Référentiel RICE',
    icon: <NodeIndexOutlined />,
  },
  {
    key: 'parametres',
    title: 'Paramètres',
    subtitle: 'Type & évaluation',
...
export default function BesoinForm() {
  ...
  const progressPercent = isSummary
    ? 100
    : Math.round(((ctx.currentStep + 1) / (STEPS_META.length + 1)) * 100);
  ...
  const stepContent = [
    ...
    <BesoinDetailsStep
      key="details"
      acteurOptions={ctx.acteurOptions}
      acteursLoading={ctx.enseignantsLoading}
    />,
        ...
        {STEPS_META.map((s, idx) => {
          ...
          return (
            <button
              ...
              onClick={() => idx <= ctx.currentStep && ctx.setCurrentStep(idx)}
              disabled={idx > ctx.currentStep}
              aria-current={idx === ctx.currentStep ? 'step' : undefined}
            >
            ...
            {isSummary ? (
              ...
              >
                <BesoinReviewStep
                  ...
                  onEditSection={(idx) => {
                    ctx.setDirection(-1);
                    ctx.setCurrentStep(idx);
                  }}
                />
              </motion.div>
            ) : (
              <motion.div
                key={`step-${ctx.currentStep}`}
                custom={ctx.direction}
                variants={slideVariants}
                initial="enter"
                animate="center"
