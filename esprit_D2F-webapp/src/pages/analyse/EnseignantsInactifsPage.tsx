...
import { Card, Table, Tag, Input, InputNumber, Button, Space, Typography, Tooltip } from 'antd';
  ...
  const columns: ColumnsType<EnseignantInactif> = [
    {
      ...
      render: (_, r) => (
        <Button
          ...
          onClick={() => {
            navigate(`/home/analytics/teacher/${r.enseignantId}`);
          }}
        >
          {r.nom} {r.prenom}
        </Button>
    ...
    { title: 'UP', dataIndex: 'up', key: 'up', render: (v: string | null) => v ?? '—' },
    {
      ...
      render: (v: string | null) =>
        v ? new Date(v).toLocaleDateString('fr-FR') : <Tag>Jamais</Tag>,
    },
    {
      ...
      key: 'mois',
      sorter: (a, b) =>
        (a.nombreMoisDepuisDerniereFormation ?? 9999) -
        (b.nombreMoisDepuisDerniereFormation ?? 9999),
      ...
      render: (_, r) => {
        ...
        return (
          <Tooltip title={`Score: ${r.scoreRisqueDecrochage}/100`}>
            <Tag color={tag.color}>
              {tag.label} · {r.scoreRisqueDecrochage}
            </Tag>
          </Tooltip>
        );
      },
    ...
    <Card>
      <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 16 }} wrap>
        <Title level={3} style={{ margin: 0 }}>
          Enseignants inactifs (formations)
        </Title>
        <Button
          icon={<DownloadOutlined />}
          loading={exporting}
          onClick={() =>
            exportExcel('INACTIFS', {
              mois,
