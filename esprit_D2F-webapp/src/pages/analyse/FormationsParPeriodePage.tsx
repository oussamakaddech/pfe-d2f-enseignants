...
function PeriodeChart({ data, height = 260 }: { readonly data: readonly PeriodePoint[]; readonly height?: number }) {
  const view = useMemo(() => {
    const width = Math.max(360, data.length * 80);
    const padX = 40, padY = 24;
    const maxVal = Math.max(1, ...data.map((d) => Math.max(d.nombreFormations, d.nombreParticipants)));
    ...
    <div style={{ overflowX: "auto" }}>
      <svg width={view.width} height={height} aria-label="Formations par période">
        {data.map((d, i) => {
          ...
          return (
            <g key={d.label}>
              <rect x={cx - barW - 1} y={baseY - hF} width={barW} height={hF} fill="#1b5e20" rx={2} />
              <rect x={cx + 1} y={baseY - hP} width={barW} height={hP} fill="#66bb6a" rx={2} />
              <text x={cx} y={height - 6} textAnchor="middle" fontSize={10} fill="#64748b">{d.label.slice(0, 7)}</text>
            </g>
          );
        })}
      </svg>
      <Space size="large" style={{ marginTop: 8, fontSize: 13 }}>
        <span><span style={{ background: "#1b5e20", display: "inline-block", width: 12, height: 12, marginRight: 6 }} />Formations</span>
        <span><span style={{ background: "#66bb6a", display: "inline-block", width: 12, height: 12, marginRight: 6 }} />Participants</span>
      </Space>
    </div>
  );
}
  ...
  return (
    <Card loading={isLoading}>
      <Space style={{ width: "100%", justifyContent: "space-between", marginBottom: 16 }} wrap>
        <Title level={3} style={{ margin: 0 }}>Formations par période</Title>
        <Space>
      ...
      <Row gutter={16} style={{ marginBottom: 20 }}>
        <Col xs={24} sm={8}><Card><Statistic title="Total formations" value={data?.totalFormations ?? 0} /></Card></Col>
        <Col xs={24} sm={8}><Card><Statistic title="Moyenne / période" value={data?.moyenneParPeriode ?? 0} precision={2} /></Card></Col>
        <Col xs={24} sm={8}>
          <Card>
            <Statistic
              title="Tendance"
