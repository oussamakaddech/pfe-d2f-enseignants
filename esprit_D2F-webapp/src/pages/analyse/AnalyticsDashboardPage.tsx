            ...
            <tbody>
              {drillQuery.data.map((t: Record<string, unknown>, i: number) => (
                <tr key={String(t.enseignant_id ?? i)}>
                  <td className="ad-tbl-td">{String(t.nom ?? "")}</td>
                  <td className="ad-tbl-td">{formatDepartment(String(t.departement ?? "")) || "Non affecté"}</td>
                  <td className="ad-tbl-td">{formatUP(String(t.up ?? "")) || "Non affecté"}</td>
                  <td className="ad-tbl-td">{Math.round((Number(t.gap_moyen ?? 0)) * 100)}%</td>
                  <td className="ad-tbl-td"><Tag>{String(t.urgence ?? "")}</Tag></td>
                </tr>
              ))}
            </tbody>
          </table>
