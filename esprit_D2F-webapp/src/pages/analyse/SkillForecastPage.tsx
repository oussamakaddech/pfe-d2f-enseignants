...
function ForecastSvg({ data }: { readonly data: ReturnType<typeof useForecast>["data"] }) {
  ...
  const line = series.map((p, i) => `${i === 0 ? "M" : "L"} ${xOf(i)} ${yOf(p.niveau_prevu).toFixed(1)}`).join(" ");
  const bandTop = series.map((p, i) => `${i === 0 ? "M" : "L"} ${xOf(i)} ${yOf(p.borne_haute).toFixed(1)}`).join(" ");
  const bandBottom = [...series].reverse().map((p) => `L ${xOf(series.indexOf(p))} ${yOf(p.borne_basse).toFixed(1)}`).join(" ");
      ...
      {[0, 1, 2, 3, 4, 5].map((t) => {
        ...
        return (
          <g key={t}>
            ...
            <text x={padX - 8} y={y + 4} textAnchor="end" fontSize={10} fill="var(--rd-muted)">{t}</text>
          </g>
        );
      })}
...
export default function SkillForecastPage() {
  ...
  const [enseignantId, setEnseignantId] = useState<string | null>(user?.id ? String(user.id) : null);
  ...
  const columns = useMemo(() => [
    ...
    {
      title: "Actuel", dataIndex: "niveau_actuel", key: "niveau_actuel",
      render: (v: number) => <Tag color="default">{v.toFixed(1)}</Tag>,
    },
    {
      title: "Requis", dataIndex: "niveau_requis", key: "niveau_requis",
      render: (v: number) => <Tag color="blue">{v.toFixed(1)}</Tag>,
    },
    {
      title: "Prévu (final)", dataIndex: "niveau_prevu_final", key: "niveau_prevu_final",
      render: (v: number) => <Tag color="green">{v.toFixed(2)}</Tag>,
    },
    {
      title: "Écart restant", dataIndex: "ecart_restant", key: "ecart_restant",
      render: (v: number) => v.toFixed(2),
    },
    {
      title: "Objectif atteint ?", dataIndex: "comblera_objectif", key: "comblera_objectif",
      render: (b: boolean) => (b ? <Tag color="success">Oui</Tag> : <Tag color="warning">Non</Tag>),
    },
    {
      title: "Régression", dataIndex: "en_regression", key: "en_regression",
      ...
      <Section
        ...
        extra={
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <Slider min={1} max={36} value={horizon} onChange={setHorizon} style={{ width: 160 }}
              tooltip={{ formatter: (v) => `${v} mois` }} />
          </div>
        }
      >
        {isLoading && <KpiSkeleton />}
        {isError && <ErrorState message="Erreur de chargement de la prévision." onRetry={() => refetch()} />}
        {data && (
          <>
            <Card title="Niveau moyen projeté" icon={<LineChartOutlined />}>
              ...
              <div style={{ marginTop: 8, color: "var(--rd-muted)", fontSize: 12 }}>
                Pente mensuelle : {data.pente_mensuelle.toFixed(4)} niv/mois ·
                Incertitude σ : {data.incertitude_sigma.toFixed(2)} ·
                Points historiques : {data.nb_points_historiques}
              </div>
            </Card>
