...
vi.mock('@/hooks/analyse/useAnalysePredictive', () => ({
  ...
  useModelPerformance: vi.fn(() => ({
    data: { gap_model_accuracy: 0.81, last_retrained: '2026-01-01T10:00:00Z' },
    isLoading: false,
  })),
  useRiskDistribution: vi.fn(() => ({
    data: { by_level: { CRITIQUE: 5, ELEVE: 10, MODERE: 20, FAIBLE: 30 }, total: 65 },
    isLoading: false,
  })),
  ...
  useDriftStatus: vi.fn(() => ({
    data: { drift_detected: false, message: 'Aucune dérive' },
    isLoading: false,
  })),
...
const overview = {
  ...
  deltas: {
    nb_enseignants_suivis: 5,
    nb_gaps_critiques: -2,
    taux_couverture_global: 3,
    score_risque_moyen: 1,
    nb_alertes_nouvelles: 2,
  },
};
...
describe('AnalyticsPage', () => {
  ...
  it('renders hero and KPIs', () => {
    vi.mocked(predictive.useOverview).mockReturnValue({
      data: overview,
      isLoading: false,
    } as never);
  ...
  it('shows loading skeleton when overview is loading', () => {
    ...
    vi.mocked(predictive.useRiskDistribution).mockReturnValue({
      data: undefined,
      isLoading: true,
    } as never);
    vi.mocked(predictive.useDemandForecast).mockReturnValue({
      data: undefined,
      isLoading: true,
    } as never);
    renderWith(queryClient);
  ...
  it('renders all data sections when loaded', async () => {
    ...
    vi.mocked(predictive.useRiskEvolution).mockReturnValue({
      data: [{ month: '2026-01', critical: 3, high: 5 }],
      isLoading: false,
    } as never);
    vi.mocked(predictive.useDemandForecast).mockReturnValue({
      data: {
        method: 'prophet',
        history: [{ month: '2026-01', value: 10, lower: 8, upper: 12 }],
        forecast: [{ month: '2026-02', value: 14, lower: 11, upper: 17 }],
      },
      isLoading: false,
    } as never);
    vi.mocked(predictive.useGapHeatmap).mockReturnValue({
      data: [
        {
          competence_nom: 'Java',
          competence_id: 1,
    ...
    vi.mocked(predictive.useDecliningCompetencies).mockReturnValue({
      data: [{ competency_name: 'COBOL', demand_12m: 8, demand_3m: 4 }],
      isLoading: false,
    } as never);
    vi.mocked(predictive.useInDemandCompetencies).mockReturnValue({
      data: [{ competency_name: 'Rust', demand_12m: 22, demand_3m: 12 }],
      isLoading: false,
    } as never);
    vi.mocked(predictive.useTeacherRiskIndicators).mockReturnValue({
      data: [
        {
          teacher_id: 'T1',
          teacher_name: 'Alice Martin',
          departement: 'INFO',
          attrition_risk_score: 0.8,
          competency_stagnation_rate: 0.6,
          disengagement_signals: ['absence'],
          recommendation: 'Formation Python',
        },
        {
          teacher_id: 'T2',
          teacher_name: 'Bob Dupont',
          departement: 'MATH',
    ...
    vi.mocked(predictive.useAlertsSummary).mockReturnValue({
      data: {
        critiques_ouvertes: 3,
        nouvelles: 5,
        total: 12,
        by_type: [{ key: 'absence', count: 4 }],
      },
      isLoading: false,
    } as never);
    renderWith(queryClient);
  ...
  it('shows drift warning when drift detected', async () => {
    ...
    vi.mocked(predictive.useDriftStatus).mockReturnValue({
      data: { drift_detected: true, message: 'Dérive détectée' },
      isLoading: false,
    } as never);
    renderWith(queryClient);
  ...
  it('calls refresh on the Actualiser button', async () => {
    ...
    const success = vi.fn();
    vi.mocked(useAppNotification).mockReturnValue({
