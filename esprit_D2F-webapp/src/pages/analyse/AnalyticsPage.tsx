...
import {
  ...
  useDemandForecast,
  useGapHeatmap,
  useDecliningCompetencies,
  useInDemandCompetencies,
  useTeacherRiskIndicators,
  useAlertsSummary,
  useDriftStatus,
  useTrainModel,
...
function Donut({
  ...
  return (
    <svg className="ap-donut" viewBox={`0 0 ${size} ${size}`}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" className="ap-donut-num">
        {total}
      </text>
      <text x={cx} y={cy + 16} textAnchor="middle" className="ap-donut-lbl">
        enseignants
      </text>
    </svg>
  );
...
function ForecastChart({
  ...
  const band =
    points
      .map((p, i) => `${i ? 'L' : 'M'}${x(i).toFixed(1)} ${y(p.upper ?? p.value).toFixed(1)}`)
      .join(' ') +
    ' ' +
    [...points]
      .reverse()
  ...
  const projLine =
    firstProj < 0
      ...
      : points
          .slice(firstProj)
          .map((p, i) => `${i ? 'L' : 'M'}${x(firstProj + i).toFixed(1)} ${y(p.value).toFixed(1)}`)
          .join(' ');
  const ticks = points
    .map((p, i) => (i % Math.ceil(points.length / 6) === 0 ? i : -1))
    .filter((i) => i >= 0);
  return (
    <svg className="ap-fchart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      {points.map((p, i) => (
        <circle
          ...
          cx={x(i)}
          cy={y(p.value)}
          r={i >= firstProj && firstProj > 0 ? 3 : 2.5}
          fill={i >= firstProj && firstProj > 0 ? C.brand : C.cyan}
        />
      ))}
      {ticks.map((i) => (
        <text key={i} x={x(i)} y={h - 10} textAnchor="middle" className="ap-axis">
          {points[i].month}
        </text>
      ))}
    </svg>
...
function AreaChart({
  ...
  const ticks = series
    .map((_, i) => (i % Math.ceil(series.length / 6) === 0 ? i : -1))
    .filter((i) => i >= 0);
  return (
    <svg className="ap-achart" viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none">
      ...
      {ticks.map((i) => (
        <text key={i} x={x(i)} y={h - 9} textAnchor="middle" className="ap-axis">
          {series[i].month}
        </text>
      ))}
    </svg>
...
function Gauge({ value, label }: { readonly value: number | null; readonly label: string }) {
  ...
  return (
    <svg className="ap-gauge" viewBox="0 0 180 110">
      ...
      />
      <text x={cx} y={cy - 14} textAnchor="middle" className="ap-gauge-num" fill={col}>
        {value == null ? '—' : `${Math.round(v * 100)}%`}
      </text>
      <text x={cx} y={cy + 6} textAnchor="middle" className="ap-gauge-lbl">
        {label}
      </text>
    </svg>
  );
...
function HBar({
  ...
  readonly rows: { readonly label: string; readonly value: number }[];
  readonly color?: string;
}) {
    ...
    <div className="ap-hbars">
      {rows.map((r) => (
        <div className="ap-hbar" key={r.label}>
          <span className="ap-hbar-label" title={r.label}>
            {r.label}
          </span>
    ...
    <header className="ap-hero">
      ...
      <div className="ap-hero-main">
        <div className="ap-hero-title">
          <span className="ap-hero-kicker">
            <ExperimentOutlined /> Analyse Prédictive · D2F
          </span>
          <h1>Intelligence des compétences &amp; anticipation des risques</h1>
          <p className="ap-hero-sub">
            Modèles ML temps-réel · {dayjs().format('dddd D MMMM YYYY')} · Horizon de projection{' '}
            <b>{horizon} mois</b>
          </p>
        </div>
        <div className="ap-hero-actions">
            ...
            <button
              ...
              onClick={() =>
                train.mutate(undefined, {
                  onSuccess: () => {
                    message.success('Ré-entraînement du modèle lancé');
                    refresh();
                  },
                  onError: () => message.error('Échec du ré-entraînement'),
                })
              }
              disabled={!isAdmin || train.isPending}
            >
          ...
          <Tooltip title="Actualiser les données">
            <button className="ap-btn ap-btn-primary" onClick={refresh} disabled={refreshing}>
              <ReloadOutlined className={refreshing ? 'ap-spin' : ''} />{' '}
      ...
      <div className="ap-hero-meta">
        <div className={`ap-model-badge ${drift?.drift_detected ? 'is-warn' : 'is-ok'}`}>
          ...
          <div>
            <b>Modèle</b>
            <span>
              {drift?.drift_detected ? 'Dérive détectée' : 'Stable'} ·{' '}
              {ov?.precision_modele != null
                ? `précision ${Math.round(ov.precision_modele * 100)}%`
                : '—'}
            </span>
          </div>
        </div>
        <div className="ap-hero-filters">
          <span className="ap-filter-ico">
            <FilterOutlined />
          </span>
          <Select
            allowClear
            ...
            classNames={{ popup: { root: 'ap-pop' } }}
          />
          <Segmented
            options={SEG.map((s) => ({ label: s.label, value: s.value }))}
            value={horizon}
...
function RiskDonutSection({
  donutSegments,
  riskDist,
  riskEvo,
  horizon,
}: {
  readonly donutSegments: { label: string; value: number; color: string }[];
  readonly riskDist: ReturnType<typeof useRiskDistribution>;
  readonly riskEvo: ReturnType<typeof useRiskEvolution>;
  readonly horizon: number;
}) {
  return (
    <section className="ap-grid ap-grid-2">
      <div className="ap-card">
        <div className="ap-card-head">
          <h3>
            <AimOutlined /> Répartition du risque enseignant
          </h3>
          <Tag color={donutSegments[0].value ? 'red' : 'green'}>
            {riskDist.data?.total ?? 0} évalués
          </Tag>
        </div>
      ...
      <div className="ap-card">
        <div className="ap-card-head">
          <h3>
            <RiseOutlined /> Évolution du risque · {horizon} mois
          </h3>
        ...
        <div className="ap-card-body">
          ...
          <div className="ap-legend ap-legend-inline">
            <span>
              <span className="ap-dot" style={{ background: C.red }} /> Critique
            </span>
            <span>
              <span className="ap-dot" style={{ background: C.amber }} /> Élevé
            </span>
          </div>
        </div>
      </div>
...
function ForecastSection({
  ...
  readonly demand: ReturnType<typeof useDemandForecast>;
  ...
  return (
    <section className="ap-card">
      <div className="ap-card-head">
        <h3>
          <LineChartOutlined /> Prévision de la demande de compétences
        </h3>
        <span className="ap-chip">
          {demand.data?.method ?? 'modèle'} · zone = intervalle de confiance
        </span>
      </div>
      <div className="ap-card-body">
        ...
        <div className="ap-legend ap-legend-inline">
          <span>
            <span className="ap-line" style={{ background: C.cyan }} /> Historique
          </span>
          <span>
            <span className="ap-line ap-dashed" style={{ background: C.brand }} /> Projection
          </span>
        </div>
      </div>
    </section>
...
function WhatIfSection({
  ...
  readonly simTeachers: { teacher_id: string; teacher_name: string; departement?: string }[];
  readonly simCompetences: { competence_id: number; competence_nom: string }[];
  readonly filteredAtRisk: { teacher_id: string }[];
}) {
  return (
    <section className="ap-card">
      <div className="ap-card-head">
        <h3>
          <ExperimentOutlined /> Simulateur d'impact · projection &amp; scénarios
        </h3>
...
function HeatmapSection({
  ...
  readonly heatRows: {
    comps: string[];
    rows: Map<string, { dept: string; avg: number; count: number }[]>;
  };
  readonly depts: string[];
  readonly selectedCell: { dept: string; comp: string } | null;
  readonly setSelectedCell: (v: { dept: string; comp: string } | null) => void;
}) {
  return (
    <section className="ap-card">
      <div className="ap-card-head">
        <h3>
          <DashboardOutlined /> Cartographie des écarts (Gap Heatmap)
        </h3>
        {selectedCell && (
          <button className="ap-btn ap-btn-ghost ap-btn-sm" onClick={() => setSelectedCell(null)}>
            Réinitialiser · {selectedCell.dept} / {selectedCell.comp}
          </button>
        )}
      </div>
      <div className="ap-card-body">
          ...
          <div className="ap-heat">
            <div className="ap-heat-row ap-heat-head">
              ...
              {depts.slice(0, 10).map((d) => (
                <span className="ap-heat-col" key={d} title={d}>
                  {d.slice(0, 6)}
                </span>
              ))}
            </div>
            {heatRows.comps.map((comp) => (
              <div className="ap-heat-row" key={comp}>
                <span className="ap-heat-rowlabel" title={comp}>
                  {comp}
                </span>
                {depts.slice(0, 10).map((d) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: cell ? gapColor(g) : 'transparent',
                        opacity: cell ? 0.35 + (g / 2.5) * 0.6 : 1,
                      }}
                      disabled={!cell}
                      onClick={() => cell && setSelectedCell({ dept: d, comp })}
                      title={
                        cell ? `${comp} · ${d} · écart ${g.toFixed(1)} · ${cell.count} ens.` : '—'
                      }
                    >
                      {cell ? g.toFixed(1) : ''}
                    </button>
...
function DeclineDemandSection({
  declining,
  inDemand,
}: {
  readonly declining: ReturnType<typeof useDecliningCompetencies>;
  readonly inDemand: ReturnType<typeof useInDemandCompetencies>;
}) {
  return (
    <section className="ap-grid ap-grid-2">
      <div className="ap-card">
        ...
        <div className="ap-card-body">
          {declining.data?.length ? (
            <HBar
              rows={declining.data.slice(0, 8).map((c) => ({
                label: c.competency_name,
                value: c.demand_12m ?? c.demand_3m ?? 0,
              }))}
              color={C.red}
            />
          ) : (
      ...
      <div className="ap-card">
        <div className="ap-card-head">
          <h3>
            <RocketOutlined /> Compétences en forte demande
          </h3>
        ...
        <div className="ap-card-body">
          {inDemand.data?.length ? (
            <HBar
              rows={inDemand.data.slice(0, 8).map((c) => ({
                label: c.competency_name,
                value: c.demand_12m ?? c.demand_3m ?? 0,
              }))}
              color={C.green}
...
function ModelPerfSection({
  ...
  readonly model: ReturnType<typeof useModelPerformance>;
  readonly drift: ReturnType<typeof useDriftStatus>;
  readonly alerts: ReturnType<typeof useAlertsSummary>;
  readonly donutSegments: { label: string; value: number; color: string }[];
  readonly horizon: number;
  ...
  return (
    <section className="ap-grid ap-grid-3">
      <div className="ap-card ap-center">
        <div className="ap-card-head">
          <h3>
            <ExperimentOutlined /> Précision du modèle
          </h3>
        </div>
        ...
        <div className="ap-model-meta">
          ...
          <b>
            {model.data?.last_retrained
              ? dayjs(model.data.last_retrained).format('DD/MM/YY HH:mm')
              : '—'}
          </b>
        </div>
        <div className={`ap-drift ${drift.data?.drift_detected ? 'is-warn' : 'is-ok'}`}>
          ...
          {drift.data?.message ??
            (drift.data?.drift_detected ? 'Dérive détectée' : 'Aucune dérive')}
        </div>
      </div>
      ...
      <div className="ap-card">
        <div className="ap-card-head">
          <h3>
            <BellOutlined /> Alertes par sévérité
          </h3>
        </div>
        <div className="ap-card-body">
          {alerts.data ? (
            <div className="ap-alert-stats">
              <div className="ap-alert-stat ap-crit">
                <b>{alerts.data.critiques_ouvertes}</b>
                <span>critiques</span>
              </div>
              <div className="ap-alert-stat ap-new">
                <b>{alerts.data.nouvelles}</b>
                <span>nouvelles</span>
              </div>
              <div className="ap-alert-stat ap-tot">
                <b>{alerts.data.total}</b>
                <span>total</span>
              </div>
            </div>
          ) : (
          ...
          <div className="ap-chips">
            {(alerts.data?.by_type ?? []).slice(0, 4).map((t) => (
              <Tag key={t.key} className="ap-tag">
                {t.key.replaceAll('_', ' ')} · {t.count}
              </Tag>
            ))}
          </div>
        </div>
      ...
      <div className="ap-card">
        <div className="ap-card-head">
          <h3>
            <BulbOutlined /> Recommandation IA
          </h3>
        </div>
        <div className="ap-card-body ap-reco">
          <p>
            Le modèle anticipe <b>{donutSegments[0].value + donutSegments[1].value}</b> enseignants
            à risque élevé/critique sur <b>{horizon} mois</b>.
          </p>
          <p>
            Priorisez les formations sur les <b>{inDemand.data?.length ?? 0}</b> compétences en
            forte demande avant la prochaine rentrée.
          </p>
          <button
...
function RiskTableSection({
  ...
  return (
    <section className="ap-card">
      <div className="ap-card-head">
        <h3>
          <TeamOutlined /> Enseignants à risque {deptFilter ? `· ${deptFilter}` : ''}
        </h3>
          ...
          {filteredAtRisk.slice(0, 12).map((t) => {
            ...
            return (
              <div className="ap-trow" key={t.teacher_id}>
                ...
                <span>
                  <span
                    className="ap-risk-pill"
                    style={{ background: RISK_COLORS[lvl], color: '#0b1020' }}
                  >
                    {Math.round(t.attrition_risk_score * 100)}%
                  </span>
                </span>
                <span className={t.competency_stagnation_rate > 0.5 ? 'ap-down' : 'ap-up'}>
                  {t.competency_stagnation_rate > 0.5 ? <FallOutlined /> : <RiseOutlined />}{' '}
                  {t.competency_stagnation_rate > 0.5 ? 'Stagnation' : 'Actif'}
                </span>
                <span className="ap-signals">
                  {(t.disengagement_signals ?? []).slice(0, 2).join(', ') || '—'}
                </span>
...
export default function AnalyticsPage() {
  ...
  const filteredAtRisk = useMemo(
    () =>
      deptFilter
        ? (atRisk.data ?? []).filter((t) => t.departement === deptFilter)
        : (atRisk.data ?? []),
  ...
  const simCompetences = useMemo(() => {
    ...
    return Array.from(map, ([competence_id, competence_nom]) => ({
      competence_id,
      competence_nom,
    }));
  }, [heatmap.data]);
  ...
  const kpis = [
    {
      ...
      value: ov?.nb_enseignants_suivis ?? null,
      delta: ov?.deltas.nb_enseignants_suivis,
      spark: [10, 12, 11, 14, 16, 18, ov?.nb_enseignants_suivis ?? 0],
      color: C.brand,
    },
    {
      icon: <WarningOutlined />,
      label: 'Gaps critiques',
      value: ov?.nb_gaps_critiques ?? null,
      delta: ov?.deltas.nb_gaps_critiques,
      spark: [8, 7, 9, 6, 8, 7, ov?.nb_gaps_critiques ?? 0],
      color: C.red,
    },
    {
      icon: <SafetyCertificateOutlined />,
      label: 'Couverture globale',
      value: ov?.taux_couverture_global ?? null,
      suffix: '%',
      delta: ov?.deltas.taux_couverture_global,
      spark: [60, 62, 65, 64, 68, 70, ov?.taux_couverture_global ?? 0],
      color: C.green,
    },
    {
    ...
    },
    {
      ...
      value: ov?.nb_alertes_nouvelles ?? null,
      delta: ov?.deltas.nb_alertes_nouvelles,
      spark: [3, 5, 4, 6, 5, 7, ov?.nb_alertes_nouvelles ?? 0],
      color: C.violet,
    },
  ];
    ...
    <div className="ap-root">
      ...
      {loading ? (
        ...
        <>
          ...
          <WhatIfSection
            simTeachers={simTeachers}
            simCompetences={simCompetences}
