...
const ETAT_CONFIG: Record<
  EtatDemande,
  { color: 'success' | 'error' | 'warning'; icon: React.ReactNode; text: string }
> = {
...
async function sendTraitementEmail(
  ...
  const dStart = formation?.dateDebut
    ? dayjs(String(formation.dateDebut)).format('DD/MM/YYYY')
    : null;
  const dEnd = formation?.dateFin ? dayjs(String(formation.dateFin)).format('DD/MM/YYYY') : null;
  const dateRange =
    dStart && dEnd ? `<br>📅 Du <strong>${dStart}</strong> au <strong>${dEnd}</strong>` : '';
  const subject = approuver
    ? `✅ Inscription approuvée — ${titre}`
    : `❌ Inscription rejetée — ${titre}`;
  ...
  return {
    ...
    render: (text: string) =>
      searchedColumn === dataIndex ? (
        <span style={{ backgroundColor: '#ffc069', padding: '0 4px', borderRadius: 4 }}>
          {text}
        </span>
      ) : (
        text
...
export default function DemandesList() {
  ...
  const {
    ...
    isLoading: loading,
    refetch,
  } = useInscriptionsByFormation(formationId);
    ...
    try {
      ...
      if (target) {
        await sendTraitementEmail(
          ...
          motif,
          formation as
            | { titreFormation?: string; dateDebut?: string; dateFin?: string }
            | undefined,
          formationId,
      ...
      render: (_, r) => (
        <Space>
          <Button
            ...
            style={
              r.etat === 'APPROVED' ? {} : { backgroundColor: '#1D6F42', borderColor: '#1D6F42' }
            }
          >
            Approuver
          </Button>
      ...
      >
        {rejectTarget && (
          <Space direction="vertical" size={12} style={{ width: '100%' }}>
            ...
            <Text type="secondary">Un email de notification lui sera envoyé automatiquement.</Text>
            <div>
              <label
