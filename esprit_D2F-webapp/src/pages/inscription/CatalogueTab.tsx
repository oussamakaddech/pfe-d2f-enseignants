...
import {
  Row, Col, Card, Space, Select, DatePicker, Input,
  Badge, Typography, Tag, Button,
} from "antd";
import {
...
function filterFormations(
  list: FormationItem[], search: string, type?: string,
  up?: string, dept?: string, ouverte?: boolean, dateRange: Dayjs[] = [],
): FormationItem[] {
...
export default function CatalogueTab() {
  ...
  const { data: myInscriptions = [] } = useInscriptionsByEnseignant((enseignantSelf as { id?: Id } | undefined)?.id);
  ...
  const resetFilters = () => {
    setSearch(""); setTypeFilter(undefined); setUpFilter(undefined);
    setDeptFilter(undefined); setOuverteFilter(undefined); setDateRange([]);
  };
  ...
  return (
    <div className="cat-page">
              ...
              <Card className="cat-card" styles={{ body: { padding: 0 } }}>
                ...
                <div className="cat-card-body">
                  ...
                  <div className="cat-card-tags">
                    ...
                    {isOpen && <Tag color="success" className="blink">Actif</Tag>}
                    {(() => {
                      const d = daysUntilStart(f.dateDebut);
                ...
                <div className="cat-card-actions">
                  <Space size={8} wrap>
                    <Button size="small" icon={<EyeOutlined />} onClick={() => navigate(`/home/ListeFormation/${f.idFormation}`)}>Détails</Button>
                    {isTeacher && isOpen && (
                      (requested.includes(f.idFormation!) || requestedServer.has(String(f.idFormation)))
                        ? <Button size="small" disabled icon={<CheckCircleOutlined />}>Envoyée</Button>
                        : <Button type="primary" size="small" icon={<UserAddOutlined />} onClick={() => f.idFormation != null && handleDemande(f.idFormation)}>S'inscrire</Button>
                    )}
                    {isAdminLike && (
                      <Button size="small" danger={!!isOpen} icon={isOpen ? <LockOutlined /> : <UnlockOutlined />} onClick={() => f.idFormation != null && handleToggle(f.idFormation)}>
                        {isOpen ? "Fermer" : "Ouvrir"}
                      </Button>
                    )}
                    {isAdminLike && (
                      <Button size="small" icon={<TeamOutlined />} onClick={() => navigate(`/home/ListeFormation/${f.idFormation}/demandes`)}>Demandes</Button>
                    )}
                  </Space>
                </div>
              </Card>
      ...
      {filtered.length === 0 && (
        <EmptyStateStandard title="Aucun résultat" description="Aucune formation ne correspond aux filtres." />
      )}
    </div>
  );
}
