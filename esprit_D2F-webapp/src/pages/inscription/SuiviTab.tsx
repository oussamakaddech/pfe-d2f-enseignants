...
import { Table, Input, Select, Button, Tooltip, Tag, Modal, Space } from 'antd';
...
import {
  useAllInscriptions,
  useInscriptionsByFormation,
  useTraiterDemande,
  useTraiterDemandeBulk,
  useSendEmail,
...
export default function SuiviTab() {
  ...
  const upOpts = useMemo(
    () =>
      [...new Set(rows.map((r) => r.enseignant?.upLibelle).filter(Boolean))].map((u) => ({
        label: u as string,
        value: u as string,
      })),
    [rows],
  ...
  const deptOpts = useMemo(
    () =>
      [...new Set(rows.map((r) => r.enseignant?.deptLibelle).filter(Boolean))].map((d) => ({
        label: d as string,
        value: d as string,
      })),
    [rows],
  ...
  const handleToggle = (f: FormationRow) => {
    ...
    modal.confirm({
      ...
      content: isOpen
        ? `Les inscriptions pour "${f.titreFormation}" seront fermées.`
        : `Les inscriptions pour "${f.titreFormation}" seront ouvertes.`,
      okText: isOpen ? 'Fermer' : 'Ouvrir',
      cancelText: 'Annuler',
      okButtonProps: isOpen ? { danger: true } : { type: 'primary' },
      centered: true,
      onOk: async () => {
        try {
          await updateOuvertes({ id, ouvert: !isOpen });
          msgApi.success(isOpen ? 'Fermées' : 'Ouvertes');
        } catch {
          msgApi.error('Erreur lors de la mise à jour');
        }
      },
    });
  };
  ...
  const { data: demandesRaw = [], isLoading: demandesLoading } = useInscriptionsByFormation(
    selectedFormationId ?? undefined,
  );
  const { data: selectedFormation } = useFormationById(selectedFormationId ?? undefined);
  const demandes = useMemo(() => {
    const list = Array.isArray(demandesRaw)
      ? demandesRaw
      : ((demandesRaw as { content?: Demande[] })?.content ?? []);
  ...
  const handleApprove = async (id: Id) => {
    try {
      ...
      msgApi.error('Échec du traitement');
    }
  };
  ...
  const handleBulkApprove = async () => {
    try {
      ...
      msgApi.error('Échec du traitement groupé');
    }
  };

  const handleBulkReject = async () => {
    try {
      await traiterBulkMut({
        ids: selectedRowIds as Id[],
        approuver: false,
        motif: bulkMotif || undefined,
      });
      ...
      msgApi.error('Échec du rejet groupé');
    }
  };
  ...
  const demandesColumns: TableColumnsType<Demande> = [
    ...
    },
    {
      ...
      render: (e: Etat) => {
        const c = {
          APPROVED: { cls: 'ins-badge--approved', text: 'Approuvé' },
          REJECTED: { cls: 'ins-badge--rejected', text: 'Rejeté' },
          PENDING: { cls: 'ins-badge--pending', text: 'En attente' },
        }[e] ?? { cls: 'ins-badge--pending', text: '?' };
  ...
  const globalColumns: TableColumnsType<InscriptionRow> = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <span>
          <ApartmentOutlined style={{ marginRight: 6, color: '#b51200' }} />
          {r.enseignant?.deptLibelle || '—'}
        </span>
      ),
      sorter: (a, b) =>
        (a.enseignant?.deptLibelle || '').localeCompare(b.enseignant?.deptLibelle || ''),
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          <div style={{ fontWeight: 600, fontSize: 13 }}>{r.formation?.titreFormation || '—'}</div>
          <div style={{ fontSize: 12, color: '#9ca3af' }}>
            {fmt(r.formation?.dateDebut)} → {fmt(r.formation?.dateFin)}
          </div>
        </div>
      ),
    },
    {
      ...
      responsive: ['md'],
      render: (_, r) => {
        const o = r.formation?.inscriptionsOuvertes;
        return (
          <span className={`ins-badge ${o ? 'ins-badge--open' : 'ins-badge--closed'}`}>
            {o ? (
              <>
                <UnlockOutlined /> Ouvertes
              </>
            ) : (
              <>
                <LockOutlined /> Fermées
              </>
            )}
          </span>
        );
    ...
    },
    {
      ...
      dataIndex: 'dateDemande',
      width: 120,
      render: (d: string) => <span style={{ color: '#64748b', fontSize: 13 }}>{fmt(d)}</span>,
      sorter: (a, b) => new Date(a.dateDemande).getTime() - new Date(b.dateDemande).getTime(),
    },
  ];
    ...
    <div className="suv-page">
      ...
      <div className="suv-section">
        <div className="suv-section-header">
          ...
          <Tag color="success">
            {formationStats.open} ouverte{formationStats.open === 1 ? '' : 's'}
          </Tag>
          <Tag color="error">
            {formationStats.closed} fermée{formationStats.closed === 1 ? '' : 's'}
          </Tag>
        </div>
        <div className="suv-formations-grid">
          {formationsList.length === 0 && (
            <span style={{ color: '#9ca3af', fontSize: 13 }}>Aucune formation trouvée.</span>
          )}
          {formationsList.map((f) => {
            ...
            return (
              <Tooltip
                ...
                title={`${o ? 'Fermer' : 'Ouvrir'} les inscriptions`}
              >
                <Button
                  size="small"
                  className={`suv-toggle ${o ? 'suv-toggle--open' : 'suv-toggle--closed'}`}
      ...
      <div className="suv-section">
        ...
        <div style={{ padding: '0 20px 16px' }}>
          <Select
            ...
            onChange={(v) => {
              setSelectedFormationId(v);
              setSelectedRowIds([]);
            }}
            options={formationsList.map((f) => ({
              label: f.titreFormation || `Formation #${f.idFormation}`,
              value: f.idFormation!,
            }))}
          />
        </div>
          ...
          <div className="ins-card">
            <div className="ins-card-toolbar" style={{ justifyContent: 'space-between' }}>
              <Space>
                <span style={{ fontWeight: 600, fontSize: 14 }}>
                  {selectedFormation?.titreFormation || 'Formation'}
                </span>
                <Tag>
              ...
              <Space>
                ...
                <Button
                  ...
                  className="ins-btn ins-btn--green"
                >
                  Exporter
                </Button>
              </Space>
            ...
            {selectedRowIds.length > 0 && (
              <div className="suv-bulk-bar">
                <span>
                  {selectedRowIds.length} sélectionnée{selectedRowIds.length > 1 ? 's' : ''}
                </span>
                <Space>
                  <Button
                    size="small"
            ...
            <Table<Demande>
              ...
              rowSelection={{
                ...
                getCheckboxProps: (r) => ({ disabled: r.etat !== 'PENDING' }),
              }}
      ...
      <div className="ins-card">
        ...
        <Table<InscriptionRow>
          ...
          pagination={{
            ...
            showTotal: (t) => `${t} inscription${t > 1 ? 's' : ''}`,
          }}
          scroll={{ x: 1100 }}
        />
      </div>
      ...
      {/* ── Bulk reject modal ── */}
      <Modal
        title={`Rejeter ${selectedRowIds.length} demande(s)`}
        open={bulkRejectOpen}
      ...
      >
        <Input.TextArea
          ...
          maxLength={500}
          showCount
          placeholder="Motif partagé (optionnel)"
          value={bulkMotif}
