      ...
      [
        {
          ...
          rows: displayed.map((r) => ({
            ...
            'Date demande': fmtDT(r.dateDemande),
            Motif: r.motif || '—',
          })),
  ...
  return (
    <div className="mis-page">
      <AppPageHeader
        ...
        actions={
          <Space wrap>
            <Button icon={<ReloadOutlined />} onClick={() => void refetch()} className="ins-btn">
              Actualiser
            </Button>
            <Button
            ...
            <Button
              type="primary"
              icon={<SendOutlined />}
              onClick={() => navigate('/home/Inscriptions?tab=catalogue')}
              className="ins-btn"
            >
              S'inscrire
            </Button>
      ...
      {error && (
        <Alert
          ...
          description={
            (error as { message?: string })?.message || 'Impossible de récupérer vos inscriptions.'
          }
        />
      )}
              ...
              >
                <div className="mis-row-body">
                  ...
                  <div className="mis-row-meta">
                    <span>
                      <CalendarOutlined /> {fmt(r.dateDebut)} → {fmt(r.dateFin)}
                    </span>
                    {r.chargeHoraire && r.chargeHoraire !== 'null' && (
                      <span>
                        <FieldTimeOutlined /> {r.chargeHoraire} h
                      </span>
                    )}
                  ...
                  {vis.length > 0 && (
                    <div className="mis-row-tags">
                      {vis.map((c) => (
                        <Tag key={c} color="processing" className="mis-tag">
                          {c}
                        </Tag>
                      ))}
                  ...
                  {r.dateTraitement && r.etat !== 'PENDING' && (
                    <div className="mis-timeline">
                      <Timeline
                        items={[
                          {
                            dot: <CalendarOutlined style={{ fontSize: 12, color: '#9ca3af' }} />,
                            children: (
                              <span className="mis-timeline-text">
                                Créée le <strong>{fmtDT(r.dateDemande)}</strong>
                              </span>
                            ),
                          },
                          {
                            dot:
                              r.etat === 'APPROVED' ? (
                                ...
                                <CloseCircleOutlined style={{ fontSize: 12, color: '#b51200' }} />
                              ),
                            children: (
                              <span className="mis-timeline-text">
                                {r.etat === 'APPROVED' ? 'Approuvée' : 'Rejetée'} le{' '}
                                <strong>{fmtDT(r.dateTraitement)}</strong>
                              </span>
                            ),
                          },
                        ]}
