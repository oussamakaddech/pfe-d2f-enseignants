...
vi.mock("@/services/formation/EnseignantService", () => ({
  default: { createEnseignantWithAccount: (...args: unknown[]) => mockCreateEnseignantWithAccount(...args) },
}));
...
function renderDrawer(props: { open?: boolean; onSuccess?: () => void; initialValues?: Record<string, string> } = {}) {
...
function fillBaseAccountFields() {
  ...
  fireEvent.change(getFieldByLabel("Adresse email"), { target: { value: "ahmed.benali@esprit.tn" } });
  ...
  fireEvent.change(getFieldByLabel("Confirmer le mot de passe"), { target: { value: "Password1!" } });
}
...
describe("CreateAccountDrawer", () => {
  ...
  it("gère l'erreur 409 (username déjà pris)", async () => {
    ...
    await waitFor(() => {
      expect(mockError).toHaveBeenCalledWith(
        expect.stringContaining("nom d'utilisateur")
      );
    });
  });
});
