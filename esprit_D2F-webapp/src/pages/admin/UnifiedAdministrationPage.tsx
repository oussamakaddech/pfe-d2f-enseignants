...
export default function UnifiedAdministrationPage() {
  ...
  const { data: teachers = [], isLoading: teachersLoading } = useQuery<UnifiedRow[]>({
    ...
    queryFn: async () => {
      ...
      else if (Array.isArray((raw as { content?: unknown[] })?.content))
    ...
    if (allAccounts) {
      const normalized = (allAccounts as unknown as UnifiedRow[]).map((acc) => {
        ...
        return {
          ...
          _key: `acc_${acc.userId ?? acc.id ?? crypto.randomUUID()}`,
          status: statusValue,
          deleted: isDeleted,
        };
      });
  ...
  const unifiedData = useMemo(() => {
    ...
    const rows: UnifiedRow[] = teachers.map((t) => {
      ...
      const acc =
        (tUserId ? accountById.get(tUserId) : undefined) ??
  ...
  const displayedData = useMemo(() => {
    ...
    switch (sortBy) {
      case 'name_asc':
        sorted.sort((a, b) => rowFullName(a).localeCompare(rowFullName(b)));
        break;
      case 'name_desc':
        sorted.sort((a, b) => rowFullName(b).localeCompare(rowFullName(a)));
        break;
      case 'email_asc':
        sorted.sort((a, b) => (a.email || a.mail || '').localeCompare(b.email || b.mail || ''));
        break;
      case 'role_asc':
        sorted.sort((a, b) => (a.role || a.type || '').localeCompare(b.role || b.type || ''));
        break;
      case 'source':
  ...
  const handleToggleStatus = (record: UnifiedRow) => {
    ...
    const fullName =
      `${record.firstName || ''} ${record.lastName || ''}`.trim() || record.userName || '';
    modal.confirm({
      title: willBlock ? 'Bloquer ce compte ?' : 'Débloquer ce compte ?',
      content: willBlock ? (
        <p>
          Le compte de <strong>{fullName}</strong> ne pourra plus se connecter.
        </p>
      ) : (
        <p>
          Le compte de <strong>{fullName}</strong> pourra de nouveau se connecter.
        </p>
      ),
  ...
  const handlePermanentDeleteAccount = (record: UnifiedRow) => {
    ...
    modal.confirm({
      ...
      content: (
        <p>
          Le compte archivé de <strong>{fullName}</strong> sera définitivement effacé de la base de
  ...
  const handleDeleteMerged = (record: UnifiedRow) => {
    ...
    modal.confirm({
      ...
      content: (
        <p>
          Le compte <strong>et</strong> la fiche enseignant de <strong>{fullName}</strong> seront
  ...
  const columns: TableColumnsType<UnifiedRow> = [
    {
      ...
      render: (_, record) => {
        ...
        const showCupChef =
          (isTeacherOnly || isMerged) &&
          (isTruthyFlag(record.cup) || isTruthyFlag(record.chefDepartement));
        return (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              >
                ...
                {showCupChef && (
                  <>
                    {isTruthyFlag(record.cup) && (
                      <span className="teachers-badge teachers-badge--cup">CUP</span>
                    )}
                    {isTruthyFlag(record.chefDepartement) && (
      ...
      render: (_, record) => {
        if (record._type === 'teacher' || record._type === 'merged') {
          return (
            <div>
              {record.upLibelle && (
                <div style={{ fontSize: 13, color: neutral[700] }}>
                  <BankOutlined style={{ marginRight: 6, fontSize: 11 }} />
                  {record.upLibelle}
                </div>
              )}
              {record.deptLibelle && (
                <div style={{ fontSize: 12, color: neutral[500] }}>{record.deptLibelle}</div>
              )}
              {!record.upLibelle && !record.deptLibelle && (
  ...
  return (
    <div className="accounts-page">
        ...
        >
          <div>
            ...
            <div className="accounts-hero-subtitle">
              Gérer les comptes utilisateurs et les fiches enseignants
            </div>
          </div>
      ...
      <Modal
        ...
        onOk={() => void handleEditAccountSubmit()}
        onCancel={() => {
          setEditAccountModalVisible(false);
          editAccountForm.resetFields();
        }}
        confirmLoading={editAccountLoading}
        okText="Enregistrer"
        cancelText="Annuler"
        centered
        width={520}
