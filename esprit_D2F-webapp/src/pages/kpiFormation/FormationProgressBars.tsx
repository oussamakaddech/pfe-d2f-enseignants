...
export default function FormationProgressCards() {
  ...
  const filtered = forms
    .filter(
      (f) =>
        !searchTitle || (f.titreFormation ?? '').toLowerCase().includes(searchTitle.toLowerCase()),
    )
    .filter((f) => !searchDomaine || f.domaine?.toLowerCase().includes(searchDomaine.toLowerCase()))
    .filter((f) => !selectedUp || f.up1?.id === selectedUp)
    .filter((f) => !selectedDept || f.departement1?.id === selectedDept);
    ...
    <div style={styles.wrapper}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
        ...
        <div>
          ...
          <span style={{ fontSize: 12, color: '#6b7280' }}>
            Participation des enseignants par formation
          </span>
        </div>
      </div>
      ...
      <Space wrap style={styles.filterBar}>
        <RangePicker
          ...
          format="YYYY-MM-DD"
          onChange={(dates) =>
            dates && setRange([dates[0] as dayjs.Dayjs, dates[1] as dayjs.Dayjs])
          }
        />
        <Search
