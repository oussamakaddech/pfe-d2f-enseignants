...
interface FormationWithSeances {
  seances?: Seance[];
}
...
export default function DonutByTrainerTypeWithFilters() {
  ...
  const fetchCountsAndLists = async () => {
    try {
      ...
      setCountsData(data);
      const findById = (id: unknown) =>
        (allFormations as Record<string, unknown>[]).find(
          (f) => f.idFormation === id || f.id === id,
        ) || null;
      setExterneFormations(
        ((data.externeOnlyIds as unknown[]) || []).map(findById).filter(Boolean) as Record<
          string,
          unknown
        >[],
      );
      setInterneFormations(
        ((data.interneOnlyIds as unknown[]) || []).map(findById).filter(Boolean) as Record<
          string,
          unknown
        >[],
      );
      setMixteFormations(
        ((data.mixteIds as unknown[]) || []).map(findById).filter(Boolean) as Record<
          string,
          unknown
        >[],
      );
          ...
          <List
            ...
            renderItem={(formation: Record<string, unknown>) => {
              ...
              const isExterne = Boolean(
                ...
                String(formation.externeFormateurEmail).trim() !== '',
              );
              return (
                <List.Item key={String(formation.idFormation)} style={{ padding: '12px 0' }}>
                  <List.Item.Meta
                    avatar={
                      <Avatar icon={<BookOutlined />} style={{ backgroundColor: '#FF4D4F' }} />
                    }
                    title={
                      <Row justify="space-between" align="middle">
                        ...
                        <Col>
                          <Text type="secondary">
                            {String(formation.dateDebut ?? '').split('T')[0] ||
                      ...
                      <div style={{ fontSize: '0.875rem', marginTop: 4 }}>
                        <Row gutter={8} align="middle">
                          <Col>
                            <HomeOutlined />{' '}
                            <Text>
                              {((formation.departement1 as Record<string, unknown>)
                                ?.libelle as string) || '—'}
                            </Text>
                          </Col>
                          <Col>
                            <HomeOutlined />{' '}
                            <Text>
                              {((formation.up1 as Record<string, unknown>)?.libelle as string) ||
                                '—'}
                            </Text>
                          </Col>
                        </Row>
                          ...
                          <div style={{ marginTop: 8 }}>
                            ...
                            <div style={{ marginLeft: 16 }}>
                              <Row align="middle" gutter={4}>
                                ...
                                <Col>
                                  <Text>
                                    {String(formation.externeFormateurNom)}{' '}
                                    {String(formation.externeFormateurPrenom)}
                                  </Text>
                                </Col>
                              </Row>
                              <Row align="middle" gutter={4} style={{ marginTop: 4 }}>
                                <Col>
                                  <MailOutlined />
                                </Col>
                                <Col>
                                  <Text>{String(formation.externeFormateurEmail)}</Text>
                                </Col>
                              </Row>
                              {Boolean(formation.organismeRefExterne) && (
                                <Row align="middle" gutter={4} style={{ marginTop: 4 }}>
                                  <Col>
                                    <Text strong>Organisme :</Text>
                                  </Col>
                                  <Col>
                                    <Text>{String(formation.organismeRefExterne)}</Text>
                                  </Col>
                                </Row>
                              )}
                            </div>
                        ...
                        {interneAnimateurs.length > 0 && (
                          <div style={{ marginTop: 8 }}>
                            <Text strong>
                              Animateur{interneAnimateurs.length > 1 ? 's' : ''} interne
                              ...
                              {interneAnimateurs.map((a) => (
                                <div key={String(a.id)} style={{ marginBottom: 4 }}>
                                  ...
                                  >
                                    ...
                                    <Col>
                                      <Text type="secondary">{String(a.mail)}</Text>
                                    </Col>
                                  </Row>
                                </div>
                              ))}
      ...
      >
        <Form
          ...
          initialValues={{
            ...
            dateRange:
              filters.start && filters.end
                ? [dayjs(filters.start, 'YYYY-MM-DD'), dayjs(filters.end, 'YYYY-MM-DD')]
                : null,
            etat: filters.etat,
          }}
          onValuesChange={(_, allValues) => {
            setFilters({
              ...
              ouverte: allValues.ouverte === undefined ? null : allValues.ouverte,
              start: allValues.dateRange ? allValues.dateRange[0].format('YYYY-MM-DD') : null,
              end: allValues.dateRange ? allValues.dateRange[1].format('YYYY-MM-DD') : null,
        ...
        >
          ...
          <Form.Item label="UP" name="upId">
            ...
            >
              {(upsOptions as Record<string, unknown>[]).map((u) => (
          ...
          <Form.Item label="Département" name="deptId">
            ...
            >
              {(deptsOptions as Record<string, unknown>[]).map((d) => (
