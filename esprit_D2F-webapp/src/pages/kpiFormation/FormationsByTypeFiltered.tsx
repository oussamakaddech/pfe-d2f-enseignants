...
import { Select, DatePicker, Switch, Input, Button, Spin, Form, Drawer } from 'antd';
import { FilterOutlined, HomeOutlined, GlobalOutlined, DesktopOutlined } from '@ant-design/icons';
...
export default function FormationsByTypeFiltered() {
  ...
  const onFormChange = (
    ...
    const av = allValues;
    const dateRange = Array.isArray(av.dateRange)
      ? (av.dateRange as [{ format: (f: string) => string }, { format: (f: string) => string }])
      : null;
    setFilters({
            ...
            {TYPE_CONFIG.map(({ key, label, sub, icon, accent, accentBg }) => (
              ...
              >
                ...
                <div className="kpi-type-meta">
                  ...
                  <div className="kpi-type-value">
                    {String(dataByType[key as keyof FormationsByType] ?? 0)}
                  </div>
