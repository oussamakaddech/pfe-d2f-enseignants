...
function buildGenericTitleParts(
  filters: CardFilters,
  ...
  pushFilterPart(parts, upId, 'UP', (v) => {
    const i = upsOptions.find((u) => u.id === v);
    return i ? (i.libelle ?? '') : String(v);
  });
  pushFilterPart(parts, deptId, 'Dépt', (v) => {
    const i = deptsOptions.find((d) => d.id === v);
    return i ? (i.libelle ?? '') : String(v);
  });
...
const MetricCards = () => {
  ...
  const buildTitle = (filters: CardFilters): string => {
    const { domaine, upId, deptId, ouverte, start, end, etat } = filters;
    const onlyOuverte =
      ouverte === true && !domaine && upId === null && deptId === null && !start && !end && !etat;
    if (onlyOuverte) return 'Formations transversales (ouvertes)';
    const onlyPeriod =
      start && end && !domaine && upId === null && deptId === null && ouverte === null && !etat;
  ...
  const openSettings = (id: unknown) =>
    setCards((prev) => prev.map((c) => (c.id === id ? { ...c, visible: true } : c)));
  const closeSettings = (id: unknown) =>
  ...
  const handleFormSubmit = async (cardId: unknown, values: Record<string, unknown>) => {
    ...
    try {
      ...
      setCards((prev) =>
        prev.map((c) =>
          c.id === cardId ? { ...c, count, totalHeures, filters, title: buildTitle(filters) } : c,
        ),
      );
    } catch {
  ...
  const handleGlobalPeriodChange = async (
