    ...
    <div>
      ...
      <div className="dash-kpi-filter-bar">
        ...
        <Select
          ...
          value={selectedUp}
          onChange={(v) => setSelectedUp(v ?? null)}
          style={{ width: 140 }}
        >
          {ups.map((u) => (
      ...
      <div className="dash-kpi-body">
        {(() => {
          ...
          if (stats.length === 0)
            return (
              <Empty
                description="Aucun enseignant non affecté sur la période"
            ...
            <div style={{ maxHeight: 420, overflowY: 'auto', paddingRight: 4 }}>
              {stats.map((item, idx) => (
                <div key={`${item.mail ?? ''}-${idx}`} className="dash-kpi-person-row">
                  <div className="dash-kpi-person-avatar">{initials(item.nom, item.prenom)}</div>
                  <div className="dash-kpi-person-main">
                    <div className="dash-kpi-person-name">
