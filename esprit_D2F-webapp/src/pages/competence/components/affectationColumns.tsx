...
export const toNiveauEnum = (niveau: unknown): string | null => {
  const raw = String(niveau ?? '')
    .trim()
    .toUpperCase();
  if (!raw) return null;
  if (
    ['N1_DEBUTANT', 'N2_ELEMENTAIRE', 'N3_INTERMEDIAIRE', 'N4_AVANCE', 'N5_EXPERT'].includes(raw)
  ) {
    return raw;
  }
  ...
  return [
    {
      ...
      render: (nom: string, rec) => (
        <Space size={8}>
          <div className="affectation-enseignant-avatar">{nom?.[0]?.toUpperCase() ?? '?'}</div>
          <div>
    ...
    },
    {
      ...
      width: 260,
      sorter: (a, b) => {
        const aNames = Array.from(new Set(a.savoirs.map((s) => s.competenceNom).filter(Boolean)))
          .sort((x, y) => x.localeCompare(y))
          .join(' | ');
        const bNames = Array.from(new Set(b.savoirs.map((s) => s.competenceNom).filter(Boolean)))
          .sort((x, y) => x.localeCompare(y))
          .join(' | ');
