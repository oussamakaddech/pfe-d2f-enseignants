...
interface SearchDomaine {
  id?: string | number;
  code?: string;
  nom?: string;
}
interface SearchCompetence {
  id?: string | number;
  code?: string;
  nom?: string;
  domaineNom?: string;
...
export default function StructureSearchResultsView({
  results,
}: Readonly<{ results: SearchResults }>) {
  const { domaines = [], competences = [], sousCompetences = [], savoirs = [] } = results;
  const hasResults =
    domaines.length || competences.length || sousCompetences.length || savoirs.length;
    ...
    <Space direction="vertical" style={{ width: '100%' }} size="middle">
      {domaines.length > 0 && (
        <Card
          ...
          title={
            <>
              <FolderOpenOutlined /> Domaines ({domaines.length})
            </>
          }
        >
          {domaines.map((d) => (
      ...
      {competences.length > 0 && (
        <Card
          ...
          title={
            <>
              <ApartmentOutlined /> Compétences ({competences.length})
            </>
          }
        >
          <Descriptions column={1} size="small" bordered>
            {competences.map((c) => (
              <Descriptions.Item key={c.id} label={<Tag color="green">{c.code}</Tag>}>
                <strong>{c.nom}</strong>
                {c.domaineNom && (
                  <Typography.Text type="secondary"> — {c.domaineNom}</Typography.Text>
                )}
                {c.description && (
                  <div>
      ...
      {sousCompetences.length > 0 && (
        <Card
          ...
          title={
            <>
              <BulbOutlined /> Compétences filles ({sousCompetences.length})
            </>
          }
        >
          <Descriptions column={1} size="small" bordered>
            {sousCompetences.map((sc) => (
              <Descriptions.Item key={sc.id} label={<Tag color="orange">{sc.code}</Tag>}>
                ...
                {sc.competenceNom && (
                  <Typography.Text type="secondary"> — {sc.competenceNom}</Typography.Text>
                )}
                {sc.description && (
                  <div>
                    <Typography.Text type="secondary">{sc.description}</Typography.Text>
                  </div>
                )}
              </Descriptions.Item>
            ))}
      ...
      {savoirs.length > 0 && (
        <Card
          ...
          title={
            <>
              <BookOutlined /> Savoirs ({savoirs.length})
            </>
          }
        >
          <Descriptions column={1} size="small" bordered>
            {savoirs.map((s) => (
              ...
              >
                <strong>{s.nom}</strong>
                {s.sousCompetenceNom && (
                  <Typography.Text type="secondary"> — {s.sousCompetenceNom}</Typography.Text>
                )}
                {s.competenceNom && (
                  <Typography.Text type="secondary"> — {s.competenceNom}</Typography.Text>
                )}
                {s.description && (
                  <div>
