...
function Badge({ text, type = 'muted' }: Readonly<BadgeProps>) {
  const cls =
    (ACCENT as Record<string, { badgeCls?: string }>)[type]?.badgeCls || 'ctp-badge--muted';
...
export default function SavoirDetailDrawer({
  payload,
  open,
  onClose,
}: Readonly<SavoirDetailDrawerProps>) {
      ...
      <Drawer
        title={
          <div className="ctp-drawer-header">
            <span className="ctp-drawer-code" style={codeStyle}>
              {single.code || '-'}
            </span>
            <Tooltip title={titleText}>
              <span className="ctp-drawer-title">{shortTitle}</span>
            </Tooltip>
          </div>
        }
        placement="right"
        ...
        <div className="ctp-drawer-body">
          <div className="ctp-drawer-section">
            ...
            <div className="ctp-drawer-row">
              ...
              <span className="ctp-drawer-val">
                <Badge
                  text={getTypeLabel(single.type || '')}
                  type={getTypeBadge(single.type || '')}
                />
              </span>
            </div>
            <div className="ctp-drawer-row">
            ...
            <div className="ctp-drawer-row">
              ...
              <span className="ctp-drawer-val">
                {single.competenceNom} ({single.competenceCode || '-'})
              </span>
            </div>
            <div className="ctp-drawer-row">
              ...
              <span className="ctp-drawer-val">
                {single.isDirect ? (
                  <Tag color="blue">Lien direct</Tag>
                ) : (
                  single.sousCompetenceNom || '-'
                )}
              </span>
            </div>
            <div className="ctp-drawer-row">
          ...
          <div className="ctp-drawer-section">
            <Alert
              ...
              message="Assignez des enseignants via le module RICE"
              style={{ fontSize: 12 }}
            />
          ...
          {single.description ? (
            <>
              ...
              <div className="ctp-drawer-section">
                <Paragraph style={{ fontSize: 12, color: '#475569', marginBottom: 0 }}>
                  {single.description}
                </Paragraph>
              </div>
            </>
    ...
    <Drawer
      title={
        <div className="ctp-drawer-header">
          ...
          <span className="ctp-drawer-title">{payload?.title || 'Detail savoirs'}</span>
        </div>
      }
      placement="right"
      width={460}
