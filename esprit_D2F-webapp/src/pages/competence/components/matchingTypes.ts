  ...
  switch (action.type) {
    ...
    case 'ASSIGN_SAVOIR': {
      ...
      const pending = {
        ...
        remove: [...state.pendingChanges.remove],
      };
      const removeIdx = pending.remove.findIndex(
        (p) => String(p.savoirId) === sId && String(p.enseignantId) === eId,
      );
      if (removeIdx === -1) {
        const exists = pending.add.some(
          (p) => String(p.savoirId) === sId && String(p.enseignantId) === eId,
        );
    ...
    case 'UNASSIGN_SAVOIR': {
      ...
      const addIdx = pending.add.findIndex(
        (p) => String(p.savoirId) === sId && String(p.enseignantId) === eId,
      );
      if (addIdx === -1) {
        const exists = pending.remove.some(
          (p) => String(p.savoirId) === sId && String(p.enseignantId) === eId,
        );
