...
function collectSavoirs(c: {
  savoirs?: { enseignantsSuggeres?: unknown[] }[];
  sousCompetences?: { savoirs?: { enseignantsSuggeres?: unknown[] }[] }[];
...
function pushScOpts(
  ...
  d: DomaineNode,
  di: number,
  c: { code?: string; sousCompetences?: { code?: string }[] },
  ci: number,
): void {
  opts.push({ label: `${d.code ?? ''} · ${c.code ?? ''}`, value: JSON.stringify([di, ci, -1]) });
  for (const [sci, sc] of (c.sousCompetences ?? []).entries())
    opts.push({
      label: `${d.code ?? ''} · ${c.code ?? ''} · ${sc.code ?? ''}`,
      value: JSON.stringify([di, ci, sci]),
    });
}
...
export default function ReviewStep({
  ...
  const coverage = allSavoirsFlat.length
    ? Math.round((assignedSavoirs * 100) / allSavoirsFlat.length)
    : 0;
  ...
  const exportCsv = () => {
    ...
    const rows = flattenTree(tree).map((s) => [
      s.code,
      s.nom,
      ((s.enseignantsSuggeres as unknown[]) ?? []).join(';'),
    ]);
    const csv = [header, ...rows].map((r) => r.map(esc).join(',')).join('\n');
    const a = Object.assign(document.createElement('a'), {
      href: URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' })),
      download: 'rice_review.csv',
    });
    a.click();
  ...
  const scOptions = useMemo(() => {
    ...
    for (const [di, d] of (tree ?? []).entries())
      for (const [ci, c] of (d.competences ?? []).entries()) pushScOpts(opts, d, di, c, ci);
    if (selectedNode?.type !== 'savoir') return opts;
    const [srcDi, srcCi, srcSci] = selectedNode.path;
    return opts.filter((o) => {
      try {
        const arr = JSON.parse(o.value) as number[];
    ...
    <div className="review-layout">
      ...
      <div className="review-tree-col">
        {orphanIds.size > 0 && (
          <Alert
            ...
            action={
              <Button size="small" onClick={cleanupOrphans}>
                Nettoyer
              </Button>
            }
          />
      ...
      <div className="review-props-panel">
        <NodePropsPanel
          ...
          propsForm={propsForm}
          mergedEnseignants={mergedEnseignants}
          scOptions={scOptions}
          handleUpdateField={handleUpdateField}
          handleRemoveEnseignant={(id) => {
            const cur = (selectedNode?.data?.enseignantsSuggeres as unknown[]) ?? [];
            handleUpdateField(
              'enseignantsSuggeres',
      ...
      <div className="review-footer">
        ...
        <Space>
          ...
          <Button icon={<CopyOutlined />} onClick={copyResultText}>
            Copier texte
          </Button>
