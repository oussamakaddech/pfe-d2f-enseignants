...
export default function EnseignantCompTable({
  rows,
  setCurrentStep,
}: Readonly<EnseignantCompTableProps>) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, row: EnseignantRow) => (
  ...
  return (
    <Card
      title={
        <Space>
          <UserOutlined /> Enseignants — Compétences techniques associées
        </Space>
      }
      size="small"
...
export function buildEnseignantSavoirsMap(allSavoirsFlat: Record<string, unknown>[]) {
  ...
  allSavoirsFlat.forEach((s) => {
    ((s.enseignantsSuggeres as unknown[]) ?? []).forEach((eid) => {
      ...
      const gcCodes =
        s.gcCodes && (s.gcCodes as string[]).length > 0
          ? (s.gcCodes as string[])
          : [shortenCode(s.code as string)];
      ensSavoirsMap
        .get(key)!
        .push({ nom: s.nom as string, code: s.code as string, gcCodes, label: s.label as string });
    });
  });
...
export function buildEnseignantRows(
  ...
  return Array.from(ensSavoirsMap.entries()).map(([eid, savoirs]) => {
    ...
    if (ensObj)
      resolvedName = ensObj.prenom ? `${ensObj.prenom} ${ensObj.nom}` : (ensObj.nom as string);
