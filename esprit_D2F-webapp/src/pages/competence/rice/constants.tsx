...
export const TYPE_ICON: Record<string, React.ReactNode> = {
  THEORIQUE: <BookOutlined />,
  PRATIQUE: <ExperimentOutlined />,
};
...
export function useDepartmentConfig(deptCode?: string) {
  return (
    DepartmentConfig[deptCode?.toLowerCase()?.trim() as keyof typeof DepartmentConfig] ??
    DepartmentConfig.gc
  );
}
...
export const DEPARTMENT_OPTIONS = Object.values(DepartmentConfig).map((cfg) => ({
  ...
  label: (
    <span>
      ...
      {cfg.nom}
      <span style={{ marginLeft: 8, color: '#8c8c8c', fontSize: 11 }}>[{cfg.campus}]</span>
    </span>
  ),
  labelText: cfg.nom,
}));
