...
import { buildExportFileName, buildMatrixRows } from '@/utils/helpers/consultationUtils';
...
const niveauMeta = (niveau: string | undefined) =>
  NIVEAU_SAVOIR_OPTIONS.find((n) => n.value === niveau) ?? {
    label: niveau ?? '-',
    color: 'default',
  };
...
export default function CompetencePage() {
  ...
  const { activeTab, handleTabChange } = useCompetencePageState({
    crud,
    loadStructure: () => void structure.loadStructure(),
  });
  ...
  const handleAddPrerequisite = useCallback(async () => {
    ...
    try {
      ...
      const prerequisiteId =
        typeof values.prerequisiteId === 'string'
      ...
      await Promise.all([loadPrerequisites(prerequisiteTarget.id), loadCompetences()]);
    } catch (err: unknown) {
  ...
  const handleUpdatePrerequisiteNiveau = useCallback(
    async (id: Id, niveauMinimum: string) => {
      if (!prerequisiteTarget?.id) return;
      try {
    ...
    () => [
      ...crud.compColumns,
      {
        ...
        render: (_, record) => {
          ...
          const manual =
            (record.prerequisiteManual || '').trim() ||
            extractLegacyPrerequisiteManual(record.description);
          ...
          return (
            <Space size={4} wrap>
              ...
              {!names.length && !!manual && <Tag color="orange">{manual}</Tag>}
            </Space>
          );
        },
      },
  ...
  const renderExpandedCompRow = useCallback(
    (record: Competence) => (
      <CompetenceExpandedRow
        ...
        onAddRoot={(parent: Competence) =>
          crud.openScModal(
            scForm,
            { type: 'competence', id: parent.id, nom: parent.nom, code: parent.code },
            null,
          )
        }
  ...
  return (
    <div className="competence-page">
      ...
      >
        ...
        <Form form={addPrerequisiteForm} layout="inline" onFinish={handleAddPrerequisite}>
          <Form.Item
            name="prerequisiteId"
          ...
          >
            <Select style={{ width: 180 }} placeholder="Niveau minimum" options={NIVEAU_OPTIONS} />
          </Form.Item>
