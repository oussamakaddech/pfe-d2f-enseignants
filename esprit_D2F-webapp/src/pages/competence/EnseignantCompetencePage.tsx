...
function niveauTag(niveau: string | undefined) {
  ...
  return found ? <Tag color={found.color}>{found.label}</Tag> : <Tag>{niveau}</Tag>;
}
...
export default function EnseignantCompetencePage() {
  ...
  const loadCompetences = useCallback(async () => {
    ...
    try {
      let data;
      if (filterDomaine) {
        data = await ecApi.getByEnseignantAndDomaine(enseignantId, filterDomaine);
      } else if (filterNiveau) {
        data = await ecApi.getByEnseignantAndNiveau(enseignantId, filterNiveau);
      } else {
