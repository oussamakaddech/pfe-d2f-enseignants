...
export function usePresenceList(seanceId: number | string) {
  ...
  const [originalById, setOriginalById] = useState<
    Record<number, { presence: boolean; commentaire: string }>
  >({});
  ...
  useEffect(() => {
    ...
    const normalized = list.map((p) => {
      ...
      return {
        ...r,
        presence: typeof r.present === 'boolean' ? r.present : !!r.presence,
      } as PresenceRecord;
    });
  ...
  const setCommentaire = (idParticipation: number, value: string) => {
    setPresences((prev) =>
      prev.map((p) => (p.idParticipation === idParticipation ? { ...p, commentaire: value } : p)),
    );
  };
  ...
  const handleMarkAll = async (present: boolean) => {
    ...
    try {
      ...
      msgApi.success(
        present ? 'Tous les enseignants marqués présents' : 'Tous les enseignants marqués absents',
      );
    } catch (err: unknown) {
      msgApi.error(
        (err as { response?: { data?: { error?: string } } })?.response?.data?.error ||
      ...
      [
        {
          name: 'Présences',
          rows,
          title: 'Feuille de Présence — Esprit',
