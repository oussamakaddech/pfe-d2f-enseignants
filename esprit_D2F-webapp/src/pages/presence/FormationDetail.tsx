...
const FormationDetail = () => {
  ...
  const presencesBySeance = useMemo(() => {
    const byId: Record<
      string,
      { presence: boolean; enseignant?: { mail?: string; id?: unknown } }[]
    > = {};
    seanceIds.forEach((sid: number, idx) => {
      byId[sid] = ((aggResults[idx] as { present?: boolean; presence?: boolean }[]) || []).map(
        (p) => ({
          ...p,
  ...
  const allParticipants = useMemo(() => {
    ...
    const map = new Map<
      string,
      { mail?: string; id?: unknown; seancesInscrites: number } & Record<string, unknown>
    >();
  ...
  function computeEnsStats(ens: Record<string, unknown>) {
    ...
    const taux =
      totalSeancesAvecPresence === 0 ? 0 : Math.round((presentes * 100) / totalSeancesAvecPresence);
    return { ...ens, presentes, totalSeancesAvecPresence, taux };
  }

  const participantsWithStats = useMemo(
    () =>
      allParticipants.map((ens) => computeEnsStats(ens)) as Parameters<
        typeof EnseignantsTab
      >[0]['participantsWithStats'],
        ...
        <div style={{ paddingTop: 8 }}>
          {formation.seances && formation.seances.length > 0 ? (
            formation.seances.map((s) => (
              <SeanceCard key={String(s.idSeance)} seance={s as SeanceCardData} />
            ))
          ) : (
