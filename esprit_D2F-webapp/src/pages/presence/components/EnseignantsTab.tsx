...
export function EnseignantsTab({
  ...
  const columns = [
    ...
    },
    {
      ...
      sorter: (a: EnseignantWithStats, b: EnseignantWithStats) =>
        a.seancesInscrites - b.seancesInscrites,
    },
    {
      title: 'Présent',
      dataIndex: 'presentes',
      ...
      render: (n: number, r: EnseignantWithStats) => (
        <Tooltip
          title={`${n} séance(s) présent / ${r.totalSeancesAvecPresence} séance(s) jouée(s)`}
        >
    ...
    },
    {
      ...
      render: (t: number) => {
        ...
        return (
          <Progress percent={t} size="small" strokeColor={strokeColor} format={(p) => `${p}%`} />
        );
      },
    ...
    <div style={{ paddingTop: 8 }}>
      <div className="fd-tab-toolbar">
        <Text type="secondary">
          ...
          {participantsWithStats.length} enseignant{participantsWithStats.length > 1 ? 's' : ''}{' '}
