...
export default function AlertsTrendChart({
  ...
  const totalPath = data
    .map((d, i) => `${i === 0 ? 'M' : 'L'} ${xOf(i).toFixed(1)} ${yOf(d.total).toFixed(1)}`)
    .join(' ');
  const critPath = data
    .map((d, i) => `${i === 0 ? 'M' : 'L'} ${xOf(i).toFixed(1)} ${yOf(d.critiques).toFixed(1)}`)
    .join(' ');
  ...
  return (
    <div className="rd-chart">
      ...
      >
        ...
        <path
          d={`${totalPath} L ${xOf(data.length - 1)} ${H - padY} L ${padX} ${H - padY} Z`}
          fill="url(#rd-alert-area)"
        />
      ...
      {hover != null && (
        <div className="rd-chart-tip" style={{ left: `${(xOf(hover) / W) * 100}%`, top: 4 }}>
          ...
          <div className="t-row">
            <span className="t-k">
              <span className="rd-legend-swatch" style={{ background: '#3b82f6' }} /> Total
            </span>
