        ...
        {byType.map((a) => {
          ...
          return (
            <div key={a.key} className="rd-palert-item" style={{ borderLeftColor: meta.color }}>
              <span className="rd-palert-icon" style={{ color: meta.color }}>
                {meta.icon}
              </span>
        ...
        {summary.top_competences.length > 0 && (
          <div className="rd-palerts-col">
            ...
            {summary.top_competences.slice(0, 4).map((c) => (
              <span key={c.competence_id} className="rd-palert-chip">
                {c.competence_nom ?? `Comp. #${c.competence_id}`}
              </span>
            ))}
          </div>
        )}
        {summary.top_departements.length > 0 && (
          <div className="rd-palerts-col">
            ...
            {summary.top_departements.slice(0, 4).map((d) => (
              <span key={d.departement_id} className="rd-palert-chip">
                {d.departement_nom ?? d.departement_id}
              </span>
            ))}
          </div>
        )}
