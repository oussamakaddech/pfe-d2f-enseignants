...
export default function DecliningCompetencies({
  ...
  if (items.length === 0)
    return <div className="rd-empty">Aucune compétence en déclin détectée</div>;

  const maxDecline = Math.max(
    0.5,
    ...items.map((c) => Math.abs(Number(pick(c, 'delta', 'delta')) || 0)),
  );
          ...
          <div key={String(pick(c, 'competence_id', 'competency_id'))} className="rd-declin-row">
            ...
            <div className="rd-declin-metrics">
              ...
              {!Number.isNaN(actuel) && !Number.isNaN(ancien) && (
                <span className="rd-declin-levels">
                  {ancien.toFixed(1)} → {actuel.toFixed(1)}
                </span>
              )}
            </div>
          </div>
