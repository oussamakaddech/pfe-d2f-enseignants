...
describe("PriorityMatrix", () => {
  ...
  it("affiche le label de l'item dans la tooltip", () => {
    const { container } = render(<PriorityMatrix items={[{ id: 1, label: "MonItem", urgency: 4, impact: 4 }]} />);
  ...
  it("utilise la couleur personnalisée fournie", () => {
    const { container } = render(<PriorityMatrix items={[{ id: 1, label: "X", urgency: 4, impact: 4, color: "#123456" }]} />);
  ...
  it("affiche la tooltip au survol et l'enlève au départ", () => {
    ...
    const group = container.querySelector("circle")?.closest("g") as SVGGElement;
    fireEvent.mouseEnter(group);
    expect(screen.getByText((c) => c.includes("U:Haute") && c.includes("I:Important"))).toBeInTheDocument();
  ...
  it("affiche des libellés d'urgence/impact faibles quand <=2", () => {
    const { container } = render(<PriorityMatrix items={[{ id: 1, label: "Low", urgency: 1, impact: 1 }]} />);
    const group = container.querySelector("circle")?.closest("g") as SVGGElement;
    fireEvent.mouseEnter(group);
  ...
  it("n'affiche pas de count quand non fourni", () => {
    const { container } = render(<PriorityMatrix items={[{ id: 1, label: "NoCount", urgency: 3, impact: 3 }]} />);
