...
export default function SupplyDemandMatrix({
  ...
  readonly items: SupplyDemandItem[];
  readonly loading: boolean;
}) {
  if (loading && items.length === 0) return <ChartSkeleton height={220} />;
  if (items.length === 0)
    return <div className="rd-empty">Aucune pression compétence détectée</div>;
      ...
      {sorted.map((it) => {
        ...
        return (
          <div
            key={it.competenceId}
            className="rd-sd-row"
            style={{ '--sd-accent': urgColor } as CSSProperties}
          >
            <div>
              <div className="rd-sd-name">{it.competenceName}</div>
              <div className="rd-sd-domain">
                {it.domain} · {it.impactedTeachers} enseignants
              </div>
            </div>
            <div className="rd-sd-num" style={{ color: 'var(--rd-text-2)' }}>
              {it.demandPct} %
            </div>
            <div className="rd-sd-urg">
              <div className="rd-bar">
                <span
                  style={{
                    width: `${(it.impactedTeachers / maxImpact) * 100}%`,
                    background: urgColor,
                  }}
                />
              </div>
              <span className="rd-sd-urg-label" style={{ color: urgColor }}>
                {urgLabel}
              </span>
            </div>
            <span className="rd-quad" style={{ background: qc }}>
              {QUAD_LABELS[it.quadrant]}
            </span>
          </div>
        );
      })}
