...
export default function ForecastChart({
  ...
  readonly view: ForecastView | null;
  ...
  if (!view || view.series.length === 0)
    return <div className="rd-empty">Aucune prévision disponible</div>;
  ...
  const histPath = history
    .map((p, i) => `${i === 0 ? 'M' : 'L'} ${xOf(i)} ${yOf(p.value).toFixed(1)}`)
    .join(' ');
  const areaPath =
    history.length > 1
  ...
  const projPts = [history.at(-1), ...projection].filter(
    (p): p is (typeof projection)[number] => p != null,
  );
  const projPath = projPts
    .map((p, i) => `${i === 0 ? 'M' : 'L'} ${xOf(projStart + i)} ${yOf(p.value).toFixed(1)}`)
    .join(' ');

  const bandTop = projection
    .map(
      (p, i) =>
        `${i === 0 ? 'M' : 'L'} ${xOf(history.length + i)} ${yOf(p.upper ?? p.value).toFixed(1)}`,
    )
    .join(' ');
  const bandBottom = [...projection]
    .reverse()
    .map((p, idx) => {
    ...
    <div className="rd-chart">
      ...
      >
        ...
        {series.map((p, i) => (
          <circle
            ...
            cx={xOf(i)}
            cy={yOf(p.value)}
            r={p.isProjection ? 3 : 3.5}
            fill={p.isProjection ? '#7c3aed' : '#b51200'}
          />
        ))}
        {hover != null && (
          <>
            ...
            <circle
              cx={xOf(hover)}
              cy={yOf(series[hover].value)}
              r={5}
              fill={series[hover].isProjection ? '#7c3aed' : '#b51200'}
              stroke="#fff"
              strokeWidth={2}
            />
          </>
        ...
        {series.map((p, i) => (
          <text
            ...
            fill="var(--rd-text-3)"
            style={{
              display: i % labelEvery === 0 || i === series.length - 1 ? undefined : 'none',
            }}
          >
            {p.period}
          </text>
      ...
      {hover != null && (
        <div className="rd-chart-tip" style={{ left: `${(xOf(hover) / W) * 100}%`, top: 4 }}>
          <div className="t-date">
            {series[hover].period}
            {series[hover].isProjection ? ' · projection' : ''}
          </div>
          <div className="t-row">
            <span className="t-k">Valeur</span>
            <span className="t-v">{Math.round(series[hover].value)}</span>
          </div>
          {series[hover].isProjection &&
            (series[hover].lower != null || series[hover].upper != null) && (
              <div className="t-row">
                <span className="t-k">Intervalle</span>
                <span className="t-v">
                  {Math.round(series[hover].lower ?? series[hover].value)}–
                  {Math.round(series[hover].upper ?? series[hover].value)}
                </span>
              </div>
            )}
        </div>
      ...
      <div className="rd-chart-legend">
        <span className="rd-legend-item">
          <span className="rd-legend-swatch" style={{ background: '#b51200' }} /> Historique
        </span>
        <span className="rd-legend-item">
          <span className="rd-legend-swatch" style={{ background: '#7c3aed' }} /> Projection
          {projection.length > 0 ? ` (${projection.length} mois)` : ''}
        </span>
        <span className="rd-legend-item">
          <span className="rd-legend-swatch" style={{ background: 'rgba(124,58,237,0.25)' }} />{' '}
          Intervalle de confiance
        </span>
      </div>
      {view.note && (
        <div className="rd-muted" style={{ fontSize: 11.5, marginTop: 6 }}>
          {view.note}
        </div>
      )}
    </div>
