...
describe("SkillHeatmap", () => {
  it("affiche le skeleton en loading sans données", () => {
    const { container } = render(<SkillHeatmap cells={[]} loading selected={null} onCellClick={vi.fn()} />);
  ...
  it("déclenche onCellClick au clic sur une cellule", () => {
    ...
    render(<SkillHeatmap cells={cells} loading={false} selected={null} onCellClick={onCellClick} />);
  ...
  it("marque la cellule sélectionnée", () => {
    render(<SkillHeatmap cells={cells} loading={false} selected={{ department: "Info", competenceId: 1 }} onCellClick={vi.fn()} />);
