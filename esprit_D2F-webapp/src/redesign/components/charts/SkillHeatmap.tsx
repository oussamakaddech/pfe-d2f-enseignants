...
export default function SkillHeatmap({
  ...
  return (
    <div className="rd-heatmap" style={{ gridTemplateColumns: `130px repeat(${comps.length}, minmax(46px, 1fr))` }}>
      <div className="rd-heat-corner" />
      {comps.map((c) => (
        <div key={c} className="rd-heat-head" title={c}>{c}</div>
      ))}
      {depts.map((d) => (
        <Row key={d} dept={d} comps={comps} get={get} selected={selected} onCellClick={onCellClick} />
      ))}
      <div />
...
function Row({
  ...
  return (
    <>
      <div className="rd-heat-head" style={{ justifyContent: "flex-start", color: "var(--rd-text-2)", fontWeight: 700 }} title={dept}>{dept}</div>
      {comps.map((c) => {
