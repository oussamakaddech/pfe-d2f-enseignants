...
const W = 600, H = 244, padX = 38, padY = 26;
const innerW = W - padX * 2, innerH = H - padY * 2;

function buildPath(values: number[], w: number, h: number, max: number, padX: number, padY: number): string {
...
export default function TrendChart({ data, loading }: { data: RiskTrendPoint[]; loading: boolean }) {
  ...
  const criticalPath = buildPath(data.map((d) => d.critical), W, H, max, padX, padY);
  const elevatedPath = buildPath(data.map((d) => d.elevated), W, H, max, padX, padY);
    ...
    <div className="rd-chart">
      ...
      >
        ...
        {hover != null && (
          <>
            ...
            <circle cx={xOf(hover)} cy={yOf(data[hover].critical)} r={5} fill="#ef4444" stroke="#fff" strokeWidth={2} />
            <circle cx={xOf(hover)} cy={yOf(data[hover].elevated)} r={5} fill="#f59e0b" stroke="#fff" strokeWidth={2} />
          </>
        )}
        {data.map((d, i) => (
      ...
      {hover != null && (
        <div className="rd-chart-tip" style={{ left: `${(xOf(hover) / W) * 100}%`, top: 4 }}>
          ...
          <div className="t-row">
            <span className="t-k"><span className="rd-legend-swatch" style={{ background: "#ef4444" }} /> Critique</span>
            <span className="t-v">{data[hover].critical}</span>
          </div>
          <div className="t-row">
            <span className="t-k"><span className="rd-legend-swatch" style={{ background: "#f59e0b" }} /> Élevé</span>
      ...
      <div className="rd-chart-legend">
        <span className="rd-legend-item"><span className="rd-legend-swatch" style={{ background: "#ef4444" }} /> Critique</span>
        <span className="rd-legend-item"><span className="rd-legend-swatch" style={{ background: "#f59e0b" }} /> Élevé</span>
      </div>
    </div>
  );
}
