...
const perf: ModelPerformance = { gap_model_accuracy: 0.82, recommendation_avg_proba: 0.7, last_retrained: "2024-01-15T10:00:00Z", last_retrain_status: "OK" };
...
describe("ModelHealthCard", () => {
  ...
  it("affiche une dérive détectée", () => {
    render(<ModelHealthCard modelPerf={perf} drift={{ drift_detected: true, message: "Dérive!" }} loading={false} />);
  ...
  it("affiche un avertissement de précision faible", () => {
    render(<ModelHealthCard modelPerf={{ ...perf, gap_model_accuracy: 0.3 }} drift={drift} loading={false} />);
    expect(screen.getByText("Précision faible")).toBeInTheDocument();
  });
  it("applique la pénalité d'entraînement absent", () => {
    const { container } = render(<ModelHealthCard modelPerf={{ ...perf, last_retrained: null }} drift={drift} loading={false} />);
