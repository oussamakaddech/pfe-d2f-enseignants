          ...
          <div key={r.formationId} className="rd-reco-item">
            ...
            <div className="rd-reco-body">
              ...
              <div className="rd-reco-meta">
                <span>{r.recommendationCount} recommandation{r.recommendationCount > 1 ? "s" : ""}</span>
                {r.avgScore != null && <span> · Score {r.avgScore.toFixed(1)}</span>}
              </div>
              <div className="rd-bar" style={{ marginTop: 6 }}>
                <span style={{ width: `${barWidth}%`, background: "linear-gradient(90deg, color-mix(in srgb, var(--rd-accent) 75%, #fff), var(--rd-accent))" }} />
              </div>
            </div>
            <div className="rd-reco-success-ring">
              <svg width="44" height="44" viewBox="0 0 44 44" role="img" aria-label={`Réussite ${successPct} %`}>
                ...
                />
                <text x="22" y="25.5" textAnchor="middle" fontSize="12.5" fontWeight="800" fill={ringColor}>{successPct}%</text>
              </svg>
