...
const data = {
  forecast_kpis: { horizon_mois: 6, nb_enseignants: 100, niveau_projet_moyen: 3.2, pct_objectifs_atteignables: 75, nb_competences_regression: 4, nb_competences_suivies: 10 },
  benchmark_departements: [{ departement_id: "Info", niveau_moyen: 3.1, ecart_vs_cohorte: 0.5, nb_enseignants: 28, position: "AU_DESSUS" }],
  anomalies_live: { nb_anomalies_recentes: 5, fenetre_jours: 30, nb_nouvelles: 2, alertes: [{ severite: "CRITICAL", titre: "Alerte 1", message: "msg" }] },
  correlation_besoins_gaps: { coefficient_pearson: 0.82, nb_competences: 12, top_paires: [], interpretation: "forte" },
};

vi.mock("@/hooks/analyse/usePilotageDashboard", () => ({
  usePilotageDashboard: vi.fn(() => ({ data: undefined, isLoading: false, isError: false, refetch: vi.fn() })),
}));

import { usePilotageDashboard } from "@/hooks/analyse/usePilotageDashboard";

beforeEach(() => {
  (usePilotageDashboard as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ data, isLoading: false, isError: false, refetch: vi.fn() });
});
...
describe("PilotageSummary", () => {
  ...
  it("affiche le skeleton en loading", () => {
    (usePilotageDashboard as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ data: undefined, isLoading: true, isError: false, refetch: vi.fn() });
    const { container } = renderSummary();
