...
import type {
  GroupedRecommendationsResponse, RecommendationGroup, WhatIfResponse, Recommendation,
} from "@/models/analyse";

vi.mock("@/hooks/analyse/useAnalytics", async () => {
  const React = await vi.importActual<typeof import("react")>("react");
  interface SimConfig { result?: unknown; isPending?: boolean; isError?: boolean }
  const useGroupedRecommendations = vi.fn(() => ({ data: { total: 0, groups: [] }, isLoading: false, isError: false }));
  const simConfig: { value: SimConfig } = { value: { result: null, isPending: false, isError: false } };
  const useSimulateWhatIf = vi.fn(() => {
    const [state, setState] = React.useState<{ data: unknown; isError: boolean; isPending: boolean }>({ data: null, isError: simConfig.value.isError ?? false, isPending: simConfig.value.isPending ?? false });
...
function setSimResult(result: unknown, opts: { isError?: boolean; isPending?: boolean } = {}) {
  (useSimulateWhatIf as unknown as { __config: { value: { result?: unknown; isError?: boolean; isPending?: boolean } } }).__config.value = {
    result, isError: opts.isError ?? false, isPending: opts.isPending ?? false,
  };
}

const groupReco = (over: Partial<Recommendation> = {}): Recommendation => ({
  id: 1, formation_id: 1, formation_titre: "Python avancé", formation_type: "WORKSHOP",
  competence_id: 1, competence_nom: "Python", score_global: 0.8, score_pertinence: 0.7,
  score_reussite: 0.6, score_disponibilite: 0.5, probabilite_reussite: 0.7,
  facteurs_score: { pairs: 0.4 }, rang_dans_parcours: 1, justification: null, statut: "PROPOSEE",
  ...over,
});

const group = (over: Partial<RecommendationGroup> = {}): RecommendationGroup => ({
  group_key: "k1", group_label: "Python", nb: 2, score_moyen: 0.6, score_max: 0.8,
  nb_acceptees: 0, items: [groupReco(), groupReco({ id: 2, formation_titre: "SQL base" })],
  ...over,
});

const groupedResponse = (over: Partial<GroupedRecommendationsResponse> = {}): GroupedRecommendationsResponse => ({
  enseignant_id: "ENS-1", group_by: "competence", total: 1, groups: [group()], ...over,
});

const whatIfResponse = (over: Partial<WhatIfResponse> = {}): WhatIfResponse => ({
  enseignant_id: "ENS-1", horizon_mois: 6,
  risk_before: { score: 0.8, niveau: "ELEVE" }, risk_after: { score: 0.4, niveau: "MODERE" },
  risk_reduction: 0.5, nb_gaps_before: 10, nb_gaps_after: 5, nb_gaps_resolus: 5,
  details: [
    { competence_id: 1, niveau_actuel: 2, niveau_requis: 4, niveau_vise: 4, gap_avant: 2, gap_apres: 0, urgence_apres: "FAIBLE", resolu: true },
    { competence_id: 2, niveau_actuel: 1, niveau_requis: 4, niveau_vise: 3, gap_avant: 3, gap_apres: 1, urgence_apres: "MODERE", resolu: false },
  ],
  ...over,
});
...
describe("RecommandationsPlus", () => {
  ...
  it("affiche le skeleton en chargement du regroupement", () => {
    (useGroupedRecommendations as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ data: undefined, isLoading: true, isError: false });
    renderReco("ENS-1");
    expect(document.querySelector(".rd-skel-block")).toBeInTheDocument();
  });

  it("affiche un message d'erreur si le regroupement échoue", () => {
    (useGroupedRecommendations as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ data: undefined, isLoading: false, isError: true });
    renderReco("ENS-1");
  ...
  it("rend les groupes et leurs recommandations", () => {
    (useGroupedRecommendations as unknown as ReturnType<typeof vi.fn>).mockReturnValue({ data: groupedResponse(), isLoading: false, isError: false });
    renderReco("ENS-1");
  ...
  it("lance la simulation et affiche le résultat", async () => {
    ...
    expect(within(screen.getByText("C1").closest(".rd-reco-row") as HTMLElement).getByText("Oui")).toBeInTheDocument();
  });
