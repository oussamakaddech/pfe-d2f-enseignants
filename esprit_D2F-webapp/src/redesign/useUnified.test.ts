...
import {
  useUnifiedRiskTeachers,
  useUnifiedRiskDistribution,
  useUnifiedRiskTrend,
  useUnifiedSupplyDemand,
  useUnifiedHeatmap,
  useUnifiedPriorityActions,
  useUnifiedAlerts,
  useUnifiedForecast,
  useUnifiedOverview,
  useUnifiedDashboard,
  selectCoverage,
  selectTrainingEffectiveness,
  selectFormationRecos,
  selectDeclining,
  selectInDemand,
  selectRiskTeachersFromDashboard,
        ...
        teacher_risk_indicators: [
          {
            ...
            teacher_name: 'A',
            score_risque: 0.8,
            disengagement_signals: ['stagnation'],
          },
...
describe('useUnifiedRiskDistribution', () => {
  it('mappe le DTO backend', () => {
    ap.useRiskDistribution.mockReturnValue({
      data: {
        total: 100,
        by_level: { CRITIQUE: 5, ELEVE: 10, MODERE: 20, FAIBLE: 65 },
        by_department: [{ departement: 'Info', score_risque_moyen: 0.4, nb_enseignants: 12 }],
      },
      isLoading: false,
    });
      ...
      data: [
        {
          ...
          teacher_name: 'A',
          score_risque: 0.9,
          competence_prioritaire: { competence_nom: 'Python' },
          nb_gaps_critiques: 2,
          action_recommandee: 'Former',
          nb_alertes_ouvertes: 1,
        },
      ],
      isLoading: false,
    });
...
describe('useUnifiedOverview / useUnifiedDashboard', () => {
  ...
  it('expose le dashboard', () => {
    (useDashboard as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
      dashboard: { taux_couverture_departements: [] },
      loading: false,
      error: null,
      lastUpdate: '2024',
      refetch: vi.fn(),
...
describe('selectTrainingEffectiveness / selectFormationRecos / selectDeclining / selectInDemand', () => {
  it("mappe l'efficacité formation", () => {
    const r = selectTrainingEffectiveness({
      training_effectiveness: [
        { formation_id: 1, formation_titre: 'F', completion_rate: 0.5, avg_level_gain: 1.2 },
      ],
    } as never);
  ...
  it('mappe les compétences en déclin', () => {
    const r = selectDeclining({
      competences_en_declin: [
        { competence_id: 1, competence_nom: 'C', domaine_nom: 'D', delta: -0.5 },
      ],
    } as never);
    expect(r[0].delta).toBe(-0.5);
  });
  it('mappe les compétences en demande', () => {
    const r = selectInDemand({
      competences_en_demande: [
        { competence_id: 2, competence_nom: 'C', domaine_nom: 'D', score_demande: 70, nb_gaps: 3 },
      ],
    } as never);
