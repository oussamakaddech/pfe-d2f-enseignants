...
import {
  ...
  useApprovedBesoins,
  useBesoinsByUp,
  useBesoinsByDepartement,
  useAddBesoin,
  useModifyBesoin,
  useRemoveBesoin,
  useApproveBesoin,
  useBesoinCompetences,
...
vi.mock('@/services/besoin/BesoinFormationService', () => ({
  default: {
    getAllBesoinFormations: vi.fn(),
    getMyBesoins: vi.fn(),
    getApprovedBesoinFormations: vi.fn(),
    getBesoinsByUp: vi.fn(),
    getBesoinsByDepartement: vi.fn(),
    addBesoinFormation: vi.fn(),
    modifyBesoinFormation: vi.fn(),
    removeBesoinFormation: vi.fn(),
    approveBesoin: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useBesoins', () => {
  ...
  it('useApprovedBesoins fetches', async () => {
    (
      BesoinFormationService.getApprovedBesoinFormations as ReturnType<typeof vi.fn>
    ).mockResolvedValue([]);
  ...
  it('useBesoinsByUp fetches when up provided', async () => {
    (BesoinFormationService.getBesoinsByUp as ReturnType<typeof vi.fn>).mockResolvedValue([
      { id: 2 },
    ]);
  ...
  it('useBesoinsByDepartement fetches when dept provided', async () => {
    ...
    const { result } = renderHook(() => useBesoinsByDepartement('d1'), {
  ...
  it('useAddBesoin calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ titre: 't' } as never);
    });
  ...
  it('useModifyBesoin calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ besoin: {}, commentaire: 'c' } as never);
    });
  ...
  it('useReplaceBesoinCompetences calls service', async () => {
    ...
    const { result } = renderHook(() => useReplaceBesoinCompetences(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
      await result.current.mutateAsync({ besoinId: 3, links: [] } as never);
    });
