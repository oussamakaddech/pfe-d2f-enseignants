...
import {
  ...
  useEvaluationGlobaleByFormation,
  useCreateEvaluationGlobale,
  useUpdateEvaluationGlobale,
  useDeleteEvaluationGlobale,
  useEvaluationsEnrichedByFormation,
  useUpdateEvaluationsBulk,
  useUpdateEvaluationsBulkFlat,
...
vi.mock('@/services/evaluation/EvaluationGlobaleService', () => ({
  default: {
    getAllEvaluationGlobales: vi.fn(),
    getEvaluationGlobaleByFormationId: vi.fn(),
    createEvaluationGlobale: vi.fn(),
    updateEvaluationGlobale: vi.fn(),
...
vi.mock('@/services/evaluation/EvaluationFormateurService', () => ({
  default: {
    listEvaluationsEnrichedByFormation: vi.fn(),
    updateEvaluationsBulkByFormation: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useEvaluations', () => {
  ...
  it('useEvaluationsGlobales fetches', async () => {
    (
      EvaluationGlobaleService.getAllEvaluationGlobales as ReturnType<typeof vi.fn>
  ...
  it('useEvaluationGlobaleByFormation fetches', async () => {
    (
      EvaluationGlobaleService.getEvaluationGlobaleByFormationId as ReturnType<typeof vi.fn>
    ).mockResolvedValue({ id: 2 });
    const { result } = renderHook(() => useEvaluationGlobaleByFormation(2), {
  ...
  it('useCreateEvaluationGlobale calls service', async () => {
    (
      EvaluationGlobaleService.createEvaluationGlobale as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
  ...
  it('useUpdateEvaluationGlobale calls service', async () => {
    (
      EvaluationGlobaleService.updateEvaluationGlobale as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
  ...
  it('useDeleteEvaluationGlobale calls service', async () => {
    (
      EvaluationGlobaleService.deleteEvaluationGlobale as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
  ...
  it('useEvaluationsEnrichedByFormation fetches', async () => {
    (
      EvaluationFormateurService.listEvaluationsEnrichedByFormation as ReturnType<typeof vi.fn>
    ).mockResolvedValue([]);
    const { result } = renderHook(() => useEvaluationsEnrichedByFormation(2), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useUpdateEvaluationsBulk calls service', async () => {
    (
      EvaluationFormateurService.updateEvaluationsBulkByFormation as ReturnType<typeof vi.fn>
    ).mockResolvedValue({});
    const { result } = renderHook(() => useUpdateEvaluationsBulk(), { wrapper: createWrapper() });
    await act(async () => {
      await result.current.mutateAsync({ formationId: 1, evaluations: [] } as never);
    });
  ...
  it('useUpdateEvaluationsBulkFlat resolves null without formationId', async () => {
    const { result } = renderHook(() => useUpdateEvaluationsBulkFlat(), {
      wrapper: createWrapper(),
    });
