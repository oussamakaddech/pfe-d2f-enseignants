...
import {
  useDriveHierarchy,
  useFormationHierarchy,
  useDownloadOneDriveFile,
  useDeleteOneDriveFile,
  useGetEmbedLink,
...
describe('useOneDrive', () => {
  ...
  it('useFormationHierarchy disabled without id', async () => {
    const { result } = renderHook(() => useFormationHierarchy(undefined), {
  ...
  it('useGetEmbedLink calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ nomFormation: 'f', nomDocument: 'd' } as never);
    });
