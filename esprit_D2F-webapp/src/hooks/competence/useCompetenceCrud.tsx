...
import {
  ...
  buildCompColumns,
  buildSavoirColumns,
} from '@/components/competence/columns/CompetenceColumns';
...
interface DomaineMutVars {
  id?: Id;
  payload: Partial<Domaine>;
}
interface CompMutVars {
  id?: Id;
  domaineId?: Id;
  payload: Partial<Competence>;
}
interface ScMutVars {
  id?: Id;
  payload: Partial<SousCompetence> & { competenceId?: Id; code?: string };
  scCreateTarget: ScTarget | null;
}
interface SavoirMutVars {
  id?: Id;
  payload: Partial<Savoir> & { code?: string };
  savoirMode: string;
  sousCompetenceId?: Id;
  competenceId?: Id;
}
...
function buildCompDeleteErrorMessage(sousCompsCount: number, savoirsDirectsCount: number) {
  ...
  const sdLabel =
    pl(savoirsDirectsCount, 'savoir') + `${savoirsDirectsCount > 1 ? 's direct' : ' direct'}`;
...
export default function useCompetenceCrud() {
  ...
  const domaineMutation = useMutation<Domaine, Error, DomaineMutVars>({
    mutationFn: ({ id, payload }) =>
      id
        ? CompetenceService.domaine.update(id, payload)
        : CompetenceService.domaine.create(payload),
  ...
  const sousCompetenceMutation = useMutation<SousCompetence, Error, ScMutVars>({
    mutationFn: ({ id, payload, scCreateTarget: target }) => {
      ...
      if (target?.type === 'sousCompetence')
        return CompetenceService.sousCompetence.createEnfant(target.id!, payload);
  ...
  const savoirMutation = useMutation<Savoir, Error, SavoirMutVars>({
    mutationFn: ({ id, payload, savoirMode: mode, sousCompetenceId, competenceId }) => {
      ...
      if (mode === 'direct')
        return CompetenceService.savoir.createForCompetence(competenceId!, payload);
  ...
  const handleDomaineSubmit = useCallback(
  ...
  const handleDomaineDelete = useCallback(
    async (id: Id) => {
      try {
  ...
  const openCompModal = useCallback(
    (form: FormInstance, record: Competence | null = null) => {
      setEditingComp(record);
      const { descriptionCore, prerequisiteManual } = splitDescriptionAndPrerequisites(
        record?.description,
      );
      const effectivePrerequisiteManual =
        (record?.prerequisiteManual || '').trim() || prerequisiteManual;
      form.setFieldsValue(
        record
          ? {
              code: record.code,
  ...
  const handleCompSubmit = useCallback(
    async (form: FormInstance) => {
      try {
  ...
  const handleCompDelete = useCallback(
    async (id: Id) => {
  ...
  const openScModal = useCallback(
    (form: FormInstance, target: ScTarget | null = null, record: SousCompetence | null = null) => {
      ...
      const buildCreatePrefix = (parentCode: string | undefined) =>
  ...
  const handleScSubmit = useCallback(
    async (form: FormInstance) => {
      try {
  ...
  const handleScDelete = useCallback(
    async (id: Id) => {
      try {
  ...
  const openSavoirModal = useCallback(
    (
      form: FormInstance,
      record: Savoir | null = null,
      targetSousComp: SousCompetence | null = null,
    ) => {
  ...
  const handleSavoirSubmit = useCallback(
    async (form: FormInstance) => {
      try {
  ...
  const handleSavoirDelete = useCallback(
    async (id: Id) => {
      try {
  ...
  const savoirColumns = useMemo<TableColumnsType<Savoir>>(() => buildSavoirColumns(), []);
