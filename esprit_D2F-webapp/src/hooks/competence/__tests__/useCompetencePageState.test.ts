...
describe('useCompetencePageState', () => {
  it('initializes activeTab from search params', () => {
    const { result } = renderHook(
      () => useCompetencePageState({ crud: {}, loadStructure: vi.fn() }),
      { wrapper },
    );
  ...
  it('normalizes sousCompetences tab to competences', () => {
    const w = ({ children }: { children: React.ReactNode }) =>
      React.createElement(
        MemoryRouter,
        { initialEntries: ['/competences?tab=sousCompetences'] },
        children,
      );
    const { result } = renderHook(
      () => useCompetencePageState({ crud: {}, loadStructure: vi.fn() }),
  ...
  it('handleTabChange updates activeTab and search params', () => {
    ...
    act(() => {
      result.current.handleTabChange('hierarchie');
    });
  ...
  it('loads structure when activeTab is hierarchie', () => {
    ...
    const { result } = renderHook(() => useCompetencePageState({ crud: {}, loadStructure }), {
      wrapper,
    });
    act(() => {
      result.current.handleTabChange('hierarchie');
