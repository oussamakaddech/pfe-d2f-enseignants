...
const wrapper = ({ children }: { children: React.ReactNode }) =>
  React.createElement(QueryClientProvider, { client: qc }, React.createElement(App, {}, children));
...
describe('useStructureData', () => {
  ...
  it('applyFilter sets filters', () => {
    const { result } = renderHook(() => useStructureData(), { wrapper });
    act(() => {
      result.current.applyFilter(1, 2);
    });
  ...
  it('handleAddNiveauSavoir requires target', async () => {
    ...
    const { result } = renderHook(() => useStructureData(), { wrapper });
    await act(async () => {
      await result.current.handleAddNiveauSavoir({ niveau: 'N1' } as never);
    });
  ...
  it('handleAddNiveauSavoir calls add when target set', async () => {
    ...
    const { result } = renderHook(() => useStructureData(), { wrapper });
    act(() => {
      result.current.openNiveauModal('competence', 5, 'c');
    });
    await act(async () => {
      await result.current.handleAddNiveauSavoir({ niveau: 'N1' } as never);
    });
