...
vi.mock("@/services/competence/CompetenceService", () => {
  const mk = () => ({
    getAll: vi.fn(() => Promise.resolve([])), getByCompetence: vi.fn(() => Promise.resolve([])),
    getBySousCompetence: vi.fn(() => Promise.resolve([])), getArbreComplet: vi.fn(() => Promise.resolve([])),
    create: vi.fn(), update: vi.fn(), delete: vi.fn(),
    createEnfant: vi.fn(), toggleActif: vi.fn(), rechercheParDomaine: vi.fn(() => Promise.resolve([])),
    rechercheGlobale: vi.fn(() => Promise.resolve([])), add: vi.fn(), remove: vi.fn(),
  });
  return {
    default: {
      domaine: mk(), competence: mk(), sousCompetence: mk(), savoir: mk(),
      enseignantCompetence: mk(), niveauDefinition: mk(), structure: mk(), prerequisite: mk(),
    },
    __esModule: true,
  };
});
...
const fakeForm = (values: Record<string, unknown> = {}) => {
  let store = values;
  return {
    setFieldsValue: (v: Record<string, unknown>) => { store = { ...store, ...v }; },
...
describe("useCompetenceCrud", () => {
  ...
  it("openDomaineModal opens modal with record values", () => {
    const form = fakeForm();
    const { result } = renderHook(() => useCompetenceCrud(), { wrapper: makeWrapper() });
    act(() => { result.current.openDomaineModal(form, { code: "C1", nom: "N1", description: "d", actif: true } as never); });
  ...
  it("handleDomaineSubmit creates when no editing id", async () => {
    ...
    const { result } = renderHook(() => useCompetenceCrud(), { wrapper: makeWrapper() });
    act(() => { result.current.openDomaineModal(form); });
    await act(async () => { await result.current.handleDomaineSubmit(form); });
  ...
  it("handleCompDelete shows error when dependents exist", async () => {
    ...
    await act(async () => { await result.current.handleCompDelete(3); });
