...
import {
  ...
  useTopParticipants,
  useTopAbsentees,
  useEnseignantsNonAffectes,
  useKpiCountAndHeures,
  useKpiFormationsByTypeFiltered,
  useKpiCountAndHeuresMutation,
  useKpiFormationsByTypeFilteredMutation,
  useKpiCountByTrainerType,
  useKpiCountByTrainerTypeMutation,
  useFormationsParticipantKPIs,
...
vi.mock('@/services/analyse/KPIService', () => ({
  default: {
    getTotalFormations: vi.fn(),
    getTotalHeures: vi.fn(),
    getUniqueParticipants: vi.fn(),
    getFormationsByEtat: vi.fn(),
    getTopParticipants: vi.fn(),
    getTopAbsentees: vi.fn(),
    getEnseignantsNonAffectes: vi.fn(),
    getCountAndHeures: vi.fn(),
    getFormationsByTypeFiltered: vi.fn(),
    getCountByTrainerTypeWithIds: vi.fn(),
  },
  __esModule: true,
}));
vi.mock('@/services/analyse/ParticipantKPIService', () => ({
  default: {
    getFormationsParticipantKPIs: vi.fn(),
    getGlobalParticipantKPI: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useKpi', () => {
  ...
  it('useTotalFormations fetches', async () => {
    ...
    const { result } = renderHook(() => useTotalFormations('2024-01-01', '2024-12-31'), {
  ...
  it('useUniqueParticipants fetches', async () => {
    ...
    const { result } = renderHook(() => useUniqueParticipants('s', 'e'), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useFormationsByEtat fetches', async () => {
    ...
    const { result } = renderHook(() => useFormationsByEtat('s', 'e'), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useEnseignantsNonAffectes fetches', async () => {
    ...
    const { result } = renderHook(() => useEnseignantsNonAffectes('s', 'e'), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
    expect(result.current.isSuccess).toBe(true);
  ...
  it('useKpiCountAndHeures fetches', async () => {
    ...
    const { result } = renderHook(() => useKpiCountAndHeures({} as never), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useKpiFormationsByTypeFiltered fetches', async () => {
    ...
    const { result } = renderHook(() => useKpiFormationsByTypeFiltered({} as never), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useKpiCountAndHeuresMutation calls service', async () => {
    (KPIService.getCountAndHeures as ReturnType<typeof vi.fn>).mockResolvedValue({});
    const { result } = renderHook(() => useKpiCountAndHeuresMutation(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
  ...
  it('useKpiFormationsByTypeFilteredMutation calls service', async () => {
    (KPIService.getFormationsByTypeFiltered as ReturnType<typeof vi.fn>).mockResolvedValue({});
    const { result } = renderHook(() => useKpiFormationsByTypeFilteredMutation(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
      await result.current.mutateAsync({} as never);
  ...
  it('useKpiCountByTrainerTypeMutation calls service', async () => {
    (KPIService.getCountByTrainerTypeWithIds as ReturnType<typeof vi.fn>).mockResolvedValue({});
    const { result } = renderHook(() => useKpiCountByTrainerTypeMutation(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
  ...
  it('useFormationsParticipantKPIs fetches', async () => {
    (
      ParticipantKPIService.getFormationsParticipantKPIs as ReturnType<typeof vi.fn>
    ).mockResolvedValue([]);
    const { result } = renderHook(() => useFormationsParticipantKPIs('s', 'e'), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useGlobalParticipantKPI fetches', async () => {
    ...
    const { result } = renderHook(() => useGlobalParticipantKPI('s', 'e'), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
