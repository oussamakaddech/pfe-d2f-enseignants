...
export function useTopParticipants(start: string, end: string, upId?: IdOrNull, deptId?: IdOrNull) {
...
export function useTopAbsentees(start: string, end: string, upId?: IdOrNull, deptId?: IdOrNull) {
  const upIdStr = upId == null ? null : String(upId);
  const deptIdStr = deptId == null ? null : String(deptId);
  return useQuery<unknown[]>({
