...
import {
  useAllDocuments, useDocumentById, useCreateDocument, useUpdateDocument,
  useDeleteDocument, useDownloadDocument,
...
vi.mock("@/services/formation/DocumentService", () => ({
  default: {
    getAllDocuments: vi.fn(), getDocumentById: vi.fn(), createDocument: vi.fn(),
    updateDocument: vi.fn(), deleteDocument: vi.fn(), downloadDocument: vi.fn(),
  },
  __esModule: true,
}));
...
describe("useDocument", () => {
  ...
  it("useCreateDocument calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ formationId: 1, pathType: "a", nomDocument: "n", obligation: "o", file: new Blob() } as never); });
  ...
  it("useUpdateDocument calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ id: 1, pathType: "a", nomDocument: "n", obligation: "o" } as never); });
