...
describe('useCalendar', () => {
  ...
  it('useImportCalendar calls importCalendar and invalidates', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ file: new Blob() as never });
    });
  ...
  it('useCalendarFormations fetches', async () => {
    ...
    const { result } = renderHook(() => useCalendarFormations({} as never), {
  ...
  it('useCalendarParticipants disabled without id', async () => {
    const { result } = renderHook(() => useCalendarParticipants(undefined), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
