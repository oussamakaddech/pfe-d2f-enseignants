...
import {
  useAllDepts, useDeptById, useCreateDept, useUpdateDept, useDeleteDept, useImportDeptsExcel,
} from "@/hooks/formation/useDeptCrud";
import {
  useAllUps, useUpById, useCreateUp, useUpdateUp, useDeleteUp, useImportUpsExcel,
...
vi.mock("@/services/formation/DeptService", () => ({
  default: {
    getAllDepts: vi.fn(), getDeptById: vi.fn(), createDept: vi.fn(),
    updateDept: vi.fn(), deleteDept: vi.fn(), importDeptsExcel: vi.fn(),
  },
  __esModule: true,
}));
vi.mock("@/services/api/UploadService", () => ({
  default: {
    getAllUps: vi.fn(), getUpById: vi.fn(), createUp: vi.fn(),
    updateUp: vi.fn(), deleteUp: vi.fn(), importUpsExcel: vi.fn(),
...
describe("useDeptCrud", () => {
  ...
  it("useCreateDept calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ nom: "d" } as never); });
...
describe("useUpCrud", () => {
  ...
  it("useCreateUp calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ nom: "u" } as never); });
