...
vi.mock('@/services/formation/FormationCustomService', () => ({
  default: { generateCertificates: vi.fn() },
  __esModule: true,
}));
vi.mock('@/services/formation/FormationReportService', () => ({
  default: { getFormationsParRoleEtPeriode: vi.fn() },
  __esModule: true,
}));
vi.mock('@/services/formation/FormationWorkflowService', () => ({
  default: { exportFormations: vi.fn() },
  __esModule: true,
}));
vi.mock('@/services/besoin/MailService', () => ({
  default: { sendEmail: vi.fn() },
  __esModule: true,
}));
vi.mock('@/services/formation/InscriptionService', () => ({
  default: {
    getInscriptionsByFormation: vi.fn(),
    getAllInscriptions: vi.fn(),
    getInscriptionsByEnseignant: vi.fn(),
    getMyInscriptions: vi.fn(),
    demanderInscription: vi.fn(),
    traiterDemande: vi.fn(),
    traiterDemandeBulk: vi.fn(),
    annulerInscription: vi.fn(),
    getFormationsAccessibles: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useFormationExtras', () => {
  ...
  it('useGenerateFormationCertificates calls service', async () => {
    ...
    const { result } = renderHook(() => useGenerateFormationCertificates(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
      await result.current.mutateAsync({ formationId: 1 } as never);
    });
  ...
  it('useFormationsParRoleEtPeriode fetches', async () => {
    (
      FormationReportService.getFormationsParRoleEtPeriode as ReturnType<typeof vi.fn>
    ).mockResolvedValue([]);
    const { result } = renderHook(() => useFormationsParRoleEtPeriode('admin', 3, 's', 'e'), {
      wrapper: createWrapper(),
    });
  ...
  it('useSendEmail calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ to: 'a@b.c', subject: 's', content: 'c' } as never);
    });
  ...
  it('useInscriptionsByFormation fetches', async () => {
    ...
    const { result } = renderHook(() => useInscriptionsByFormation(3), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useInscriptionsByEnseignant fetches', async () => {
    ...
    const { result } = renderHook(() => useInscriptionsByEnseignant(3), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useDemanderInscription calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ formationId: 1, enseignantId: 2 } as never);
    });
  ...
  it('useTraiterDemande calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 1, approuver: true } as never);
    });
  ...
  it('useTraiterDemandeBulk calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ ids: [1], approuver: true } as never);
    });
  ...
  it('useAnnulerInscription calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 1, enseignantId: 2 } as never);
    });
  ...
  it('useExportFormations calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ start: 's', end: 'e' } as never);
    });
