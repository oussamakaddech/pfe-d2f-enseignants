...
export function useDragAndDrop({
  ...
  const [draggedSavoirInfo, setDraggedSavoirInfo] = useState<{ nom: string; type: string } | null>(
    null,
  );
  ...
  const onSavoirDragStart = useCallback(
    (e: React.DragEvent, di: number, ci: number, sci: number, si: number) => {
      ...
      try {
        ...
        /* ignore – Firefox may block setData in some contexts */
      }
    },
    [tree],
  );
  ...
  const onTagDragStart = useCallback((e: React.DragEvent, fromEnsId: string, s: SavoirDragInfo) => {
    e.stopPropagation();
    const info: SavoirDragInfo = {
      di: s.di,
      ci: s.ci,
  ...
  const parseDropInfo = (e: React.DragEvent): SavoirDragInfo | null => {
    ...
    if (!info) {
      try {
        info = JSON.parse(e.dataTransfer.getData('application/json')) as SavoirDragInfo;
      } catch {
        return null;
      }
    }
  ...
  const onEnsDrop = useCallback(
    (e: React.DragEvent, ensId: string) => {
      ...
      const ensObj = effectiveEnseignants.find((en) => String(en.id ?? en.enseignantId) === ensId);
