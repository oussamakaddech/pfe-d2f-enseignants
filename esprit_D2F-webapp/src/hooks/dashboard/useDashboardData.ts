...
import type {
  ...
  EnseignantsInactifsResponse, FormationsParPeriodeResponse,
...
export function useInactifs(enabled = true) {
  return useQuery<EnseignantsInactifsResponse>({
    ...
    queryFn: () => AnalyticsService.getEnseignantsSansFormation({ mois: INACTIFS_SEUIL_MOIS, page: 0, size: 5 }),
    enabled,
    staleTime: STALE,
  });
}
...
export function useFormationsTimeline(scope: DashboardScope) {
  return useQuery<FormationsParPeriodeResponse>({
    ...
    queryFn: () => AnalyticsService.getFormationsParPeriode({ granularite: "MOIS", debut: scope.start, fin: scope.end }),
