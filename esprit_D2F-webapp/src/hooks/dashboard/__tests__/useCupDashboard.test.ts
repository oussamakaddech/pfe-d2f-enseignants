...
describe("useCupDashboard", () => {
  ...
  it("besoinsParDept aggregates by department", async () => {
    ...
    expect(result.current.besoinsParDept[0].total).toBeGreaterThanOrEqual(result.current.besoinsParDept[1].total);
  });

  it("besoinsParDept uses default dept and handles unresolved values", async () => {
    K.getFormationsByEtat.mockResolvedValue({});
    B.getAllBesoinFormations.mockResolvedValue([
      { approuveAdmin: undefined }, { priorite: undefined },
    ]);
    const { result } = renderHook(() => useCupDashboard(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.besoinsParDept).toHaveLength(1));
    expect(result.current.besoinsParDept[0]).toMatchObject({ departement: "Non assigné", total: 2, approuves: 0, enAttente: 2, critiques: 0, hautes: 0 });
  });
  ...
  it("topCompetences derives from besoins and in-demand", async () => {
    ...
    seeded.setQueryData(["analyse", "in-demand"], [
      { competency_name: "Python", demand_12m: 5 },
    ]);
  ...
  it("besoinsPriorises maps only non-approved besoins", async () => {
    K.getFormationsByEtat.mockResolvedValue({});
    B.getAllBesoinFormations.mockResolvedValue([
      { idBesoinFormation: 1, titre: "T1", priorite: "CRITIQUE", impactStrategique: "Très stratégique", approuveAdmin: false, departement: "INFO", up: "UP1" },
      { idBesoinFormation: 2, theme: "Th2", priorite: "HAUTE", impactStrategique: "faible", approuveAdmin: true },
    ]);
    const { result } = renderHook(() => useCupDashboard(), { wrapper: createWrapper() });
    ...
    expect(result.current.besoinsPriorises[0]).toMatchObject({
      id: 1, label: "T1", urgency: 5, impact: 5, priorite: "CRITIQUE", departement: "INFO", up: "UP1",
    });
  });
  ...
  it("besoinsPriorises defaults when missing fields", async () => {
    ...
    const { result } = renderHook(() => useCupDashboard(), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.besoinsPriorises).toHaveLength(1));
    expect(result.current.besoinsPriorises[0]).toMatchObject({ label: "Sans titre", urgency: 2, impact: 2, count: 1, priorite: "NON_DEFINIE" });
  });
  ...
  it("tauxReussite derives from dept analytics", async () => {
    ...
    expect(result.current.tauxReussite[0].reussite).toBeGreaterThanOrEqual(result.current.tauxReussite[1].reussite);
