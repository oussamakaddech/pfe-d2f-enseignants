...
import {
  useGlobalDashboard,
  useInactifs,
  useFormationsTimeline,
  useParticipationByDept,
...
describe('useDashboardData', () => {
  ...
  it('useFormationsTimeline disabled without dates', async () => {
    const { result } = renderHook(() => useFormationsTimeline({ start: '', end: '' } as never), {
  ...
  it('useFormationsTimeline fetches', async () => {
    ...
    const { result } = renderHook(() => useFormationsTimeline({ start: 's', end: 'e' } as never), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
