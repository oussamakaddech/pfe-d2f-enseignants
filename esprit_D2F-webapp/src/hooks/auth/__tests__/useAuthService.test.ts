...
import {
  useLogin, useForgotPassword, useResetPassword, useUpdatePassword,
  useEditProfile, useBanAccount, useEnableAccount, useDeleteAccount,
  usePermanentDeleteAccount, useUpdateAccount,
} from "@/hooks/auth/useAuthService";
import {
  login as loginApi, forgotPassword, resetPassword,
} from "@/services/auth/AuthService";
import {
  editProfile, updatePassword as updatePasswordApi, banAccount, enableAccount,
  deleteAccount, permanentDeleteAccount, updateAccount,
...
describe("useAuthService", () => {
  ...
  it("useForgotPassword calls forgotPassword", async () => {
    ...
    await act(async () => { await result.current.mutateAsync("a@b.c"); });
  ...
  it("useResetPassword calls resetPassword", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ token: "t", password: "p" } as never); });
  ...
  it("useBanAccount calls banAccount", async () => {
    ...
    await act(async () => { await result.current.mutateAsync("user1"); });
  ...
  it("useDeleteAccount calls deleteAccount", async () => {
    ...
    await act(async () => { await result.current.mutateAsync("id1"); });
  ...
  it("useUpdateAccount calls updateAccount", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ userId: "id1", data: {}, role: "R" } as never); });
  ...
  it("useUpdatePassword calls updatePasswordApi", async () => {
    (updatePasswordApi as ReturnType<typeof vi.fn>).mockResolvedValue({});
    const { result } = renderHook(() => useUpdatePassword(), { wrapper: createWrapper() });
    await act(async () => { await result.current.mutateAsync({ oldPassword: "a", newPassword: "b" } as never); });
