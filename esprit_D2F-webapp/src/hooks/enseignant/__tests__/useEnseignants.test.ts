...
vi.mock('@/services/formation/EnseignantService', () => ({
  default: {
    ...
    getEnseignantById: vi.fn(),
    createEnseignant: vi.fn(),
    updateEnseignant: vi.fn(),
    deleteEnseignant: vi.fn(),
    uploadEnseignants: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useEnseignants', () => {
  ...
  it('useEnseignants maps email field', async () => {
    (EnseignantService.getAllEnseignants as ReturnType<typeof vi.fn>).mockResolvedValue([
      { id: 1, mail: '', email: 'a@b.c' },
    ]);
  ...
  it('useEnseignantById falls back to list on 404', async () => {
    (EnseignantService.getEnseignantById as ReturnType<typeof vi.fn>).mockRejectedValue({
      response: { status: 404 },
    });
    (EnseignantService.getAllEnseignants as ReturnType<typeof vi.fn>).mockResolvedValue([
      { mail: 'a@b.c' },
