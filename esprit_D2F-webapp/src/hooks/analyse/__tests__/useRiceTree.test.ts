    ...
    code: "D1", nom: "Domaine1", description: "", competences: [
      {
        code: "C1", nom: "Comp1", description: "", ordre: 1, savoirs: [
          ...
          {
            code: "SC1", nom: "Sc1", description: "", savoirs: [
              { code: "S2", nom: "Sav2", type: "PRATIQUE", niveau: null, enseignantsSuggeres: [] },
            ],
          },
        ],
      },
...
describe("useRiceTree", () => {
  ...
  it("addCompetence adds competence", () => {
    ...
    act(() => { result.current.addCompetence(0); });
  ...
  it("addSavoir adds savoir under competence", () => {
    ...
    act(() => { result.current.addCompetence(0); });
    act(() => { result.current.commitRename(); });
    act(() => { result.current.addSavoir(0, 0); });
    const comp = (result.current.tree[0] as { competences: { savoirs: unknown[] }[] }).competences[0];
  ...
  it("toggleType flips THEORIQUE to PRATIQUE", () => {
    ...
    act(() => { result.current.commitRename(); });
    act(() => { result.current.addSavoir(0, 0); });
    act(() => { result.current.toggleType(0, 0, -1, 0); });
    const savoir = (result.current.tree[0] as { competences: { savoirs: { type: string }[] }[] }).competences[0].savoirs[0];
  ...
  it("toggleEnsAssign assigns and unassigns", () => {
    ...
    act(() => { result.current.commitRename(); });
    act(() => { result.current.addSavoir(0, 0); });
    act(() => { result.current.toggleEnsAssign(0, 0, -1, 0, "e1"); });
    let savoir = (result.current.tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0];
    expect(savoir.enseignantsSuggeres).toContain("e1");
    act(() => { result.current.toggleEnsAssign(0, 0, -1, 0, "e1"); });
    savoir = (result.current.tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0];
  ...
  it("deleteSavoir removes a savoir", () => {
    ...
    act(() => { result.current.commitRename(); });
    act(() => { result.current.addSavoir(0, 0); });
    act(() => { result.current.deleteSavoir(0, 0, -1, 0); });
    const comp = (result.current.tree[0] as { competences: { savoirs: unknown[] }[] }).competences[0];
  ...
  it("liveStats computes counts", () => {
    const { result } = renderHook(() => useRiceTree(msgApi), { wrapper });
    act(() => { result.current.setTree(seedTree()); });
  ...
  it("clearAllAssignments clears assignments", () => {
    const { result } = renderHook(() => useRiceTree(msgApi), { wrapper });
    const tree = seedTree();
    ((tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0]).enseignantsSuggeres = ["e1"];
    act(() => { result.current.setTree(tree); });
    act(() => { result.current.clearAllAssignments(); });
    const savoir = (result.current.tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0];
  ...
  it("confirmMerge merges two savoirs", () => {
    ...
    act(() => { result.current.setTree(seedTree()); });
    act(() => { result.current.toggleEnsAssign(0, 0, -1, 0, "e1"); });
    act(() => { result.current.openMerge(0, 0, -1, 0); });
    act(() => { result.current.setMergeDst({ di: 0, ci: 0, sci: 0, si: 0 }); });
  ...
  it("remapInTree replaces ids", () => {
    const { result } = renderHook(() => useRiceTree(msgApi), { wrapper });
    const tree = seedTree();
    ((tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0]).enseignantsSuggeres = ["ext_1"];
    act(() => { result.current.setTree(tree); });
    act(() => { result.current.remapInTree("ext_1", "real_1"); });
    const savoir = (result.current.tree[0] as { competences: { savoirs: { enseignantsSuggeres: string[] }[] }[] }).competences[0].savoirs[0];
  ...
  it("treeFilteredIndices filters by search", () => {
    const { result } = renderHook(() => useRiceTree(msgApi), { wrapper });
    act(() => { result.current.setTree(seedTree()); });
    act(() => { result.current.setTreeSearch("Sav1"); });
