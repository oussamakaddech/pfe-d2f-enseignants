...
describe("useModelStatus", () => {
  ...
  it("status loading when no report", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockResolvedValue(null as never);
  ...
  it("status no_model on non-trained message", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockResolvedValue({ message: "No model trained yet" });
    const { result } = renderHook(() => useModelStatus(), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("status drift when drift detected", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockResolvedValue({ drift_detected: true, days_since_training: 10 });
    const { result } = renderHook(() => useModelStatus(), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("status stale when drift and old", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockResolvedValue({ drift_detected: true, days_since_training: 120 });
    const { result } = renderHook(() => useModelStatus(), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("status fresh when no drift", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockResolvedValue({ drift_detected: false });
    const { result } = renderHook(() => useModelStatus(), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("status error on failure", async () => {
    (AnalysePredictiveService.getDrift as ReturnType<typeof vi.fn>).mockRejectedValue(new Error("x"));
    const { result } = renderHook(() => useModelStatus(), { wrapper: createWrapper() });
    await flushQuery(result);
