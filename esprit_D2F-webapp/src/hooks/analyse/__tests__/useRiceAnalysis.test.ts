...
const makeEnseignant = (over: Record<string, unknown> = {}) => ({
  id: 1,
  nom: 'Doe',
  prenom: 'John',
  modules: ['M1'],
...
describe('useRiceAnalysis', () => {
  ...
  it('handleAnalyze warns when no files', async () => {
    ...
    const { result } = renderHook(() => useRiceAnalysis(baseParams({ files: [], msgApi })), {
      wrapper,
    });
    await act(async () => {
  ...
  it('handleAnalyze dedupes extracted enseignants already in DB', async () => {
    ...
    const { result } = renderHook(
      () =>
        useRiceAnalysis(
          baseParams({ files: [new File([], 'a.pdf')], riceAnalyze, allEnseignants }),
        ),
      { wrapper },
    );
    await act(async () => {
  ...
  it('handleAnalyze detects new department and warns', async () => {
    ...
    const { result } = renderHook(
      () =>
        useRiceAnalysis(
          baseParams({ files: [new File([], 'a.pdf')], riceAnalyze, setDepartement, msgApi }),
        ),
      { wrapper },
    );
    await act(async () => {
    ...
    vi.useRealTimers();
    expect(msgApi.warning).toHaveBeenCalledWith(
      'Département détecté : TI. Rechargement des enseignants...',
    );
  ...
  it('handleAnalyze error path', async () => {
    ...
    const msgApi = { warning: vi.fn(), error: vi.fn() } as any;
    const riceAnalyze = {
      mutateAsync: vi.fn().mockRejectedValue({ response: { data: { detail: 'boom' } } }),
    } as any;
    const { result } = renderHook(
      () =>
        useRiceAnalysis(
          baseParams({ files: [new File([], 'a.pdf')], riceAnalyze, setCurrentStep, msgApi }),
        ),
      { wrapper },
    );
    await act(async () => {
  ...
  it('handleAnalyze falls back to message on error', async () => {
    ...
    const { result } = renderHook(
      () => useRiceAnalysis(baseParams({ files: [new File([], 'a.pdf')], riceAnalyze, msgApi })),
      { wrapper },
    );
    await act(async () => {
      const p = result.current.handleAnalyze();
  ...
  it('cleanTreePropositions drops metadata savoirs and empty sous-competences', async () => {
                ...
                sousCompetences: [
                  {
                    code: 'SC',
                    nom: 'SC',
                    savoirs: [{ code: 'S3', nom: 'Metadata line: : :', enseignantsSuggeres: [] }],
                  },
                ],
              },
            ],
    ...
    const setTree = vi.fn();
    const { result } = renderHook(
      () => useRiceAnalysis(baseParams({ files: [new File([], 'a.pdf')], riceAnalyze, setTree })),
      { wrapper },
    );
    await act(async () => {
      const p = result.current.handleAnalyze();
    ...
    const comp = (tree[0] as { competences: { savoirs: unknown[]; sousCompetences: unknown[] }[] })
      .competences[0];
  ...
  it('handleAnalyze includes enseignants mapped from allEnseignants', async () => {
      ...
      () =>
        useRiceAnalysis(
          baseParams({
            ...
            riceAnalyze,
            allEnseignants: [
              makeEnseignant({ id: 7, nom: 'X', prenom: 'Y', modules: ['M2'] }),
            ] as any,
          }),
        ),
      { wrapper },
    ...
    vi.useRealTimers();
    const sent = riceAnalyze.mutateAsync.mock.calls[0][0] as {
      enseignants: { id: string; nom: string; prenom: string; modules: string[] }[];
    };
