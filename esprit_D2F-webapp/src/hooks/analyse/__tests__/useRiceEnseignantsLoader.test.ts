...
vi.mock("@/services/analyse/RiceService", () => ({
  default: { getEnseignants: vi.fn(), getSavoirs: vi.fn(), getEnseignantAffectations: vi.fn(), analyze: vi.fn() },
  __esModule: true,
}));
...
describe("useRiceEnseignantsLoader", () => {
  ...
  it("loads enseignants into state", async () => {
    (RiceService.getEnseignants as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: 1, nom: "A" }]);
    const { result } = renderHook(() => useRiceEnseignantsLoader("INFO", msgApi), { wrapper: createWrapper() });
  ...
  it("sets error on failure with warning", async () => {
    (RiceService.getEnseignants as ReturnType<typeof vi.fn>).mockRejectedValue({ response: { status: 403 } });
    const { result } = renderHook(() => useRiceEnseignantsLoader("INFO", msgApi), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("continueWithoutEnseignants clears state", async () => {
    ...
    const { result } = renderHook(() => useRiceEnseignantsLoader("INFO", msgApi), { wrapper: createWrapper() });
    await flushQuery(result);
    act(() => { result.current.continueWithoutEnseignants(); });
