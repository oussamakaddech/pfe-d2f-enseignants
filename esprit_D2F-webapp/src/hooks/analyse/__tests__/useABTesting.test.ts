...
describe("useABTesting", () => {
  ...
  it("useABAssign calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ experiment_name: "exp1", enseignant_id: "e" } as never); });
  ...
  it("useABEvent calls service", async () => {
    ...
    await act(async () => { await result.current.mutateAsync({ experiment_name: "exp1", event: "shown" } as never); });
