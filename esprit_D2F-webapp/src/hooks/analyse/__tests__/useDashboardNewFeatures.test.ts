...
vi.mock('@/services/analyse/AnalyticsService', () => ({
  default: {
    ...
    getPilotageDashboard: vi.fn(),
    getForecast: vi.fn(),
    getBenchmark: vi.fn(),
    detectAnomalies: vi.fn(),
    detectAnomaliesDepartment: vi.fn(),
  },
  __esModule: true,
}));
...
describe('useDashboard', () => {
  ...
  it('error when fetch fails', async () => {
    (AnalyticsService.getDashboardGlobal as ReturnType<typeof vi.fn>).mockRejectedValue(
      new Error('boom'),
    );
    const { result } = renderHook(() => useDashboard(), { wrapper: createWrapper() });
    await flushQuery(result);
...
describe('useNewFeatures', () => {
  ...
  it('useDetectAnomaliesDepartment calls service', async () => {
    ...
    const { result } = renderHook(() => useDetectAnomaliesDepartment(), {
