...
describe('useAnalytics', () => {
  ...
  it('fetchGaps enables gaps query', async () => {
    ...
    const { result } = renderHook(() => useAnalytics('e1'), { wrapper: createWrapper() });
    act(() => {
      result.current.fetchGaps('HAUTE', 0);
    });
  ...
  it('fetchRecommendations enables reco query', async () => {
    (AnalyticsService.getRecommendations as ReturnType<typeof vi.fn>).mockResolvedValue({
      items: [],
    });
    const { result } = renderHook(() => useAnalytics('e1'), { wrapper: createWrapper() });
    act(() => {
      result.current.fetchRecommendations(5, 0);
    });
    await flushQuery(result);
  ...
  it('fetchTrainingPath enables training path query', async () => {
    ...
    const { result } = renderHook(() => useAnalytics('e1'), { wrapper: createWrapper() });
    act(() => {
      result.current.fetchTrainingPath(5);
    });
    await flushQuery(result);
  ...
  it('updateRecoStatus calls service', async () => {
    ...
    const { result } = renderHook(() => useAnalytics('e1'), { wrapper: createWrapper() });
    await act(async () => {
      await result.current.updateRecoStatus({ recommendationId: 1, statut: 'ACCEPTEE' } as never);
    });
  ...
  it('useGroupedRecommendations fetches', async () => {
    ...
    const { result } = renderHook(() => useGroupedRecommendations('e1', 'competence' as never), {
  ...
  it('useSimulateWhatIf calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ plan: [] } as never);
    });
