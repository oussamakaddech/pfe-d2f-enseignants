...
vi.mock('@/services/analyse/RiceService', () => ({
  default: {
    ...
    assignCompetence: vi.fn(),
    removeAssignment: vi.fn(),
    createEnseignant: vi.fn(),
    updateEnseignant: vi.fn(),
    deactivateEnseignant: vi.fn(),
...
describe('useRiceService', () => {
  ...
  it('useRiceEnseignantAffectations fetches', async () => {
    ...
    const { result } = renderHook(() => useRiceEnseignantAffectations(), {
  ...
  it('useRiceSaveAssignments calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ add: [], remove: [] } as never);
    });
  ...
  it('useRiceDeactivateEnseignant calls service', async () => {
    ...
    const { result } = renderHook(() => useRiceDeactivateEnseignant(), {
      wrapper: createWrapper(),
    });
    await act(async () => {
      await result.current.mutateAsync(1);
