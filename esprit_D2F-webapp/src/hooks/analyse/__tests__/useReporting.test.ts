...
import {
  useEnseignantsInactifs,
  useFormationsParPeriode,
  useFormationsParUp,
...
vi.mock('@/services/analyse/AnalyticsService', () => ({
  default: {
    getEnseignantsSansFormation: vi.fn(),
...
describe('useReporting', () => {
  ...
  it('useEnseignantsInactifs fetches', async () => {
    (AnalyticsService.getEnseignantsSansFormation as ReturnType<typeof vi.fn>).mockResolvedValue({
      items: [],
    });
    const { result } = renderHook(() => useEnseignantsInactifs({ mois: 6, page: 0, size: 5 }), {
  ...
  it('useFormationsParPeriode fetches', async () => {
    (AnalyticsService.getFormationsParPeriode as ReturnType<typeof vi.fn>).mockResolvedValue({
      items: [],
    });
    const { result } = renderHook(() => useFormationsParPeriode({ granularite: 'MOIS' } as never), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useFormationsParUp fetches', async () => {
    (AnalyticsService.getFormationsParUp as ReturnType<typeof vi.fn>).mockResolvedValue({
      items: [],
    });
  ...
  it('useFormationsParDepartement fetches', async () => {
    (AnalyticsService.getFormationsParDepartement as ReturnType<typeof vi.fn>).mockResolvedValue({
      departements: [],
    });
    const { result } = renderHook(() => useFormationsParDepartement(), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
  ...
  it('useAnalyticsExport exportExcel triggers download', async () => {
    ...
    await act(async () => {
      await result.current.exportExcel('FORMATION' as never);
    });
  ...
  it('useAnalyticsExport exportPdf triggers download', async () => {
    ...
    const { result } = renderHook(() => useAnalyticsExport(), { wrapper: createWrapper() });
    await act(async () => {
      await result.current.exportPdf('SYNTHESE' as never);
    });
