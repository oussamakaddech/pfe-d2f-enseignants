...
import {
  useFormationsByAnimateur,
  useSeancePresences,
  useSeancePresenceStats,
  useUpdatePresence,
  useBatchUpdatePresences,
  useMarkAllPresences,
  useAggregatedPresences,
  useMesPresences,
...
vi.mock('@/services/formation/FormationWorkflowService', () => ({
  default: {
    getFormationsByAnimateur: vi.fn(),
    getPresencesBySeance: vi.fn(),
    getSeancePresenceStats: vi.fn(),
    updatePresence: vi.fn(),
    batchUpdatePresences: vi.fn(),
    markAllPresences: vi.fn(),
    getMesPresences: vi.fn(),
  },
  __esModule: true,
}));
...
describe('usePresence', () => {
  ...
  it('useFormationsByAnimateur fetches', async () => {
    (
      FormationWorkflowService.getFormationsByAnimateur as ReturnType<typeof vi.fn>
  ...
  it('useSeancePresences disabled without id', async () => {
    const { result } = renderHook(() => useSeancePresences(undefined), {
  ...
  it('useUpdatePresence calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 1, isPresent: true } as never);
    });
  ...
  it('useBatchUpdatePresences calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ seanceId: 1, updates: [] } as never);
    });
  ...
  it('useMarkAllPresences calls service', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ seanceId: 1, present: true } as never);
    });
  ...
  it('useAggregatedPresences fetches many', async () => {
    ...
    const { result } = renderHook(() => useAggregatedPresences([1, 2]), {
      wrapper: createWrapper(),
    });
    await flushQuery(result);
