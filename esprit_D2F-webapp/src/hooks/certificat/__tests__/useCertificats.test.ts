...
import {
  useAllCertificates, useCertificatesByFormation, useCertificatesByEmail,
  useCreateCertificate, useUpdateCertificate, useDeliverCertificate, useGenerateCertificates,
} from "@/hooks/certificat/useCertificats";
import {
  useMyPassportData, usePassportDataByUsername, useDownloadMyPassport, useDownloadPassportByUsername,
...
vi.mock("@/services/certificat/CertificateService", () => ({
  default: {
    getAllCertificates: vi.fn(), getCertificatesByFormation: vi.fn(),
    getCertificatesByEmail: vi.fn(), createCertificate: vi.fn(),
    updateCertificate: vi.fn(), deliverCertificate: vi.fn(), generateCertificates: vi.fn(),
  },
  __esModule: true,
}));
vi.mock("@/services/certificat/SkillPassportService", () => ({
  default: {
    getMyPassportData: vi.fn(), getPassportDataByUsername: vi.fn(),
    downloadMyPassport: vi.fn(), downloadPassportByUsername: vi.fn(),
  },
  __esModule: true,
}));
...
describe("useCertificats", () => {
  ...
  it("useAllCertificates fetches", async () => {
    (CertificateService.getAllCertificates as ReturnType<typeof vi.fn>).mockResolvedValue([{ id: 1 }]);
  ...
  it("useCertificatesByFormation disabled without id", async () => {
    const { result } = renderHook(() => useCertificatesByFormation(undefined), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("useCertificatesByFormation fetches with id", async () => {
    ...
    const { result } = renderHook(() => useCertificatesByFormation(3), { wrapper: createWrapper() });
    await flushQuery(result);
...
describe("useSkillPassport", () => {
  ...
  it("usePassportDataByUsername disabled without username", async () => {
    const { result } = renderHook(() => usePassportDataByUsername(undefined), { wrapper: createWrapper() });
    await flushQuery(result);
    expect(result.current.isFetched).toBe(false);
  });
  ...
  it("usePassportDataByUsername fetches with username", async () => {
    ...
    const { result } = renderHook(() => usePassportDataByUsername("u1"), { wrapper: createWrapper() });
    await flushQuery(result);
  ...
  it("useDownloadPassportByUsername calls service", async () => {
    ...
    const { result } = renderHook(() => useDownloadPassportByUsername(), { wrapper: createWrapper() });
