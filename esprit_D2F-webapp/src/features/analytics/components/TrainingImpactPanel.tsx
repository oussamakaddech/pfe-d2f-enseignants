...
import { useTrainingImpact, useTrainingImpactFormations } from "../hooks/useAnalyticsQueries";
import type { FormationImpactRow } from "../types";
  ...
  return (
    <div>
      <Space size="large" wrap style={{ marginBottom: 16 }}>
        ...
        <Statistic title="Gain de niveau moyen" value={d.gain_niveau_moyen.toFixed(2)} suffix="pts" />
        <Statistic title="Réduction risque moyenne" value={`${Math.round(d.reduction_risque_moyenne * 100)}%`} valueStyle={{ color: "#52c41a" }} />
        <Statistic title="Risques réduits" value={d.nb_risque_reduit} valueStyle={{ color: "#52c41a" }} />
        <Statistic title="Risques augmentés" value={d.nb_risque_augmente} valueStyle={{ color: "#f5222d" }} />
      </Space>

      <Card size="small" style={{ borderRadius: 12 }} title="Top formations par impact (gain de niveau moyen)">
        <Table
          size="small"
          pagination={false}
          ...
          columns={[
            ...
            { title: "Type", dataIndex: "formation_type", render: (t: string | null) => (t ? <Tag>{t}</Tag> : "—") },
