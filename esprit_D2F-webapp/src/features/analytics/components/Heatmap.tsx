      ...
      render: (_: unknown, row: { departement: string }) => {
        ...
        const content = (
          <div style={{ fontSize: 12, lineHeight: 1.5 }}>
            <div><b>{comp}</b></div>
            <div>Écart moyen : <b>{pct}%</b> ({sev})</div>
            <div>Enseignants touchés : <b>{cell.enseignants_count}</b></div>
        ...
        return (
          <Tooltip title={content}>
            <Tag
              ...
              onClick={() =>
                onCellClick?.(row.departement, cell.competence_id, comp)
              }
            >
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {LEGEND.map((l) => (
            <span key={l.label} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, color: "#64748b" }}>
