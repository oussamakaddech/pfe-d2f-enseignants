...
if (-not (Test-Path $PYTHON)) {
    ...
    $riceEnv = "TESSERACT_CMD='$tessExe'; `$env:TESSDATA_PREFIX='$tessData'; "
    Start-Process powershell -ArgumentList "-NoExit", "-Command", `
        "Set-Location '$ROOT\esprit_D2F-rice'; `$env:TESSERACT_CMD='$tessExe'; `$env:TESSDATA_PREFIX='$tessData'; Write-Host 'Démarrage RICE Service sur port 8001 (LLM: mistral)...' -ForegroundColor Cyan; & '$PYTHON' -m uvicorn main:app --host 0.0.0.0 --port 8001"
    Start-Sleep -Seconds 2
}
