...
function Show-Coverage-Summary {
    ...
    foreach ($module in $modules) {
        ...
        Write-Host ("{0,-30} : {1:F2}%" -f $module, $coverage) -ForegroundColor $color
    }
    ...
    if ($averageCoverage -lt 80) {
        Write-Host "`n⚠ WARNING: Average coverage is below 80%!" -ForegroundColor Yellow
    } else {
        Write-Host "`n✓ Average coverage meets the 80% target!" -ForegroundColor Green
    }
}
