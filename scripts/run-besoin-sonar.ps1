# =============================================================================
# run-besoin-sonar.ps1 - Lancer l'analyse SonarQube UNIQUEMENT pour BESOINS-FORMATION
# =============================================================================
param(
...
function Get-SonarQubeToken {
    ...
    try {
        ...
        $tokenName = "besoin-scanner-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
...
Write-Host "Lancement de l'analyse Sonar pour : esprit_D2F-besoin-formation" -ForegroundColor Cyan

$token = Get-SonarQubeToken -Url $SonarUrl -Username $SonarUser -Secret $SonarSecret
if (-not $token) { Write-Host "Erreur Token"; exit 1 }

Push-Location ".\esprit_D2F-besoin-formation"
try {
    ...
    .\mvnw.cmd "sonar:sonar" "-Dsonar.projectKey=besoinsformation-service" "-Dsonar.host.url=$SonarUrl" "-Dsonar.token=$token" "-Dsonar.coverage.jacoco.xmlReportPaths=target/site/jacoco/jacoco.xml"
    
    Write-Host "`n[OK] Analyse terminée !" -ForegroundColor Green
    Write-Host "Accedez au rapport : $SonarUrl/dashboard?id=besoinsformation-service" -ForegroundColor Cyan
} finally {
    Pop-Location
}
